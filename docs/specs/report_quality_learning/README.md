...
- `POST /report-workflows/learning/correction-artifacts/pilot-export/package`
  ...
  - `create_report_quality_pilot_pack.py --source-package`는 ZIP을 풀지 않고 같은 package validator를 다시 실행한 뒤 embedded JSONL과 receipt를 기존 source-bound import 흐름에 연결한다.
...
생성된 `SOURCE_MANIFEST.json` v2는 원본 package, embedded JSONL과 copied receipt의 SHA-256, request ID, tenant, artifact 순서를 남긴다. 같은 batch ID의 기존 출력이 있으면 import를 거부하며, 이후 `sync_report_quality_pilot_pack.py`도 receipt와 manifest를 다시 검증하고 같은 순서를 적용한다. 기존 JSONL+receipt 개별 입력도 호환 경로로 유지한다.
...
5. export를 source-bound 파일럿 review pack으로 가져온다.
   ...
   python3 scripts/create_report_quality_pilot_pack.py \
     ...
     --output-root reports/report-quality
   ```
6. `SOURCE_MANIFEST.json` v2와 copied `SOURCE_EXPORT_RECEIPT.json`이 원본 package 및 embedded JSONL SHA-256, request ID, tenant, artifact 순서를 함께 보존하는지 확인한다.
