...
- `POST /report-workflows/learning/correction-artifacts/pilot-export`
  ...
  - Preview와 export audit은 같은 SHA-256, artifact count, server verification state를 tenant audit log에 남긴다.
...
4. UI에서 서로 다른 ready artifact 3~5개를 선택해 `report_quality_pilot_artifacts_<sha12>.jsonl`로 내려받는다.
