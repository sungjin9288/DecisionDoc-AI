...
4. UI에서 서로 다른 ready artifact 3~5개를 선택해 `report_quality_pilot_review_package_<sha12>.zip`을 내려받고, 내부의 `report_quality_pilot_artifacts_<sha12>.jsonl`과 `report_quality_pilot_receipt_<sha12>.json`을 같은 디렉터리에 푼다. 기존 JSONL + receipt 개별 다운로드도 호환 경로로 유지한다.
