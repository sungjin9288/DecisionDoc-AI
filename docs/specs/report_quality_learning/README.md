...
- `POST /report-workflows/learning/correction-artifacts/pilot-export`
  ...
  - Admin Ops audit 화면에서 두 action을 직접 필터링하고 request ID, 전체 SHA-256, artifact count, preview verification state를 확인할 수 있다.
  - Audit CSV는 현재 action/result 필터와 전체 detail JSON을 보존한다. Pilot 식별자는 별도 column으로도 제공하며 당일 로그와 1,000건을 넘는 결과를 포함한다.
...
생성된 `SOURCE_MANIFEST.json` v2는 원본 JSONL과 copied receipt의 SHA-256, request ID, tenant, artifact 순서를 남긴다. 같은 batch ID의 기존 출력이 있으면 import를 거부하며, 이후 `sync_report_quality_pilot_pack.py`도 receipt와 manifest를 다시 검증하고 같은 순서를 적용한다.
...
4. UI에서 서로 다른 ready artifact 3~5개를 선택해 `report_quality_pilot_artifacts_<sha12>.jsonl`과 `report_quality_pilot_receipt_<sha12>.json`을 함께 내려받는다.
...
5. export를 source-bound 파일럿 review pack으로 가져온다.
   ...
   python3 scripts/create_report_quality_pilot_pack.py \
     ...
     --output-root reports/report-quality
   ```
6. `SOURCE_MANIFEST.json` v2와 copied `SOURCE_EXPORT_RECEIPT.json`이 원본 JSONL SHA-256, request ID, tenant, artifact 순서를 함께 보존하는지 확인한다.
