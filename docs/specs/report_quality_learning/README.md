...
- `POST /report-workflows/learning/correction-artifacts/pilot-export`
  ...
  - Admin Ops audit 화면에서 두 action을 직접 필터링하고 request ID, 전체 SHA-256, artifact count, preview verification state를 확인할 수 있다. 전체 건수와 현재 범위를 표시하며 이전·다음 페이지에서도 같은 filter를 유지한다.
