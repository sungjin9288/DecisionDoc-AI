...
수신 package 검증 화면의 `명령 복사`는 package SHA-256 기반 batch ID와 검증한 파일명을 사용한다. 기본 경로는 `~/Downloads`이며, 실제 위치가 다르면 `--source-package`만 수정한다. 출력 batch가 이미 있으면 기존 증거를 삭제하거나 덮어쓰지 말고 `--batch-id`를 새 값으로 바꾼다.
...
승인할 artifact만 `decision=accepted`로 바꾸고, 반려나 보완 요청은 `changes_requested` 또는 `rejected`로 둔다. Generator는 기존 template, workspace, symlink를 덮어쓰지 않는다. Decision template은 write-once publication을 사용하므로 사전 검사 뒤 같은 경로가 생겨도 먼저 존재한 파일을 보존한다. Draft를 직접 바꿔 기존 binding이 stale해졌다면 기존 파일을 보존하고 `review_decisions.refreshed.json`, `HUMAN_REVIEW_WORKSPACE.refreshed.html`처럼 새 경로에 template과 workspace를 다시 만든다.
...
`--browser-draft`는 report type, schema, `training_authorized=false`, source/draft binding, 전체 decision batch를 쓰기 전에 확인한다. 통과하면 내려받은 파일의 정확한 바이트를 `review_decisions.browser-draft.<sha12>.json`으로 pack에 보존하고, 같은 suffix의 `review_decision_application_receipt.<sha12>.json`을 자동 생성한다. SHA 보관본은 write-once publication을 사용하며 동시 경로 충돌이 생기면 기존 파일을 보존하고 draft 적용 전에 중단한다. 적용으로 draft hash나 검토 상태가 바뀌면 `HUMAN_REVIEW_WORKSHEET.md`와 `human_review_manifest.json`도 현재 pack binding으로 즉시 갱신한다. 외부 draft, 보관본, receipt, worksheet, manifest가 symlink이거나 기존 SHA 이름과 충돌하면 덮어쓰지 않는다. Dry-run이나 실패 batch에서는 보관본, draft, receipt, worksheet, manifest를 변경하지 않는다.
...
이 demo의 review 입력은 simulated data이며 receipt도 `human_review_claimed=false`를 유지한다. Read-only checker는 receipt schema, artifact identity, SHA-256, stage order, simulated/no-training/secret boundary를 재검증하지만 삭제된 temporary artifact 자체를 다시 검증하지는 않는다. Provider key와 temporary artifact는 실행 중에만 사용 가능한 context에서 격리되고 외부 API, AWS runtime, dataset upload, training, model promotion은 실행하지 않는다.
