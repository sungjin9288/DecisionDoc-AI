...
- captured provider-backed Agent runs can carry a browser-generated operation identity. The selected backend claims it before provider execution, binds it to the canonical request hash, replays only the stored trajectory-bound success, and returns `409` for changed input or an unfinished prior attempt. Corrupt retry state returns `503` before provider execution. An API-key protected tenant-scoped status read exposes only operation ID, state, timestamps, replay availability, and next action, while its audit omits private owner/hash/result data. After a lost response, the browser reads status without cache and requires its schema, exact operation identity, state-specific fields, read-only flag, and provider-call denial to agree. Mismatched, unavailable, or running state keeps the tenant and original payload only in current-page memory; the Agent button and explicit status recheck share one recovery promise, so neither starts a new operation. Verified success alone exact-replays the original payload. Before a captured POST, an exact three-field marker stores only schema version, tenant ID, and operation ID in a tenant-scoped same-origin storage key. A tenant-scoped Web Lock serializes marker claim where supported, so simultaneous tabs converge on one POST while another tab, reload, or a page reopened after owner-tab close performs status-only inspection. Different tenant markers coexist without foreign read/write/clear deleting each other, and H96 base-key markers remain owner-readable. Authorized switching persists the next tenant before clearing previous-context evidence; storage failure preserves the current tenant, draft, recovery promise, and marker. Payload is never persisted, explicit release confirms that backend execution continues, invalid scoped markers are removed, and current auth-context termination clears its marker. Shared storage failure falls back to tenant-scoped tab storage; failure of both stores leaves same-page execution usable without reload or cross-tab protection. Uncaptured runs keep their existing behavior. Payload-free exact replay, cross-device coordination, atomic simultaneous claims without Web Locks, process-crash recovery, semantic deduplication across different operation IDs, exactly-once provider execution, and receipt GC are outside this contract
...
- if the other tab's final signed user or tenant differs from the current page, the receiving page reloads once and
  rebuilds page-memory evidence under that session; same-identity token rotation does not reload
...
- training execution request latest-response static gate: 1 passed, 1 warning
...
- pending Agent recovery binding static gate: 2 passed, 159 deselected, 1 warning
- pending Agent recovery binding Chromium gate: 1 passed, 56 deselected
- reload status marker static gate: 1 passed, 160 deselected, 1 warning
- reload status marker Chromium gate: 1 passed, 57 deselected
- cross-tab shared marker Chromium gate: 2 passed, 58 deselected
...
- password credential auth/identity/security/infrastructure gate: 312 passed, 1 warning
- credential-version cross-tab focused Chromium gate: 3 passed
- auth/tenant related Chromium gate: 9 passed, 54 deselected
- full main-flow Chromium gate: 71 passed, 1 skipped
...
- full repository non-live gate: 4292 passed, 2 skipped, 4 deselected, 1 warning
