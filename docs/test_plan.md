...
Cross-tab auth refresh 회귀는 같은 browser context의 두 page를 사용한다. 첫 page의 refresh 응답을 지연하고 둘째 page가 동일 refresh token으로 replacement access token을 commit한 뒤 실제 `storage` event를 관측한다. 첫 page의 session revision이 증가하고 늦은 응답이 `superseded`로 끝나 replacement token을 덮어쓰지 않아야 한다. `dd_ops_key` 같은 unrelated storage 변경은 revision을 바꾸지 않아야 하며, static contract는 access token, refresh token, tenant ID와 full clear만 allowlist에 포함하는지 고정한다.
