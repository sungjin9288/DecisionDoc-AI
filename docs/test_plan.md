...
Governance inventory browser 회귀는 기존 Governance action이 training summary와 inventory를 함께 조회하는지, inventory 요청이 Ops-key가 포함된 GET인지, attention 문제 상세와 무삭제·재확인 경계가 표시되는지 확인한다. 재조회 뒤 clean 상태로 전환되는지, desktop/mobile에서 count와 경계 문구가 겹치지 않는지, 390px viewport에 가로 overflow가 없는지도 실제 Chromium으로 검증한다.
