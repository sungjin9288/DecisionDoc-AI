...
| 시험 항목 | 대표 시험 파일 | 비고 |
|-----------|----------------|------|
| OWASP Top 10 대응 | `tests/test_security.py` | XSS, auth, SSRF, project/approval/procurement/report-workflow/model-registry의 tenant 필수 조회, tenant-bound billing/user/invite/style/SSO/template 상태와 cross-tenant IDOR 차단 |
