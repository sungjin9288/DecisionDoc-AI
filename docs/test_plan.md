...
| 인증/인가 | `tests/test_auth_*.py`, `tests/storage/test_auth_session_store.py`, `tests/test_identity_store_integrity.py`, `tests/test_audit.py`, `tests/e2e/test_main_flow.py` | JWT, API key, tenant/auth 흐름, 계정·초대 state 무결성, credential epoch와 persisted exact-session logout authority |
...
Procurement reviewed package 테스트는 completed receipt만 허용하고 accepted/changes-requested/rejected 결과를 모두 보존하는지 확인한다. v2 project package는 packet, receipt, reviewer attestation, manifest의 fixed entry order와 SHA256/size를 재검증하고 trusted tenant/project/stable-user scope, current session principal, renamed username 기록, API key/Ops key/sessionless JWT/viewer/mismatched reviewer 거부, full audit rationale/session/token/IP/User-Agent redaction을 검증한다. Packet creation, inbox, project history와 package download는 session-bound role matrix를 검증한다: admin tenant-wide access, member self-assignment와 own v2 records only, foreign reviewer/project/package denial, v1 package admin-only, authorized count projection, private identity-free HTTP/DOM, foreign artifact corruption isolation, unauthorized package byte read 차단, stale auth/tenant response와 packet/package blob discard. 동일 assignee의 동일 payload replay는 persisted package를 반환하고 changed replay는 거부하며, confirmed CAS loser artifact는 제거한다. v1 standalone CLI package와 completed legacy record의 HTTP list/download는 호환성을 유지한다.
...
`/auth/logout`은 current session만 폐기해 separate login을 유지하고 viewer도 사용할 수 있어야 한다. Inventory는 local/fake-S3 tenant prefix 전체를 strict 검증하고 현재 user·credential version의 active record만 newest-first `no-store`로 반환해야 한다. Label mutation은 active owner만 허용하고 foreign/missing/stale/revoked/expired target을 같은 `404`로 숨긴다. Selected revoke는 current target `409`, foreign/missing target `404`, malformed body `422`, already-revoked owner retry `200`으로 동작해야 한다. Other-session과 all-device bulk revoke는 strict confirmation과 whole-prefix validation을 요구하며 full revoke는 current를 마지막에 써야 한다. Prefix 손상은 mutation 전 `503`으로 닫고, 중간 failure의 partial progress와 current-write response loss를 multi-object transaction으로 과장하지 않는다.

Audit은 `user.logout`, session list/label/revoke actions와 aggregate count만 남기고 access token, refresh token, session ID와 label을 복사하지 않아야 한다. Chromium은 session ID를 DOM에 넣지 않은 채 label과 current/other·시작·만료 정보를 표시하고, label/save/revoke action의 single-flight와 stale-response guard를 확인한다. All-device success만 browser credential과 page-memory evidence를 정리하며 `503`은 이를 보존해야 한다.

Auth-session retention preview 회귀는 local/fake-S3에서 active, recent inactive와 cutoff 이전 expired/revoked record를 함께 만들고 `auth-session-retention-preview.v1` aggregate가 정확한지 확인한다. 응답은 `read_only=true`, `deletion_authorized=false`, `no-store`이며 user ID, session ID와 label을 포함하지 않아야 한다. 호출 전후 모든 object bytes가 같아야 하고, malformed object 하나라도 있으면 원본을 바꾸지 않은 채 `503`으로 fail closed 해야 한다. API는 admin JWT 또는 Ops key만 허용하고 일반 user를 `403`, 미인증 요청을 `401`, 1~3650일 밖의 query를 `422`로 거부해야 한다. Audit은 retention 기간과 inspected/eligible count만 기록한다. User-Agent/IP inventory, admin mass revoke, 실제 expired-session 삭제·scheduler, immediate push와 외부 identity/provider runtime은 검증 범위가 아니다.

Ops retention UI 회귀는 로그인 또는 Ops key 상태에서 30/90/180/365일 selector가 aggregate를 표시하는지 확인한다. 미인증 최초 진입은 network request를 만들지 않아야 하며, 화면에는 icon refresh 외 mutation control이 없어야 한다. Chromium은 정상 응답, 연속 요청의 역순 completion, `deletion_authorized=true` 변조 응답, 390x844 viewport를 검증한다. 최신 request generation과 tenant가 일치하는 정상 contract만 렌더링하고 invalid contract는 metric 없이 fail closed error로 바뀌어야 한다.

Auth-session retention comparison 회귀는 local/fake-S3에서 여러 inactive age를 만들고 한 번의 validated record inspection으로 30/90/180/365일 정책이 정확히 계산되는지 확인한다. `auth-session-retention-comparison.v1`은 네 정책의 exact order, 공통 inspected/active count, 정책별 eligible reason과 retained count를 제공하고 `read_only=true`, `deletion_authorized=false`, `snapshot_atomic=false`, `requires_recheck_before_mutation=true`, `no-store`를 유지해야 한다. 호출 전후 object bytes는 같아야 하며 corrupt prefix는 원본 보존 `503`으로 닫는다. Audit에는 policy day와 eligible count 배열만 남기고 식별자를 포함하지 않는다. Chromium은 최초 comparison 1회, selector 변경의 추가 request 0건, refresh 역순 completion, contract 변조, 정책별 표, 390x844 overflow 0을 검증한다.

Auth-session retention review handoff 회귀는 local/fake-S3에서 `retention_days` 30/90/180/365만 허용하고 한 strict inspection 결과를 tenant-bound `auth-session-retention-review-handoff.v2`으로 감싸는지 확인한다. `tenant_id`, `selected_policy_days`, full comparison, canonical `comparison_sha256`, `review_only=true`, `policy_change_authorized=false`, `deletion_authorized=false`, `scheduler_authorized=false`, `snapshot_atomic=false`, `requires_recheck_before_mutation=true`, `handoff_persisted=false`가 모두 정확해야 한다. v1 historical artifact는 recheck 입력에서 거부한다. API는 admin JWT 또는 Ops key만 허용하고 corrupt prefix를 원본 보존 `503`으로 닫으며 JSON attachment, `no-store`, `nosniff`, exact body SHA-256 header를 반환해야 한다. Object bytes와 audit은 호출 전후 aggregate-only로 유지하고 user/session/label/token을 포함하지 않아야 한다. Chromium은 새 inspection의 verified same comparison만 화면에 반영하고 exact bytes만 download하며 body/header hash, comparison hash, authority flag, nested-schema drift, stale request 또는 tenant mismatch를 fail closed로 확인한다. 삭제, scheduler, policy 저장/적용, mass revoke와 외부 provider/runtime은 범위 밖이다.

Auth-session retention freshness recheck 회귀는 admin/Ops dependency가 malformed body parsing보다 먼저 실행되고 exact `auth-session-retention-recheck-request.v1`의 v2 source handoff와 canonical source SHA-256만 받는지 확인한다. API는 source/current handoff와 SHA-256, stable aggregate fingerprint SHA-256, `aggregate_status=changed|unchanged`, `fingerprint_algorithm=sha256`, `volatile_fields_excluded`, `aggregate_only=true`, `review_only=true`, false policy/delete/scheduler authority, `snapshot_atomic=false`, `requires_recheck_before_mutation=true`, `recheck_persisted=false`의 exact `auth-session-retention-recheck-receipt.v1`을 `no-store`, `nosniff`, exact body SHA header와 safe attachment filename으로 반환해야 한다. Malformed/hash drift/v1/foreign tenant는 fail closed하고 corrupt current prefix는 원본 보존 `503`, changed aggregate는 정상 `200`이어야 한다. Session bytes는 recheck 전후 같아야 하며 audit은 selected policy, current aggregate count, status와 read-only/snapshot boundary만 남긴다. Chromium은 page-memory source만 사용하고 selector, refresh, tenant·auth-context change와 newer request가 stale completion을 버리며, source/current schema, body hash, aggregate hash, tenant, selected policy 또는 authority flag가 변조되면 panel과 download를 바꾸지 않아야 한다. `unchanged`는 aggregate equivalence일 뿐 session set identity나 mutation safety가 아니다.

Auth-session retention operator review disposition 회귀는 `POST /admin/auth-sessions/retention-handoff/review-disposition`의 admin/Ops dependency가 malformed body보다 먼저 실행되고 exact `auth-session-retention-review-disposition-request.v1`만 받는지 확인한다. Route는 H117 source receipt 전체의 exact schema, outer/source/current SHA-256, aggregate fingerprint, tenant, selected policy, aggregate status와 false authority flags를 fail closed로 재검증한다. `auth-session-retention-review-disposition-receipt.v1`은 source receipt, current handoff/fingerprint SHA-256, selected policy, aggregate status, allowlisted disposition, canonical binding SHA-256와 `receipt_status=issued`, `review_only=true`, `reviewer_identity_bound=false`, `approval_granted=false`, `execution_authorized=false`, `policy_change_authorized=false`, `deletion_authorized=false`, `scheduler_authorized=false`, `mass_revoke_authorized=false`, `snapshot_atomic=false`, `requires_recheck_before_mutation=true`, `decision_receipt_persisted=false`만 정확히 포함해야 한다. unchanged에는 `acknowledged_unchanged|review_deferred`, changed에는 `new_handoff_required|review_deferred`만 허용하며 replay는 같은 bytes와 SHA-256으로 수렴한다. 인증 뒤 handler는 retention 대상 session/policy state를 조회·변경하지 않고 audit에는 policy, status, disposition, source/emitted hash와 review-only flag만 남긴다. Chromium은 page-memory H117 source, body/header hash, nested binding, tenant, policy, status, matrix, false authority flags를 모두 통과한 경우에만 render/download하며 policy/disposition selector, refresh, handoff, recheck, tenant/auth/Ops-key context change 또는 newer request 뒤 stale completion을 버려야 한다. approval, deletion, scheduler, mass revoke, provider/runtime 호출은 범위 밖이다.
...
Password credential 회귀는 legacy user record의 missing version을 0으로 읽고 bool·음수·문자열 version은 원본 bytes를 바꾸지 않은 채 fail closed 처리하는지 확인한다. Password hash 교체와 version 증가가 local/fake-S3 CAS에서 profile concurrent update를 잃지 않고 한 번 확정돼야 한다. 변경 전 access/refresh token과 올바르게 서명됐지만 malformed version을 가진 token은 `401`, 변경 응답의 새 pair와 새 password login은 성공해야 한다. Chromium은 profile modal의 실제 변경 동선이 새 pair를 browser에 commit하고, same-origin 다른 tab이 version mismatch에서 reload해 이전 page-memory draft를 제거하는지 확인한 뒤 테스트 계정 password를 복원한다. 실제 identity provider나 paid provider API는 호출하지 않는다.
