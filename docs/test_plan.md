...
Audit은 `user.logout`, session list/label/revoke actions와 aggregate count만 남기고 access token, refresh token, session ID와 label을 복사하지 않아야 한다. Chromium은 session ID를 DOM에 넣지 않은 채 label과 current/other·시작·만료 정보를 표시하고, label/save/revoke action의 single-flight와 stale-response guard를 확인한다. All-device success만 browser credential과 page-memory evidence를 정리하며 `503`은 이를 보존해야 한다. User-Agent/IP inventory, admin mass revoke, automatic expired-session GC, immediate push와 외부 identity/provider runtime은 검증 범위가 아니다.
