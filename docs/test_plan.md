...
Per-session auth 회귀는 register·login·invite·LDAP·SAML·GCloud·password-change token pair가 같은 random session ID와 `auth-session.v1` object를 공유하고 refresh가 그 identity를 유지하는지 확인한다. Store는 strict exact schema, duplicate-key rejection, tenant/user/version/expiry binding과 conditional create/CAS를 사용해야 한다. 12-way concurrent exact revoke는 한 `revoked_at`으로 수렴하고 wrong owner/version, malformed identity, corrupt state와 backend failure는 원본 bytes를 바꾸지 않아야 한다. `/auth/logout`은 current session만 폐기해 separate login을 유지하고 viewer도 사용할 수 있어야 한다. Legacy sessionless token은 protected request에서 호환하되 exact logout을 `409`로 거부한다. Audit은 `user.logout` result만 남기고 access token·refresh token·session ID를 복사하지 않아야 한다. Chromium은 local cleanup이 revoke 응답보다 먼저 완료되고, 같은 session을 복사한 다른 context가 protected request와 열린 SSE에서 거부되며, endpoint `503`에서도 local logout은 유지되고 서버 폐기 미확인 경고가 표시되는지 확인한다. Session inventory, admin mass revoke, automatic expired-session GC, immediate push와 외부 identity/provider runtime은 검증 범위가 아니다.
