...
Per-session auth 회귀는 register·login·invite·LDAP·SAML·GCloud·password-change token pair가 같은 random session ID와 `auth-session.v1` object를 공유하고 refresh가 그 identity를 유지하는지 확인한다. Store는 strict exact schema, duplicate-key rejection, tenant/user/version/expiry binding과 conditional create/CAS를 사용해야 한다. 12-way concurrent exact revoke는 한 `revoked_at`으로 수렴하고 wrong owner/version, malformed identity, corrupt state와 backend failure는 원본 bytes를 바꾸지 않아야 한다. `/auth/logout`은 current session만 폐기해 separate login을 유지하고 viewer도 사용할 수 있어야 한다. Session inventory는 local/fake-S3 tenant prefix의 모든 direct child를 strict 검증하고 현재 user·credential version의 unrevoked·unexpired record만 newest-first로 반환해야 한다. API는 current 표시와 `no-store`를 제공하고 legacy sessionless caller를 `409`로 막아야 한다. Selected revoke는 viewer를 포함한 owner가 다른 session 하나만 종료하고 current target `409`, foreign/missing target `404`, malformed/extra body `422`, already-revoked owner retry `200`으로 동작해야 한다. Bulk revoke는 strict `confirm=true`만 받고 local/fake-S3에서 current와 foreign user, 다른 credential version을 보존한 채 검증 snapshot의 다른 active session을 모두 종료해야 한다. Prefix 손상은 어떤 candidate도 바꾸기 전에 `503`으로 닫고, 반복 요청은 count 0으로 수렴해야 한다. Audit은 `user.logout`, `user.session_list`, `user.session_revoke`, `user.session_revoke_others` result와 bulk count만 남기고 access token·refresh token·session ID를 복사하지 않아야 한다. Chromium은 local cleanup이 logout 응답보다 먼저 완료되고, 같은 session을 복사한 다른 context가 protected request와 열린 SSE에서 거부되며, profile의 selected/bulk revoke가 current browser를 유지하는지 확인한다. Session ID는 DOM에 없어야 하고 개별·일괄 action이 같은 single-flight를 사용하며 늦은 list와 이전 revoke의 `finally`는 최신 목록과 lock을 덮어쓰지 않아야 한다. Prefix scan과 bulk write는 multi-object transaction이 아니므로 중간 failure의 부분 진행과 snapshot 뒤 create 가능성을 문서화한다. User-Agent/IP inventory, current-inclusive 전체 로그아웃, admin mass revoke, automatic expired-session GC, immediate push와 외부 identity/provider runtime은 검증 범위가 아니다.
