...
| Project and approval state integrity | Project/approval record를 tenant별 local/S3 object에 결속하고 missing-state 무부작용, blank·invalid UTF-8·backend failure·owned schema/duplicate identity를 원본 보존 상태로 차단한다. 서로 다른 virtual base의 동일 S3 object도 process-local logical lock을 공유하고 corrupt approval state는 domain 400으로 축소하지 않는다 | `app/storage/project_store.py`, `app/storage/approval_store.py`, `tests/test_project_approval_store_integrity.py` |
| Report workflow state integrity | Planning·slide·visual asset·approval·promotion state를 tenant별 local/S3 object에 결속하고 blank·invalid UTF-8·backend failure·workflow/nested identity drift를 원본 보존 상태로 차단한다. 서로 다른 virtual base의 동일 S3 object도 process-local logical lock을 공유하고 persisted error를 domain 400으로 축소하지 않는다 | `app/storage/report_workflow/`, `tests/test_report_workflow_store_integrity.py` |
...
| Billing authority integrity | tenant별 plan·status·Stripe identity를 local/S3 state에 결속하고 malformed state·unknown value를 원본 보존 상태로 차단하며 tenant/auth resolution 뒤 metered request 한도를 적용한다. Checkout은 환경별 recurring Price ID와 subscription metadata를 사용한다 | `app/storage/billing_store.py`, `app/middleware/billing.py`, `app/routers/billing.py`, `app/services/billing_service.py`, `tests/test_billing_store_integrity.py` |
...
| Quality learning state integrity | tenant별 feedback·eval evidence·runtime prompt override를 local/S3 state에 결속하고 malformed state·duplicate key·owned schema drift를 원본 보존 상태로 차단한다. Route와 generation prompt context는 앱 backend를 공유하며 손상된 품질 상태를 빈 힌트로 바꾸지 않는다 | `app/storage/feedback_store.py`, `app/eval/eval_store.py`, `app/storage/prompt_override_store.py`, `app/domain/schema.py`, `tests/test_quality_learning_store_integrity.py` |
...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | `pytest tests/ -m "not live" -q` -> `4002 passed, 2 skipped, 4 deselected` (2026-07-17 실측) |
| GitHub Actions CI | 최근 확인한 main 자동화 증적: commit `e286f2f`, CI `29502322163` success (`3554 passed, 5 skipped`) |
| GitHub Actions CD | 최근 확인한 main 자동화 증적: commit `e286f2f`, CD `29502322086` success. image digest는 `sha256:c72c286bcaabea41d59081631e4cf5ef6a1496f2f0cafaf01a96114732e6a384`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
