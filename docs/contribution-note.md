...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4239 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-21 H87 실측) |
| GitHub Actions CI | 마지막으로 확인한 main 자동화 증적: commit `7ea8344`, CI `29778202677` success (`4237 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 확인한 main 자동화 증적: commit `7ea8344`, CD `29778202646` success. image digest는 `sha256:4c7c6271befd2122914db5af296d719eae7760d0c6b2fee3ee3d3007919a7865`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
