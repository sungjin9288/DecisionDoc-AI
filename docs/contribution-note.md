...
| DocumentOps browser freshness | Trajectory 목록 응답을 요청 시점의 tenant, task/review filter, search query, ordering에 결속해 debounce 중 늦은 이전 검색 결과가 현재 조건 아래 렌더링되지 않게 한다 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
| DocumentOps write action single-flight | Export·freeze·dry-run approval·execution request·audit export와 provider-backed Agent 실행 control을 action 완료 전까지 비활성화해 연속 클릭이 같은 browser에서 duplicate record 또는 provider request를 시작하지 못하게 하고, read-only refresh는 독립적으로 유지한다 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
| DocumentOps governance write idempotency | Freeze·dry-run approval·execution request·audit export의 optional `operation_id`를 canonical payload hash와 private metadata에 결속한다. 동일 payload replay는 원래 size·SHA-256·tenant·identity가 검증된 artifact를 반환하고, 다른 payload 재사용은 `409`로 차단한다. Local/fake-S3 CAS는 한 authoritative record로 수렴하지만 metadata 확정 전 concurrent loser artifact까지 자동 삭제하지는 않는다 | `app/storage/trajectory/artifact_state_mixin.py`, `app/storage/trajectory/freeze_mixin.py`, `app/storage/trajectory/training_approval_mixin.py`, `app/storage/trajectory/training_execution_mixin.py`, `app/storage/trajectory/training_audit_mixin.py`, `app/routers/document_ops_agent.py`, `tests/storage/test_trajectory_artifact_authority.py`, `tests/test_document_ops_agent_api.py` |
| DocumentOps captured Agent retry recovery | Trajectory capture를 선택한 Agent run의 optional `operation_id`를 provider 호출 전에 selected backend에서 claim한다. Exact payload replay는 저장된 trajectory-bound 결과를 반환해 provider와 usage 기록을 반복하지 않고, changed/running/failed/corrupt 상태는 자동 재실행 전에 차단한다. Tenant-scoped status는 private receipt를 redacted projection으로만 보여주고 audit에 남긴다. Browser는 schema·operation·state contract를 모두 확인하고 lost response 뒤 terminal success에서만 같은 payload를 replay한다. Mismatched·unavailable·running 상태는 current-page memory에 보존하며 Agent 버튼과 재확인 버튼의 동시 요청도 recovery promise 하나로 수렴한다. Captured POST 전 payload 없는 exact 3-field marker를 tenant-scoped same-origin shared storage에 남기고 tenant별 Web Lock claim으로 동시 tab 중 owner 하나만 POST를 시작하게 한다. 다른 tab과 owner tab 종료 뒤 다시 연 화면은 status만 확인하며, backend 실행 비취소 경고를 승인한 explicit release에서만 marker를 제거한다. Foreign tenant read/write/clear는 다른 tenant marker를 보존하고 tenant 전환은 previous marker만 정리하며 H96 base-key marker는 owner만 호환해 읽는다. Shared storage가 막히면 tenant-scoped tab fallback을 사용하고 두 storage가 모두 막히면 same-page 실행만 유지한다. Uncaptured run, payload 없는 exact replay, 다른 browser/device, Web Locks 미지원 환경의 완전 동시 claim, process-crash recovery, cross-ID semantic deduplication과 receipt GC는 범위 밖이다 | `app/storage/document_ops_run_operation_store.py`, `app/services/document_ops_service.py`, `app/routers/document_ops_agent.py`, `app/middleware/document_ops_audit.py`, `app/static/index.html`, `tests/storage/test_document_ops_run_operation_store.py`, `tests/test_document_ops_agent_api.py`, `tests/test_audit.py`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
| Browser tenant context failure atomicity | Signed-token tenant sync와 승인된 selector switch는 `dd_tenant_id` write가 성공한 뒤에만 current context와 previous-context evidence를 바꾼다. Browser storage write가 실패하면 기존 tenant, 미저장 review draft, pending recovery promise와 payload-free marker를 보존하며, storage 값을 권한 근거로 사용하지 않는다 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
| Browser auth session failure atomicity | Login·register·refresh·LDAP login의 token과 signed tenant 저장을 공통 helper로 모았다. Claims를 먼저 검증하고 access/refresh token 뒤 tenant를 commit하며, 실패하면 이전 browser credential과 tenant를 복원한다. 동시 401은 refresh 1회에 합류하고, generic mutating request는 refresh 뒤 자동 replay하지 않으며, 늦은 이전 refresh 응답은 교체된 session을 덮어쓰지 않는다 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
| Persisted user authorization authority | Protected request와 `/events` query access token을 tenant `UserStore`의 현재 role·`is_active`에 결속한다. Role 변경은 token 만료 전에도 현재 RBAC로 반영하고 비활성 사용자의 기존 token은 거부하며, browser는 다른 tab의 role mismatch에서 reload해 이전 authorization page memory를 제거한다. Store failure는 fail closed 처리한다 | `app/services/auth_service.py`, `app/middleware/auth.py`, `app/routers/events.py`, `app/static/index.html`, `tests/test_auth.py`, `tests/e2e/test_main_flow.py` |
...
| Per-session auth revocation | 새 token pair를 tenant-scoped persisted session identity에 결속하고 refresh에서도 유지한다. `/auth/logout`은 current session만 CAS 폐기해 separate login을 유지하고, same-session request·refresh·SSE를 차단한다. Browser는 server revoke와 병렬로 local credential·draft·pending recovery를 즉시 정리하며 endpoint failure를 unconfirmed warning으로 구분한다. Corrupt state는 원본 보존 `503`, legacy sessionless exact logout은 `409`이고 audit은 token/session ID를 복사하지 않는다. Session inventory·mass revoke·expired-session GC·즉시 push는 범위 밖이다 | `app/storage/auth_session_store.py`, `app/services/auth_service.py`, `app/middleware/auth.py`, `app/routers/auth.py`, `app/routers/sso.py`, `app/routers/admin/_invite.py`, `app/static/index.html`, `tests/storage/test_auth_session_store.py`, `tests/test_auth.py`, `tests/test_audit.py`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
...
| DocumentOps review operations | tenant-scoped trajectory를 summary-first로 탐색하고 선택 상세만 불러온 뒤 사람 점수와 메모를 기록하며, 상세 열람·review 결정은 입력·초안·메모를 제외한 append-only audit으로 추적한다. Governance review overview는 training governance, selected-backend artifact inventory, reviewer sign-off를 독립적으로 읽어 경계·무결성·blocker·sign-off 우선순위와 다음 검토 행동을 보여준다. Export·freeze·dry-run approval·execution request·audit 저장 또는 planning provider/model 변경 뒤에는 기존 overview를 이전 관측으로 낮추고, 성공한 새 조회에서만 fresh 상태를 복구한다. Trajectory Stats, task-filtered Reviewed SFT export 목록, Training Readiness, Training Execution Request Records는 same-tenant request version으로 이전 success/error를 폐기해 최신 count, artifact 목록, freeze 승인 대상, two-person guard 기록만 유지한다. Execution request 저장 뒤의 새 조회는 저장 전 진행 중 read를 무효화한다. Training Audit Checklist도 request·tenant·provider/model 조건이 모두 최신일 때만 렌더링하고 planning 변경 시 기존 `Audit 저장` control을 제거하며, audit 저장 뒤에는 진행 중 이전 read가 새 evidence를 가리지 못하게 한다. Adapter Contract과 Rehearsal은 각자 request version·tenant·planning query를 현재 입력과 대조해 이전 config 안전 표시, artifact reference, 오류가 현재 provider/model을 대신하지 못하게 하고 조건 변경 시 열린 결과를 `RECHECK REQUIRED`로 전환한다. SFT Export Preview와 artifact 목록은 요청 시점의 task에, Training Plan Preview는 정확한 provider/model query에 결속하고 입력 변경 시 기존 evidence와 진행 중 응답을 무효화한다. DocumentOps Agent 실행 결과는 latest initiated run과 tenant에 결속하며, 늦은 이전 저장 완료는 현재 결과와 filter/page를 덮지 않고 목록 refresh와 알림으로만 관측한다. Governance 조회와 sign-off handoff 다운로드도 별도 action/resource로 추적하지만 audit detail에는 surface·aggregate status·read-only 여부만 남기고 fingerprint·source report·reviewer record를 복사하지 않는다. 합성 snapshot은 atomic이 아니며 cleanup이나 외부 실행 action을 제공하지 않는다 | `app/routers/document_ops_agent.py`, `app/services/document_ops_service.py`, `app/services/document_ops_governance.py`, `app/storage/trajectory/artifact_inventory_mixin.py`, `app/static/index.html`, `app/middleware/audit.py`, `app/middleware/document_ops_audit.py`, `tests/storage/test_trajectory_artifact_authority.py`, `tests/test_document_ops_governance.py`, `tests/test_document_ops_agent_api.py`, `tests/e2e/test_main_flow.py` |
...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 빈 값으로 고정하고 provider capability를 mock으로 강제한 `pytest tests/ -m "not live" -q` -> `4314 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-22 H107 실측) |
| GitHub Actions CI | 마지막으로 확인한 main 자동화 증적: commit `ee4f9e8`, CI `29881470754` success (`4272 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 확인한 main 자동화 증적: commit `ee4f9e8`, CD `29881470750` success. image digest는 `sha256:801316bafbfa0ce5ced6dd1e0320103e936d08d25e4b4fcdd5cafe0874ff31ae`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
