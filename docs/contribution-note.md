...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4225 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-21 H81 실측) |
| GitHub Actions CI | 마지막으로 문서화한 main 자동화 증적: commit `ce7c965`, CI `29761516772` success (`4224 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 문서화한 main 자동화 증적: commit `ce7c965`, CD `29761516124` success. image digest는 `sha256:c174e70f79c5124a7db0cf86d25ecb81cb4872e45ad1d1cc304ddd3719ee9ca5`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
