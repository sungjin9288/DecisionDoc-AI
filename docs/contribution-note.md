...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | `pytest tests/ -m "not live" -q` -> 2026-07-09 실측 `2804 passed, 2 skipped, 4 deselected` |
| GitHub Actions CI | 최근 기능 변경 커밋 `fb3f89b` 기준 CI `28961688250` success |
| GitHub Actions CD | 최근 기능 변경 커밋 `fb3f89b` 기준 CD `28961688221` success. staging deploy/smoke는 설정 부재로 skip되어 M6 proof로 보지 않는다 |
