...
| Report quality pilot review | 같은 tenant의 ready correction artifact 3~5개를 검증 가능한 ZIP으로 handoff하고, package를 다시 검증해 source-bound local review pack으로 직접 import | `app/routers/report_workflows.py`, `app/services/report_quality_pilot_package.py`, `scripts/create_report_quality_pilot_pack.py`, `scripts/report_quality_pilot_pack_provenance.py`, `tests/test_create_report_quality_pilot_pack.py` |
