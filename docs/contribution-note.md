...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | `pytest tests/ -m "not live" -q` -> `3330 passed, 2 skipped, 4 deselected` (2026-07-16 실측) |
| GitHub Actions CI | 최근 확인한 main 자동화 증적: commit `9694d43`, CI `29467236194` success (`3308 passed, 5 skipped`) |
| GitHub Actions CD | 최근 확인한 main 자동화 증적: commit `9694d43`, CD `29467236207` success. image build/push는 통과했고 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
