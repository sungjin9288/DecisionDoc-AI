...
| Report quality pilot review | 같은 tenant의 ready correction artifact 3~5개를 ordered JSONL로 묶기 전에 artifact·readiness·SHA-256·외부 학습 비승인 경계를 검토하고, server precondition과 append-only audit으로 preview/export를 결속 | `app/routers/report_workflows.py`, `app/services/report_workflow/service_quality_mixin.py`, `app/middleware/audit.py`, `tests/test_report_workflows_api.py` |
