...
| DocumentOps review operations | tenant-scoped trajectory를 summary-first로 탐색하고 선택 상세만 불러온 뒤 사람 점수와 메모를 기록하며, 상세 열람·review 결정은 입력·초안·메모를 제외한 append-only audit으로 추적한다. Governance inventory는 metadata authority와 selected-backend artifact를 read-only로 대조하고, local workbench에서 exact issue count와 재확인 경계를 보여주되 cleanup action을 제공하지 않는다 | `app/routers/document_ops_agent.py`, `app/services/document_ops_service.py`, `app/storage/trajectory/artifact_inventory_mixin.py`, `app/static/index.html`, `app/middleware/audit.py`, `tests/storage/test_trajectory_artifact_authority.py`, `tests/test_document_ops_agent_api.py`, `tests/e2e/test_main_flow.py` |
...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4216 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-20 H76 실측) |
| GitHub Actions CI | 마지막으로 문서화한 main 자동화 증적: commit `95db749`, CI `29742732681` success (`4215 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 문서화한 main 자동화 증적: commit `95db749`, CD `29742732620` success. image digest는 `sha256:455c99cb5b0502537f2014097a459bc837793ce506af9af88d0f77fc0eca1d4a`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
