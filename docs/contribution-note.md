...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4223 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-21 H80 실측) |
| GitHub Actions CI | 마지막으로 문서화한 main 자동화 증적: commit `5d59e15`, CI `29759143687` success (`4223 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 문서화한 main 자동화 증적: commit `5d59e15`, CD `29759143729` success. image digest는 `sha256:eb28ebf4527d851be98a8d25d96b4456f5df8d2e4f75f387067f55f37ce1cf28`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
