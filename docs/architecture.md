...
Ops dashboard의 retention panel은 30/90/180/365일 selector와 icon refresh만 제공한다. Access token 또는 Ops key가 없으면 요청을 만들지 않고 authorization 안내를 유지한다. 응답은 schema version, read/delete boundary, nonnegative integer count와 합계, timestamp를 browser에서 다시 검증한다. Request generation과 tenant가 현재 상태와 일치하는 최신 응답만 화면을 갱신하므로 tenant 전환 또는 연속 조회의 늦은 completion은 버린다. 이 UI는 삭제 endpoint, scheduler 또는 mutation control을 추가하지 않는다.
