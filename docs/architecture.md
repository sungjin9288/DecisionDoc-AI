...
Auth refresh의 monotonic revision은 현재 tab의 session commit/cleanup뿐 아니라 같은 origin의 다른 tab에서 발생한 `dd_access_token`, `dd_refresh_token`, `dd_tenant_id` 변경과 `localStorage.clear()`에도 반응한다. Refresh 시작 뒤 이 revision이 바뀌면 응답 token이 유효하고 refresh-token bytes가 같아도 이전 응답을 `superseded`로 폐기한다. 다른 storage key는 revision을 바꾸지 않는다. Receiving page는 final access-token claims와 stored tenant를 현재 user·tenant·role과 비교해 authorization-context mismatch에서 reload를 한 번만 요청하고, identity와 role이 같은 token rotation은 현재 page-memory 작업을 유지한다. Protected request middleware는 signed tenant/user identity로 현재 `UserStore`를 읽어 persisted role과 `is_active`를 적용한다. Public middleware 예외인 `/events`도 query access token을 같은 resolver에 연결하므로 비활성 사용자의 기존 token은 새 SSE subscription을 만들 수 없다. Store failure는 authorization을 축소하지 않고 `503`으로 닫는다. 이 구조는 매 request의 current-user authority이며 별도 session revocation table, active SSE connection 강제 종료나 cross-device browser coordination은 제공하지 않는다.
