...
- 운영/검증 기능
  ...
  - pytest test suite
  - smoke scripts
- 공공조달 decision package local evidence
  - fixture 기반 package builder / validator
  - versioned CLI stdout contract manifest
  - manifest validation receipt and persisted receipt checker
- 배포 경로
  - Docker Compose
  - AWS SAM/Lambda
...
Local procurement decision package evidence:
...
이 경로는 `docs/samples/procurement_decision_package_local_demo/cli_contract_manifest.json`의 `contract_version`과 local evidence CLI stdout JSON success/failure contract를 확인한다. Provider API, AWS runtime, dataset upload, training execution, model promotion, production service resume, bid submission, legal approval, contractual commitment는 실행하지 않는다.
...
- local evidence contract 문제: stdout JSON success/failure contract를 versioned manifest로 고정하고 persisted receipt checker로 재검증 가능하게 분리
...
- local procurement decision package evidence contract를 README와 portfolio evidence 문서에 연결
