...
- DocumentOps trajectory 상태
  ...
  - Trajectory capture를 선택한 Agent run의 optional `operation_id`는 provider 호출 전에 `document_ops_agent_operations/<sha256>.json` private receipt로 선점한다. Receipt에는 request hash만 저장하고 tenant·operation·owner·상태·UTC timestamp를 strict schema로 검증한다. 성공 상태는 저장된 trajectory-bound result와 hash를 포함하며 exact replay만 반환한다. Changed payload, running/failed retry와 손상 receipt는 provider 호출 전에 각각 `409` 또는 `503`으로 중단한다. API-key 보호 status 조회는 current tenant path만 읽고 operation ID·상태·시각·다음 행동만 반환한다. Owner, request/result hash와 결과 본문은 응답·audit에서 제외하며 missing tenant path는 `404`, 손상 state는 `503`으로 닫는다. Browser는 status를 cache하지 않고 schema·operation identity·state별 필드·read-only·provider-call 비승인을 모두 검증한다. Mismatched·unavailable·running 상태는 captured tenant와 payload를 page memory에만 보존해 두 recovery control이 새 operation 대신 같은 status를 single-flight로 다시 읽는다. Captured POST 직전에는 payload를 제외한 schema·tenant·operation ID만 tab-scoped marker로 기록한다. Reload 뒤에는 strict marker와 current tenant가 일치할 때 status-only read로 새 POST를 막고, backend 실행 비취소와 evidence 확인 경고를 승인한 명시적 release에서만 marker를 제거한다. Extra field, wrong tenant, wrong schema, non-browser UUID marker는 제거하고 logout·invalid session·승인된 tenant 전환도 marker를 폐기한다. Browser storage 접근 실패는 same-page 실행을 막지 않지만 reload guard는 제공하지 못한다. Terminal success의 exact replay는 payload가 남은 같은 page에서만 가능하며 tab 종료·cross-tab 조정, process crash 자동 복구, 서로 다른 ID의 의미상 중복 차단, exactly-once provider 실행, receipt expiry·GC는 제공하지 않는다.
