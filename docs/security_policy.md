...
- 런타임 인증 키
  ...
  - Browser tenant header는 signed access token의 tenant claim과 동기화한다. Login·register·refresh·LDAP login은 claims를 먼저 검증하고 access/refresh token과 tenant ID를 공통 commit helper로 저장한다. Tenant write를 포함한 저장이 실패하면 이전 token과 tenant를 복원하고 current user와 DocumentOps evidence를 바꾸지 않는다. 같은 tab의 동시 401은 하나의 refresh promise에 합류한다. Generic API request는 refresh 성공 뒤에도 자동 replay하지 않고 explicit retry를 요구하며, refresh 시작 뒤 현재 tab 또는 같은 origin의 다른 tab에서 auth credential·tenant storage가 바뀌면 늦은 응답을 `superseded`로 폐기한다. Unrelated storage key는 auth revision을 바꾸지 않는다. 다른 tab의 최종 signed user·tenant가 현재 page와 다르면 앱을 한 번 reload해 page-memory evidence를 새 인증 맥락에서 다시 구성하고, 같은 identity의 token rotation은 reload하지 않는다. Refresh credential이 400/401/403으로 거절된 경우에만 invalid-session cleanup을 수행하고, endpoint·response·storage failure는 기존 credential·tenant·작업 evidence를 보존한다. JWT tenant와 다른 selector 전환은 access preflight에서 거부하고 기존 tenant로 rollback하며, admin JWT도 `TENANT_MISMATCH`를 우회하지 않는다. 승인된 selector 전환도 tenant ID의 browser storage write가 성공한 뒤에만 in-memory context를 바꾼다. Storage 값 자체는 authorization 근거가 아니며 이 browser coordination은 다른 device에 적용되지 않는다.
