...
- 런타임 인증 키
  ...
  - 새 access/refresh pair는 tenant-scoped `auth-session.v1` object의 random session ID를 공유하고 refresh 뒤에도 같은 ID를 유지한다. `/auth/logout`은 signed current session만 owner 조건 아래 CAS로 폐기해 별도 login을 유지한다. `GET /auth/sessions`는 현재 credential version의 본인 활성 session만 `no-store`로 반환한다. `POST /auth/sessions/revoke`는 다른 로그인 하나를 exact owner 조건으로 종료하고, `POST /auth/sessions/revoke-others`는 strict confirmation 뒤 current를 제외한 검증 snapshot을 종료한다. `POST /auth/sessions/revoke-all`은 strict confirmation 뒤 현재 session을 마지막으로 써서 snapshot의 본인 active session 전체를 종료한다. Current selected target은 `409`, foreign/missing target은 같은 `404`이고 legacy sessionless token은 exact logout·inventory·selected/bulk revoke를 사용할 수 없다. Prefix의 모든 direct child는 mutation 전에 strict 검증되어 corrupt·unavailable state를 원본 보존 `503`으로 닫는다. Bulk write는 multi-object transaction이 아니므로 중간 failure의 부분 진행과 snapshot 뒤 새 session을 숨기지 않으며 재조회·재시도를 요구한다. Current-last 순서는 다른 session write 실패 시 현재 browser를 보존하지만 current write 응답 유실까지 원자적으로 조정하지는 않는다. Browser는 session ID를 DOM에 넣지 않고 모든 session action의 늦은 응답을 같은 generation guard로 버리며, 전체 종료 성공 뒤에만 local credential과 미저장 evidence를 정리한다. Protected request, refresh exchange와 `/events`는 persisted session authority를 다시 확인한다. Audit에는 `user.logout`, `user.session_list`, `user.session_revoke`, `user.session_revoke_others`, `user.session_revoke_all` action/result와 bulk count만 남기고 token·target session ID를 복사하지 않는다. User-Agent/IP, admin mass revoke와 automatic expired-session GC는 제공하지 않는다. 일반 browser logout은 revoke 결과와 무관하게 local cleanup을 즉시 수행하고 endpoint failure를 서버 폐기 미확인 상태로 경고한다.
