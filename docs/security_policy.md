...
- 품질 학습 상태
  ...
  - Feedback/eval append와 prompt override, A/B experiment, request pattern의 persistence authority는 conditional CAS이며 process-local lock은 contention 완화 수단이다. Feedback/eval은 기존 JSONL prefix를 보존하고 stable append identity로 불확실한 commit을 조정하며 eval private identity는 public record에 노출하지 않는다. 이 보장은 각 단일 state object 범위이고 A/B와 override를 함께 묶는 distributed transaction은 아니다. Fine-tune과 model registry는 기존 process-local shared lock 경계를 유지하며 실제 provider API, dataset upload, training execution, external job polling과 model promotion은 별도 운영 승인 범위다.
- 공개 공유 상태
