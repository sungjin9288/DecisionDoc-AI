...
- 품질 학습 상태
  ...
  - Prompt override, A/B experiment, request pattern의 persistence authority는 conditional CAS이며 process-local lock은 contention 완화 수단이다. 이 보장은 각 단일 state object 범위이고 A/B와 override를 함께 묶는 distributed transaction은 아니다. Feedback, eval, fine-tune, model registry는 기존 process-local shared lock 경계를 유지하며 실제 provider API, dataset upload, training execution, external job polling과 model promotion은 별도 운영 승인 범위다.
- 공개 공유 상태
