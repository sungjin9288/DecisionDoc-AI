...
- 런타임 인증 키
  ...
  - 위 same-identity·role token rotation 보존 규칙은 `credential_version`도 같을 때만 적용한다. Password change는 bcrypt hash와 version 증가를 같은 user-state CAS에 저장하고 이전 access/refresh token을 모두 `401`로 폐기한다. 변경 요청 browser는 응답의 새 pair를 공통 session helper로 commit하며, 다른 same-origin tab은 version mismatch에서 reload한다. Version 없는 legacy record/token은 persisted version이 0일 때만 호환한다. 이는 password change 기반 all-token epoch revocation이며 per-session 선택 폐기, 이미 열린 SSE 강제 종료, cross-device push revocation은 제공하지 않는다.
