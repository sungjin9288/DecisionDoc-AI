...
- 런타임 인증 키
  ...
  - 열린 `/events` connection은 최대 15초마다 access token의 expiry와 현재 user authority를 다시 검사한다. Invalid credential은 private state가 없는 `auth_revoked` event 뒤 연결을 닫고, user state를 읽을 수 없으면 `auth_unavailable` event 뒤 application event delivery를 중단한다. Browser는 revoked event에서 refresh를 한 번만 수행하고 명시적으로 거절된 refresh credential만 destructive cleanup으로 연결한다. Authority·endpoint·storage failure는 credential과 미저장 evidence를 보존한 채 polling과 reconnect로 복구한다. 이는 bounded active-stream termination이며 per-session 선택 폐기, 즉시 cross-device push revocation이나 15초 이내 종료를 보장하지 않는다.
