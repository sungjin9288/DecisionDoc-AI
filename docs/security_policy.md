...
- 런타임 인증 키
  ...
  - 새 access/refresh pair는 tenant-scoped `auth-session.v1` object의 random session ID를 공유하고 refresh 뒤에도 같은 ID를 유지한다. `/auth/logout`은 signed current session만 owner 조건 아래 CAS로 폐기해 별도 로그인 session을 유지한다. `GET /auth/sessions`는 현재 credential version의 본인 활성 session만 `no-store`로 반환하고 `POST /auth/sessions/revoke`는 다른 로그인 하나만 exact owner 조건 아래 retry-safe revoke한다. Current target은 `409`, foreign/missing target은 동일한 `404`이며 legacy sessionless token은 두 route와 exact logout을 `409`로 거부한다. Prefix inventory는 모든 direct child를 strict 검증해 corrupt·unavailable state를 원본 보존 `503`으로 닫지만 multi-object atomic snapshot은 아니다. Browser는 session ID를 DOM에 넣지 않고 늦은 list/revoke 응답을 generation guard로 버린다. Protected request, refresh exchange와 `/events`는 persisted session의 owner·credential version·expiry·revoked state를 다시 확인한다. Audit에는 `user.logout`, `user.session_list`, `user.session_revoke` action/result만 남기고 access token·refresh token·target session ID를 복사하지 않는다. User-Agent/IP inventory, 전체 기기 또는 admin mass revoke, automatic expired-session GC는 제공하지 않는다. Browser logout은 revoke 결과와 무관하게 local credential과 미저장 evidence를 즉시 정리하고, endpoint 실패 시 서버 폐기 미확인 상태를 경고한다.
  - 열린 `/events` connection은 최대 15초마다 access token의 expiry와 현재 user/session authority를 다시 검사한다. Invalid credential은 private state가 없는 `auth_revoked` event 뒤 연결을 닫고, authority를 읽을 수 없으면 `auth_unavailable` event 뒤 application event delivery를 중단한다. Browser는 revoked event에서 refresh를 한 번만 수행하고 명시적으로 거절된 refresh credential만 destructive cleanup으로 연결한다. Authority·endpoint·storage failure는 credential과 미저장 evidence를 보존한 채 polling과 reconnect로 복구한다. 이는 exact-session 폐기를 반영하는 bounded active-stream termination이며 즉시 cross-device push revocation이나 15초 이내 종료를 보장하지 않는다. 전체 기기 로그아웃·관리자 일괄 폐기·expired-session GC도 현재 범위 밖이다.
