...
- The Ops dashboard exposes retention governance as a bounded operational read. One strict inspection compares 30/90/180/365-day policy aggregates, while the browser validates the versioned contract, policy monotonicity, `snapshot_atomic=false`, and the false deletion boundary before rendering the latest current-tenant response. Selector changes remain local to that verified comparison and only refresh starts a new inspection. No delete or scheduler action is available.
