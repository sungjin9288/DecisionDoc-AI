...
- Project detail recalculates each review-bound document against the current tenant's packet record and procurement update time. Current, stale, missing, and invalid evidence remain visible; stale or unverifiable documents require an explicit confirmation before approval or sharing. Project-linked shares preserve that share-time baseline, while every public view resolves the current tenant-scoped source again and records any post-share drift in audit history.
