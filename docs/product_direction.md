...
- DocumentOps now presents training governance, selected-backend artifact integrity, and reviewer sign-off through one read-only review overview. The service preserves the three source reports, prioritizes boundary drift before artifact, governance, and sign-off issues, and gives the reviewer one next action. The combined view is explicitly non-atomic, requires recheck, and grants no dataset upload, provider, training, job, or model-promotion authority.
