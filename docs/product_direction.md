...
- The Ops dashboard exposes that retention preview as a bounded operational read: 30/90/180/365-day policy selection, one refresh control, no request before an access credential exists, and no delete or scheduler action. The browser independently validates the versioned read-only contract and aggregate consistency, then accepts only the latest response for the current tenant. This makes cleanup scope inspectable without widening deletion authority.
