...
| Browser auth session failure atomicity | 검증 완료 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` | Refresh response 뒤 tenant write 실패를 실제 Chromium으로 주입해 기존 access/refresh token, tenant, current user와 DocumentOps draft/recovery/marker가 복원되는지 확인한다. 상위 401 recovery에서도 storage failure가 invalid session으로 축소되지 않고 `AUTH_SESSION_STORAGE_FAILED`로 끝나는지, refresh 성공은 원 요청을 한 번 재시도하고 refresh credential 401만 기존 cleanup을 수행하는지 검증한다 | Browser storage는 signed auth나 backend authority를 대체하지 않는다. 일시 장애와 storage 실패는 기존 session evidence를 보존하며 외부 identity provider, provider API와 runtime은 호출하지 않는다 |
...
결과: `4273 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-22 H100 실측, 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정).
