...
| DocumentOps trajectory 상태 무결성 | 검증 완료 | `app/storage/trajectory/core_mixin.py`, `app/storage/trajectory/state_mixin.py`, `app/main.py`, `tests/storage/test_trajectory_store.py`, `tests/storage/test_trajectory_store_integrity.py`, `tests/test_document_ops_agent_api.py` | 선택된 local/fake-S3 backend의 단일 JSONL conditional create/CAS, process lock 없는 20-way append/review, expected review version, 32회 conflict cap, 최근 64개 private receipt, append/review lost-response·successor reconciliation, legacy retry·손상·중복·public projection 회귀 | CAS는 trajectory/review JSONL 한 object 범위. Metadata·SFT export·freeze·training approval/request/audit artifact는 local filesystem handoff이며 실제 AWS/provider/training runtime은 범위 밖 |
...
결과: `4200 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-20 H73 실측, 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정).
