...
- quality-first provider preflight: `python3 scripts/check_prod_env.py --env-file .env.prod --expected-origin https://<site-domain> --provider-profile quality-first`
...
- `python3 scripts/post_deploy_check.py --env-file .env.prod` — health(+provider route visibility), compose 상태, nginx 설정, deployed smoke preflight, deployed smoke, Report Workflow ERP smoke를 묶은 post-deploy check
