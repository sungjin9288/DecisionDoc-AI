...
- `scripts/deploy.sh` 는 `.env.prod` 를 기준으로 `check_prod_env.py` preflight, compose rollout, `post_deploy_check.py`(health + nginx + smoke preflight + deployed smoke + Report Workflow ERP smoke) 및 `./reports/post-deploy/` history 저장까지 수행합니다.
