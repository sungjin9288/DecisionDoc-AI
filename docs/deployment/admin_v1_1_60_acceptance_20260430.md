# DecisionDoc AI Admin v1.1.60 Acceptance Record — 2026-04-30

이 문서는 `admin.decisiondoc.kr` 운영 환경에 `v1.1.60` production release가 정상 반영되었음을 고정하기 위한 acceptance record입니다.
...
| release tag | `v1.1.60` |
| release commit | `364aa07bed1fd08fc167587d9e46978471654f7a` |
| Docker image | `ghcr.io/sungjin9288/decisiondoc-ai:1.1.60` |
| Docker digest | `sha256:3a730798c5109e3ff81efdfab365516e787129f921a1c2fead231e8f8f18fda2` |
| GitHub tag | `https://github.com/sungjin9288/DecisionDoc-AI/releases/tag/v1.1.60` |
| GitHub Actions CD run | `https://github.com/sungjin9288/DecisionDoc-AI/actions/runs/25162935841` |
| CD result | `success` |
| post-deploy timestamped report | `reports/post-deploy/post-deploy-20260430T113125Z.json` |
| acceptance recorded at | `2026-04-30 20:33 KST` |
...
| service | result |
|---------|--------|
| app | `ghcr.io/sungjin9288/decisiondoc-ai:1.1.60`, healthy |
...
| 항목 | 결과 |
|------|------|
| deploy started | `2026-04-30T11:30:45Z` |
| deploy completed | `2026-04-30T11:32:58Z` |
| production job duration | `2m13s` |
...
| `GET /export/pptx` | `200`, `43539` bytes |
| `GET /export/snapshot` | `200`, `decisiondoc_report_workflow_snapshot.v1` |

## 4. Release Scope

`v1.1.60`는 `v1.1.59` production deploy acceptance와 company handoff evidence를 tag source에 포함시키기 위한 follow-up release입니다.

주요 변경:

- `docs/deployment/admin_v1_1_59_acceptance_20260430.md`를 추가해 직전 production deploy acceptance를 repository에 고정
- `docs/deployment/admin_v1_handoff.md`의 최신 production acceptance 기준을 v1.1.59로 갱신
- company handoff readiness gate와 bundle 생성 기준을 v1.1.59 acceptance evidence로 갱신
- company handoff regression tests를 최신 release evidence 기준으로 보강
- runtime API feature 동작은 v1.1.59 배포 기준과 동일하며, 이 release는 acceptance/handoff evidence를 태그에 포함하는 운영 증적 release로 정의
...
- 이 record는 `v1.1.60` production deploy acceptance이며, customer-specific isolated environment rollout은 포함하지 않습니다.
