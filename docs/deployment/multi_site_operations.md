# Multi-site 운영 가이드 (사무실 + 회사 A/B)

이 문서는 사무실, 회사 A, 회사 B 총 3개 장소에서 서로 완전히 분리된 환경으로 DecisionDoc AI를 운영하는 절차를 정리합니다.
권장 방향은 각 장소별로 독립된 배포 스택, 독립된 키, 독립된 데이터 저장소를 유지하는 것입니다.

---

## 운영 원칙 (요약)

1. 완전 분리: 코드/환경/데이터/키를 장소별로 분리
2. 독립 운영: 장애나 키 유출이 다른 장소에 영향 주지 않게 설계
3. 일관 버전: 동일 버전 릴리즈를 세 장소에 동기화
...
각 장소에 아래 구성을 **독립적으로** 배포합니다.

- 배포 방식: Docker Compose (on-prem 서버/VM)
- 저장소: `DECISIONDOC_STORAGE=local`
- 데이터 볼륨: 장소별로 다른 `decisiondoc_data` 볼륨
- API 키: 장소별로 서로 다른 `DECISIONDOC_API_KEYS` / `DECISIONDOC_OPS_KEY`

추가로, 내부 규정이 허용되면 장소별 S3 버킷을 사용하는 것도 가능합니다.
이 경우에도 버킷/접근 권한은 반드시 장소별로 분리합니다.

---

## 장소별 환경 파일 템플릿

각 장소마다 `.env.prod` 파일을 분리합니다.
아래 템플릿을 기반으로 세 장소에 각각 값을 채웁니다.
...
4. `OPENAI_API_KEY`
   - OpenAI API 키
   - 회사별 billing/account 분리가 필요하면 장소별로 별도 키 사용 권장
