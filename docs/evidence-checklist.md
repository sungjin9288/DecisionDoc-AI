...
| Non-live 전체 pytest gate | 완료 | `pytest tests/ -m "not live" -q` -> `3286 passed, 2 skipped, 4 deselected` (2026-07-16 실측) |
| GitHub Actions CI | 완료 | 최근 확인한 main 자동화 증적: commit `d8e1553`, CI `29463836462` success (`3250 passed, 5 skipped`) |
| GitHub Actions CD | 완료 | 최근 확인한 main 자동화 증적: commit `d8e1553`, CD `29463836573` success. image build/push는 통과했고 staging deploy/smoke와 production deploy는 skip되어 배포 proof에서 제외 |
