...
| 축 | 현재 | 완성 기준 |
|----|------|-----------|
| **기능 검증** | non-live test suite 통과 (`pytest tests/ -m "not live" -q` → 3,120 passed, 2 skipped, 4 deselected, 2026-07-16) | 외부 의존 경로(live LLM, G2B 실데이터)도 최소 1회 실증 + 증적 |
...
```bash
# 재현: 테스트 베이스라인
pytest tests/ -m "not live" -q     # 2026-07-16 실측: 3120 passed, 2 skipped, 4 deselected
...
| G6 | **배포 접근성 미검증** — 최근 확인한 GitHub Actions CD는 성공했지만 staging deploy/smoke와 production deploy는 skip되어 운영 URL 동작 보장 없음 (README §Scope 명시) | GitHub Actions CD `29449254755` success, image build/push passed, deploy/smoke skipped | MED | 미착수 |
