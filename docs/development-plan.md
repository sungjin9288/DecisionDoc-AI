...
| 축 | 현재 | 완성 기준 |
|----|------|-----------|
| **기능 검증** | mock/local 경로에서 전 기능 테스트 통과 (`pytest -q tests/ -m "not live" --tb=short` → 2,967 passed, 1 skipped, 4 deselected, 2026-07-14) | 외부 의존 경로(live LLM, G2B 실데이터)도 최소 1회 실증 + 증적 |
...
```bash
# 재현: 테스트 베이스라인
pytest tests/ -m "not live" -q     # 2026-07-14 실측: 2967 passed, 1 skipped, 4 deselected
