...
```bash
# 재현: 테스트 베이스라인
pytest tests/ -m "not live" -q     # 2026-07-16 실측: 3127 passed, 2 skipped, 4 deselected
...
| G6 | **배포 접근성 미검증** — 최근 확인한 GitHub Actions CD는 성공했지만 staging deploy/smoke와 production deploy는 skip되어 운영 URL 동작 보장 없음 (README §Scope 명시) | GitHub Actions CD `29451108511` success, image build/push passed, deploy/smoke skipped | MED | 미착수 |
