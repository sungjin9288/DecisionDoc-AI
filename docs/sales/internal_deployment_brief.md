...
두 환경은 운영 기준과 데이터 경계를 분리합니다.

따라서 권장 구조는 **한 개의 루트 도메인 + 환경별 서브도메인 + 환경별 독립 배포**입니다.
...
- 한 환경의 장애가 다른 환경에 영향을 주지 않게 하기 위해
- 운영자 공용 데이터와 고객 전용 데이터가 섞이지 않게 하기 위해
- 키 유출이나 설정 실수가 전체 환경으로 번지지 않게 하기 위해
- 고객별 유지보수 일정을 독립적으로 가져가기 위해

## 4. 운영 방식

각 환경은 동일한 코드 버전을 사용하되, 아래 항목은 반드시 분리합니다.
...
필요 시 OpenAI API 키도 환경별로 분리할 수 있습니다.
...
필요하면 `admin`과 `dawool`처럼 운영 목적에 맞게 환경을 분리해 데이터 통제와 운영 안정성을 동시에 확보할 수 있습니다.
