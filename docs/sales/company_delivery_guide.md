...
- `company-handoff-<timestamp>.zip`
- `company-handoff-<timestamp>.zip.sha256`
...
전달자가 보내는 단일 파일은 `company-handoff-<timestamp>.zip`이며, 수신자는 `.zip.sha256` 값을 이용해 압축 파일 자체의 전달 중 변조 여부를 먼저 확인한 뒤 압축을 해제합니다.
...
- 키 전달 경로를 별도로 정했다.
- `python3 scripts/prepare_company_handoff.py` 가 `PASS`를 반환했다.
...
- `python3 scripts/archive_company_handoff_bundle.py output/company-handoff/company-handoff-<timestamp>` 가 `PASS`를 반환했다.
- `output/company-handoff/company-handoff-<timestamp>.zip.sha256` 를 전달 checksum 증적으로 보관했다.
