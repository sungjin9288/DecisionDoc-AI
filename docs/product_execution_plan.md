...
- Same-origin auth storage changes now advance the receiving tab's session revision for access token, refresh token, tenant ID, and full local-storage clear events. An older in-flight refresh is discarded even when another tab reuses the same refresh-token bytes; unrelated storage keys are ignored. The receiving page compares the final signed user and tenant with its current context. A mismatch requests one reload so current user and page-memory evidence are rebuilt under the new session, while a same-identity token rotation leaves current work in place. This is browser context reconciliation, not backend authorization or cross-device session coordination.
