...
- Password change also advances a persisted credential version in the same CAS as the password hash. Older access and refresh tokens fail on their next protected request or refresh exchange, the profile flow commits the returned replacement pair, and another same-origin tab reloads when the version changes. Open SSE subscriptions revalidate expiry and persisted user authority every 15 seconds, emit only fixed `auth_revoked` or `auth_unavailable` control data, and unsubscribe before any further application event. The browser refreshes once for revocation, clears evidence only when the refresh credential is rejected, preserves the current session on temporary failures, and ignores callbacks from superseded EventSource instances. Quiet same-identity rotation therefore applies only while credential version is unchanged; per-session revocation, immediate cross-device push, and a shorter stream-termination SLA remain outside this plan.
