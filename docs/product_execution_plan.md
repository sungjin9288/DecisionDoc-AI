...
- A receiver can now select a pilot review package in the Report Workflow UI and verify it without a CLI or server-side package persistence. The browser computes the ZIP SHA256 before upload, the API reuses the independent verifier in memory, and the response exposes exact membership, entry hashes, receipt binding, tenant ownership, artifact count, and false external-action boundaries. Tampered, oversized, and cross-tenant packages are rejected, while success and denial evidence remain visible in the existing audit console.
