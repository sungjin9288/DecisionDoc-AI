...
- DocumentOps controls that can create an export, freeze, dry-run approval, execution request, audit artifact, or provider-backed Agent request run as browser single-flight actions. The initiating button is disabled before the async action starts and restored after success or failure, while read-only refresh controls remain independent. The browser also assigns one UUID operation identity to each freeze, dry-run approval, execution request, and audit export. The backend binds that identity to the canonical request payload in private metadata, returns the original verified artifact for an exact replay, and rejects a changed payload with `409`. This backend contract applies to the four governance records only; content-fingerprinted SFT export reuse and provider-backed Agent execution keep their existing boundaries. Concurrent workers can still leave an unreferenced immutable artifact before one metadata CAS entry wins, so this is not a distributed transaction or zero-orphan guarantee.
