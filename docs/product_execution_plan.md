...
- Imported report quality pilot packs now include a local browser review workspace bound to the source manifest and current draft SHA256 values. It presents before/after planning and slide evidence, visible claims, workflow/final references, validation blockers, and required actions before capturing human decisions, scores, scans, rationale, and structured change requests into a downloaded JSON draft. The apply CLI validates the external draft without moving it, preserves the exact bytes under a SHA-derived pack-local name, applies the all-or-nothing decision batch, writes a matching receipt, and refreshes the worksheet and human review manifest against the resulting draft hashes. Ready sync then requires that current manifest and a validated `require_ready` accepted decision receipt, rechecks both before writing, and returns their hashes with the JSONL hash. Dry-run, invalid batches, pending local review, and unsafe evidence targets leave downstream output unchanged, and no path authorizes external training actions.
- A ready sync can now be sealed as a deterministic reviewed-pilot handoff ZIP. The package binds the exact JSONL, current human-review manifest, accepted decision receipt and decision file, final draft bytes, and available source provenance sidecars to an embedded manifest. `HANDOFF_SUMMARY.md` gives a non-engineering reviewer the reviewer, score, decision state, evidence hashes, and authorization boundary without reading raw JSON. The standalone verifier regenerates that summary and rechecks membership, hashes, artifact readiness, review transitions, source binding, and the no-training boundary without the original pack. Its optional summary output is written atomically only after the archive passes verification and returns the exact summary SHA256 for handoff evidence. Package and summary publication use a write-once local file contract so a concurrent destination cannot replace earlier evidence.
