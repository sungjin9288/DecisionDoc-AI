...
| `python3 scripts/check_report_quality_pilot_handoff_demo_receipt.py /tmp/decisiondoc-report-quality-pilot-handoff-demo.json --json` | full-chain demo receipt의 schema, artifact identity, SHA-256, stage order, simulated/no-training/secret boundary를 read-only로 재검증. 삭제된 temporary artifact 자체를 다시 검증하지는 않음 | 재현 가능 |
