...
# 2026-07-14 실측: 2975 passed, 2 skipped, 4 deselected
...
- 현재 상태:
  ...
  - 2026-07-14 pilot review pack 생성이 source-bound `HUMAN_REVIEW_WORKSHEET.md`와 `human_review_manifest.json`까지 한 번에 준비하도록 연결했다. 검수자는 import 직후 review table과 artifact별 required action을 확인할 수 있고, draft 수정 뒤에는 같은 generator를 refresh 용도로 재실행한다. 사람이 작성하는 decision template은 자동 생성하지 않아 기존 입력을 덮어쓰지 않는다.
