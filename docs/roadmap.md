...
# 2026-07-14 실측: 2980 passed, 2 skipped, 4 deselected
...
- 현재 상태:
  ...
  - 2026-07-14 non-engineering reviewer가 JSON을 직접 편집하지 않아도 되도록 pack-local `HUMAN_REVIEW_WORKSPACE.html`을 자동 생성한다. 화면은 결정, 점수, scan, 차원별 근거, 구조화된 보완 요청을 받아 source/draft binding과 `training_authorized=false`를 보존한 browser draft만 내려주며 원본 pack은 수정하지 않는다. Apply CLI가 stale binding과 전체 batch를 다시 검증하는 기존 경계는 유지한다.
