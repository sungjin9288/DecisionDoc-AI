...
# 2026-07-14 실측: 2961 passed, 1 skipped, 4 deselected
...
- 현재 상태:
  ...
  - 2026-07-14 DocumentOps trajectory 목록의 `total`을 제한된 응답 길이가 아닌 tenant·filter 기준 실제 건수로 바로잡고, 최신 항목 기준 `offset`/`limit`과 `returned`/`has_more` 계약을 추가했다. Browser workbench는 10건 단위 이력 이동, 새 실행 시 첫 페이지 복귀, 리뷰 시 현재 페이지 유지를 지원하며 mock/local desktop·390px mobile에서 최신순·overflow·console error를 검증한다.
