...
FastAPI (app/main.py — create_app(), 모듈 레벨 side-effect 없음)
  ...
  ├─ Routers (23 top-level files, 라우트 280): generate / approvals / projects / knowledge
...
FastAPI 라우트는 **280개**입니다.

```bash
python3 scripts/count_readme_metrics.py --field route_decorators  # → 280
```
...
테스트 함수는 **3,625개**, **258개 파일**입니다 (AST source definition 기준 카운트). 자동생성 phase 영수증 검증 테스트(제품 기능과 무관)는 2026-07-02 정리에서 제거해 수치에서 제외했습니다.

```bash
python3 scripts/count_readme_metrics.py --field test_functions  # → 3625
...
<sub>이 README의 모든 정량 수치(라우트 280 · 테스트 3,625 · env 키 94 등)는 소스 코드에서 직접 카운트했으며, 재현 커맨드를 함께 표기했습니다. 측정 근거가 없는 비용 절감률·자동화율·정확도 수치는 사용하지 않습니다.</sub>
