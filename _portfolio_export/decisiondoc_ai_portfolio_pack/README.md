...
현재 non-live test suite는 통과했습니다 (`pytest tests/ -m "not live" -q` → 4,265 passed, 2 skipped, 4 deselected, 1 warning, 2026-07-21 H93 실측). "완성"을 막는 갭과 마일스톤은 [docs/development-plan.md](./docs/development-plan.md)에 정의돼 있습니다.
...
- `capture_trajectory=true`인 DocumentOps Agent run은 optional `operation_id`를 사용할 수 있습니다. Backend는 provider 호출 전에 tenant별 private operation receipt를 conditional create하고 request SHA-256에 결속합니다. Exact replay는 저장된 성공 결과를 반환해 provider 호출과 usage event를 반복하지 않으며 changed payload는 `409`, 실행 중이거나 실패로 끝난 동일 operation은 증거 확인 전 자동 재호출하지 않고 `409`, 손상된 receipt는 provider 호출 전에 `503`으로 중단합니다. Conditional write 응답 유실은 exact receipt read-back으로 조정합니다. Tenant-scoped status API는 operation ID·상태·시각·다음 행동만 반환하고 owner, request/result hash와 결과 본문은 숨기며 조회 이력을 redacted audit에 남깁니다. Browser는 최초 응답을 잃은 뒤 이 status가 `succeeded`와 replay 가능, provider 호출 비승인을 함께 증명할 때만 같은 payload를 한 번 exact replay합니다. `running`·`failed`·missing·corrupt 상태에서는 POST를 반복하지 않습니다. Capture하지 않는 요청은 결과를 새로 저장하지 않도록 이 계약에서 제외합니다. 이 보장은 operation identity를 보존한 재전달 범위이며 provider 성공과 receipt 확정 사이의 process crash 자동 복구, 서로 다른 operation ID의 의미상 중복 차단, exactly-once 실행, receipt expiry·GC는 제공하지 않습니다.
