...
- The browser workspace now closes the next local handoff step by rendering pack-bound dry-run and apply commands itself. Python shell-quoting preserves paths with spaces or apostrophes, the command preview changes immediately with reviewer decisions, and `--require-ready` appears only when every artifact is accepted. Pending, changes-requested, and rejected batches keep the non-ready apply path, while clipboard-restricted local pages use a selection-copy fallback without external requests.
