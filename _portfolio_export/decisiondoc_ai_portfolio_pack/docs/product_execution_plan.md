...
- Imported report quality pilot packs now include a local browser review workspace bound to the source manifest and current draft SHA256 values. It captures human decisions, scores, scans, rationale, and structured change requests into a downloaded JSON draft; it never mutates the pack directly or authorizes external training actions, and the existing apply CLI still rejects stale or partially invalid batches.
