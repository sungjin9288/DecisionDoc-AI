...
- Report quality pilot export now has a reviewer preflight before download. The tenant-scoped API resolves the selected 3-5 ready artifacts once, preserves order, and returns the exact JSONL SHA256 plus explicit false dataset-upload, provider-fine-tune, training, and model-promotion boundaries. The browser downloads only when the export response hash still matches that preview.
