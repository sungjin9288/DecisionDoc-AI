...
- The Ops dashboard now presents the auth-session retention preview as a read-only aggregate panel with 30/90/180/365-day selection and one refresh action. It does not issue an unauthenticated probe, validates `auth-session-retention-preview.v1` plus the false deletion boundary and aggregate consistency, and discards stale request or tenant completions. No deletion, scheduler, provider call, or external runtime action is exposed.
