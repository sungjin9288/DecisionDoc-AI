...
- Password change also advances a persisted credential version in the same CAS as the password hash. Older access and refresh tokens fail on their next protected request or refresh exchange, the profile flow commits the returned replacement pair, and another same-origin tab reloads when the version changes. Register, login, invite, LDAP, SAML, GCloud, and password-change pairs additionally share a random tenant-scoped persisted session identity; refresh preserves it. `POST /auth/logout` revokes only the signed current session through selected-backend CAS, so a separate login remains active while copied same-session credentials fail on their next protected request, refresh, or open SSE recheck. Self-service inventory validates every direct object in the tenant session prefix, then returns only the current user's active current-version sessions with `no-store`; the sequential multi-object observation is not atomic. Selected revoke is strict, owner-bound, retry-safe for an already-revoked target, rejects current-session cleanup bypass, and gives foreign and missing targets the same response. The profile keeps session IDs out of the DOM, displays start/expiry only, and uses request/token/modal plus revoke-generation guards so late work cannot overwrite or unlock a newer action. Open SSE subscriptions revalidate expiry and persisted user/session authority every 15 seconds, emit only fixed `auth_revoked` or `auth_unavailable` control data, and unsubscribe before any further application event. Browser logout starts server revocation but completes local credential and page-memory cleanup immediately; an unavailable endpoint leaves an explicit unconfirmed-server-revocation warning. Corrupt session state fails closed and audit omits tokens and target session IDs. Quiet same-identity rotation therefore applies only while credential version and persisted session authority remain current. Legacy sessionless exact logout/inventory/selected revoke, immediate cross-device push, User-Agent/IP inventory, all-device or administrator mass revoke, expired-session GC, and a shorter stream-termination SLA remain outside this plan.
