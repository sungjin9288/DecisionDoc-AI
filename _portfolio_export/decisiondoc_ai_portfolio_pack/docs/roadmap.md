...
# 2026-07-14 실측: 2955 passed, 2 skipped, 4 deselected
...
- 현재 상태:
  ...
  - 2026-07-14 Admin Ops audit 화면에 report-quality pilot preview/export 필터와 receipt 대조용 request ID·전체 SHA-256·artifact count·검증 상태를 연결했다. Audit 문자열은 HTML escape하고 모바일에서는 table 내부 스크롤로 화면 overflow를 차단한다. CSV export는 화면의 action/result 필터를 이어받아 전체 detail과 pilot 식별자를 보존하며, 당일 상한과 1,000건 query cap으로 증빙이 빠지지 않도록 별도 full export 경로를 사용한다. Spreadsheet formula로 해석될 수 있는 문자열도 안전한 text cell로 기록한다.
