...
The reviewed directory can be wrapped with `scripts/manage_procurement_decision_review_packet.py` into a deterministic ZIP containing the 12 package artifacts plus embedded `packet_manifest.json`. The packet remains `review_ready` with `operational_approval: false`; independent verification checks exact membership and order, path boundaries, SHA256 and size fingerprints, excluded actions, and the full package semantics before reporting success.
`scripts/manage_procurement_review_receipt.py` creates a companion `procurement_review_receipt.json` outside that ZIP, binds it to the packet's `packet_sha256`, and records one requested-reviewer decision through `pending` and `completed` `review_status` values. Validation rejects a different packet, wrong reviewer, second decision, malformed UTC time, empty rationale, or any `operational_approval` value other than false.
`scripts/manage_procurement_reviewed_package.py` closes the local export loop after receipt completion. It creates `procurement-reviewed-package.zip` with the original packet, completed receipt, and `reviewed_package_manifest.json`, then verifies their exact source SHA256/size values and semantics. The outer status `review_completed` records the reviewer outcome but does not authorize any operational action.
