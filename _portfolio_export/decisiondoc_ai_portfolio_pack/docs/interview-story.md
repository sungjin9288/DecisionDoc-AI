...
| report workflow state의 동시성과 손상은 어떻게 다루나요? | tenant별 backend relative path를 lock identity로 사용해 서로 다른 virtual base도 같은 S3 object면 한 process에서 직렬화한다. Blank·invalid UTF-8·backend failure는 domain 400이 아니라 원본 보존 500으로 처리한다. | `app/storage/report_workflow/`, `tests/test_report_workflow_store_integrity.py` |
