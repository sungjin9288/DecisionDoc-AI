...
Record·packet·reviewed-package는 앱이 선택한 local/S3 `StateBackend`와 tenant/project/packet SHA-256 scope를 공유한다. v2 project record는 assignment 시점의 stable reviewer user ID와 username, completion 시점의 session-bound principal attestation, attestation SHA-256을 함께 검증한다. Packet과 reviewed-package는 persisted size·SHA-256뿐 아니라 receipt·embedded receipt·manifest·attestation의 semantic binding도 다시 대조한다. v1 record와 three-entry package는 읽기 호환 경로로만 유지한다.

Prepare는 packet을 `write-if-absent`로 확정하고 exact orphan packet만 재사용한 뒤 record를 create-if-absent로 만든다. Completion은 원본 packet을 다시 검증하고 reviewed-package를 content-addressed immutable object로 만든 뒤 pending record를 CAS로 completed record에 전환한다. 확인된 CAS loser package는 삭제하고, write 결과가 불확실한 exact artifact는 persisted record 재조정 또는 동일 payload 재시도에 사용하도록 보존한다. 완료 응답을 잃은 assignee가 같은 decision/rationale를 다시 보내면 record를 재변경하지 않고 persisted package를 trusted tenant/project/user scope로 검증해 반환한다. Local backend는 conditional file lock과 atomic replace, S3 backend는 `If-None-Match`와 ETag `If-Match`를 사용한다. Commit 응답이 불확실하면 persisted bytes를 다시 읽어 성공 여부를 조정하며, record가 확정되지 않은 immutable artifact는 권위 상태로 사용하지 않는다. Persisted 오류는 review domain `ValueError`와 분리되어 API에서 `500 INTERNAL_ERROR`로 전달된다. 여러 object를 하나의 원자적 commit으로 묶는 distributed transaction은 보장하지 않는다.
...
DocumentOps Agent의 실행 provenance는 strict `document_ops_skill_binding_v1`이다. Binding은 selected skill name/version, risk level, exact Markdown source-byte SHA-256, 전체 public catalog fingerprint와 `code_execution_authorized=false`, `external_runtime_authorized=false`만 포함하고 instruction body와 local source path를 포함하지 않는다. Service는 이 binding을 provider와 captured operation claim보다 먼저 resolve해 canonical request identity에 넣고 Agent에 expected binding으로 전달한다. Agent는 task/preferred-skill mapping과 현재 catalog를 다시 resolve해 expected binding과 exact 일치할 때만 provider property를 읽는다. Unknown skill, unsupported preferred mapping, binding drift는 provider 접근 전에 fail closed한다. 성공 시 하나의 verified binding이 prompt, `DocumentOpsResult`, trajectory detail/summary와 stored replay result에 이어지고 기존 `skill_name`/`skill_version`은 호환 필드로 유지된다. Browser도 response binding을 untrusted input으로 재검증한다: exact eight-field key set, name/version/hash/risk formats, literal false authority와 legacy field agreement가 모두 맞을 때만 normal result flow에 진입한다. 검증 실패는 draft·plan·evidence·QA·trajectory refresh·marker cleanup·success notification 전에 고정 provenance error로 닫고, 성공 화면은 별도 persistence 없이 allowlisted skill/version, risk, two full hashes, replay state와 not-authorized execution/runtime 상태만 표시한다. Authenticated Agent run audit은 task type, canonical binding SHA-256, replay 여부와 두 false authority만 DocumentOps detail로 기록하며 skill instruction, source path, request/source body, credential과 provider response는 복사하지 않는다.

`document_comparison_review`는 binding resolve보다 먼저 `baseline_document_text`와 `candidate_document_text`를 non-blank string·20,000-character limit으로 검증하고 선택 `comparison_criteria`는 생략 시 `[]`, 최대 8개·각 120자로 정규화한다. 서버는 exact UTF-8 input bytes에서 `document_ops_comparison_context_v1`의 두 SHA-256, equality, normalized criteria와 `raw_content_included=false`를 계산한다. Context는 result, redacted trajectory와 exact captured-operation replay에만 포함하고 raw document text는 trajectory input·audit·provenance metadata에 복사하지 않는다. Browser도 exact key set, literal false raw-content flag와 context hash를 현재 input bytes에 다시 대조한 뒤에만 결과와 success effect를 표시한다. Comparison QA는 기존 forbidden-term/evidence/governance gate를 유지하고 필수 비교 섹션을 별도 deterministic gate로 추가한다. 이 경계는 textual/byte-level 비교일 뿐 의미·정책·법적 효력 또는 운영 영향의 자동 판정이 아니며, GitHub-derived skill design reference는 [external-repository analysis](./specs/external_repo_integration/ANALYSIS_20260812.md)의 clean-room documentation에 한정되고 remote loading이나 runtime authority를 추가하지 않는다.

`POST /api/agent/document-ops/comparison-documents/extract`는 기존 maintenance/API-key gate 뒤 multipart `UploadFile` 하나를 받는다. 전용 intake service는 `app.services.attachment_service.extract_text`만 호출하며 provider factory, OCR/AI fallback, external runtime, tenant/storage access를 갖지 않는다. Safe basename과 exact upload bytes의 SHA-256, returned UTF-8 text의 SHA-256, Python character count, parser-cap 가능성, `extraction_mode=deterministic_local_existing_parser`, `provider_called=false`, `persisted=false`만 strict `document_ops_comparison_document_v1`으로 반환하고 `Cache-Control: no-store`를 고정한다. Upload bytes, extracted text, path-bearing original filename은 local/S3 state, trajectory, audit detail, response-provenance metadata 또는 application log에 쓰지 않는다; endpoint audit은 route/status/duration 같은 aggregate metadata만 남긴다. Browser는 두 slot을 별도 generation으로 관리하고 file bytes·strict key set·hash·size·count·false flags를 모두 확인한 뒤에만 direct-editable textarea를 채운다. 직접 편집과 file/task/tenant/auth context 변경은 page-memory file provenance를 제거하며, valid text만 기존 comparison run의 raw-text redaction과 returned comparison-context hash check로 이어진다.

`POST /api/agent/document-ops/comparison-documents/change-set`는 같은 maintenance/API-key gate 뒤 strict raw-text request를 받지만 provider, storage, trajectory 또는 operation receipt에 접근하지 않는다. Pure comparison service의 단 하나인 `difflib.SequenceMatcher(..., autojunk=False)` opcode stream이 source `splitlines()` count, exact UTF-8 SHA-256, 동일 여부, normalized criteria, 전체 equal/add/remove/replace aggregate와 zero-based half-open hunk를 함께 만든다. `replaced_line_count`는 여러 replace opcode의 baseline replacement 합과 candidate replacement 합 중 큰 값이다. Response는 모든 `baseline_lines`와 `candidate_lines` 합계 200의 global budget에서 contiguous zero-prefix만 노출하고, 처음 초과하는 hunk를 range와 array가 일치하도록 clip한 뒤 중단한다. Full aggregate와 `total_hunk_count`는 보존하고 `hunks_truncated`가 partial coverage를 명시하며 per-hunk truncation flag는 없다. Canonical UTF-8 JSON bytes는 exact body SHA-256인 `X-DecisionDoc-Document-Comparison-SHA256`, `no-store`, `nosniff`, safe attachment filename과 함께 반환된다. Authority의 approval, code execution, external effect/runtime, persistence, provider call, semantic 필드는 모두 false다. Raw text·source line·hash·response bytes·path는 log, trajectory, audit detail, storage, operation receipt 또는 tracked portfolio artifact에 기록하지 않는다.

Browser는 response body를 `arrayBuffer()`로 받아 header SHA-256을 exact bytes와 먼저 대조하고 fatal UTF-8 decode, canonical JSON, exact schema, source hash와 `splitlines()` array, zero-prefix range, global budget, full aggregate, false authority, current task/source/criteria/tenant/auth revision을 모두 통과한 경우에만 verified panel을 렌더링하고 Agent POST를 시작한다. Download는 page memory의 바로 그 verified bytes만 사용한다. Edit, file intake, task·tenant·auth·criteria 변경, 실패, newer request와 late completion은 evidence를 폐기하고 stale object URL을 revoke하므로 이전 응답은 provider call이나 download를 복구할 수 없다.

Trajectory capture가 활성화된 DocumentOps Agent 요청은 service가 provider를 호출하기 전에 tenant별 operation receipt를 selected `StateBackend`에 conditional create한다. Receipt는 caller operation ID를 path-safe SHA-256 key로 바꾸고 exact skill binding을 포함한 canonical request hash만 보존한다. 최초 owner만 provider를 호출하며 성공 결과와 trajectory binding을 같은 receipt의 terminal state로 CAS한다. Exact replay는 terminal result와 original binding을 읽고 provider usage를 다시 기록하지 않는다. 같은 operation ID에서 selected skill이나 catalog가 달라지면 claim identity가 달라져 provider 전에 conflict한다. API-key 보호 status 조회는 같은 tenant path의 receipt를 strict decode한 뒤 operation ID, 상태, 시각, replay 가능 여부와 다음 행동만 반환한다. Owner, request/result hash와 저장 결과는 응답과 audit에서 제외한다. Browser는 status를 `no-store`로 읽고 schema, operation ID, state별 timestamp·replay·next-action, read-only와 provider-call 비승인 필드를 함께 검증한다. 최초 응답을 잃은 뒤 mismatched·unavailable·running 상태면 captured tenant와 payload를 page memory에 보존하며 Agent 버튼과 상태 재확인 버튼을 같은 recovery promise에 연결한다. Terminal success만 exact replay를 허용하고 failed 상태는 pending recovery를 끝낸다. Captured POST 직전에는 schema version, tenant ID, browser UUID operation ID만 tenant-scoped same-origin shared marker로 기록하고 payload는 저장하지 않는다. Shared storage가 가능하면 tenant별 Web Lock 안에서 현재 marker 확인과 새 marker 기록을 한 claim으로 직렬화한다. 다른 tab이 이미 claim했다면 새 operation을 보내지 않고 기존 status만 읽는다. Marker storage key도 tenant별로 분리하므로 같은 origin의 foreign tenant read/write와 clear가 다른 tenant operation을 지우지 않는다. Login·register·refresh·LDAP login은 token claims를 먼저 검증하고 access/refresh token을 쓴 뒤 signed tenant ID를 마지막 browser commit point로 저장한다. 어느 write든 실패하면 확보한 이전 token과 tenant 값을 복원하고 current user를 바꾸지 않는다. 401 recovery는 refresh 성공, credential 거절, 일시 장애, storage commit 실패를 구분하고 같은 tab의 동시 caller를 refresh promise 하나로 합친다. 기존 provider retry helper만 refresh 성공 뒤 caller-bound request를 한 번 재시도한다. Generic API error path는 mutating request를 자동 replay하지 않고 explicit retry를 요구하며, refresh token snapshot이 바뀐 늦은 응답은 새 session을 덮어쓰지 않는다. Credential 거절만 invalid-session cleanup을 수행하고 나머지 실패는 이전 session과 DocumentOps evidence를 보존한다. 승인된 tenant 전환도 tenant ID write가 성공한 뒤 previous context marker와 page-memory evidence를 정리한다. 저장 실패는 이전 tenant와 evidence를 그대로 두고 새 tenant marker도 건드리지 않는다. 다른 tab의 auth storage가 현재 page와 다른 signed user 또는 tenant로 바뀌면 앱을 한 번 reload해 current user, draft와 recovery state를 새 인증 맥락에서 다시 구성한다. 같은 user·tenant의 token rotation은 reload하지 않는다. Browser는 shared와 tab의 scoped marker를 먼저 읽고 H96 base-key marker는 그 뒤 strict decode한 owning tenant에게만 legacy fallback으로 제공한다. Foreign tenant는 legacy marker를 삭제하지 않는다. Shared storage가 막히면 tenant-scoped tab fallback으로 내려가며, 두 storage가 모두 막혀도 same-page 요청은 계속된다. Reload와 tab close 뒤에는 shared marker로 status-only read를 수행하고 새 POST를 차단한다. Payload가 없으므로 exact replay하지 않으며, operator가 backend 실행 비취소와 evidence 확인 경고를 승인해 marker를 명시적으로 제거해야 새 operation을 시작할 수 있다. Tenant-scoped slot의 marker는 exact key set·tenant·UUID 형식에 fail closed하고 logout 또는 invalid session에서 현재 context marker를 제거한다. Web Locks 미지원 환경의 완전 동시 claim, 다른 browser/device, provider 성공 이후 process crash 자동 복구, cross-ID deduplication, exactly-once 실행, receipt expiry·GC는 제공하지 않는다. 2026-08-12 focused local registry·Agent·API·operation-store·audit command는 `143 passed`였고 mock provider와 local/fake-S3 backend에 한정된다. 실제 live provider와 AWS runtime은 별도 증거가 필요하다.

Auth refresh의 monotonic revision은 현재 tab의 session commit/cleanup뿐 아니라 같은 origin의 다른 tab에서 발생한 `dd_access_token`, `dd_refresh_token`, `dd_tenant_id` 변경과 `localStorage.clear()`에도 반응한다. Refresh 시작 뒤 이 revision이 바뀌면 응답 token이 유효하고 refresh-token bytes가 같아도 이전 응답을 `superseded`로 폐기한다. 다른 storage key는 revision을 바꾸지 않는다. Receiving page는 final access-token claims와 stored tenant를 현재 user·tenant·role·credential version과 비교해 authorization-context mismatch에서 reload를 한 번만 요청하고, 네 값이 같은 token rotation은 현재 page-memory 작업을 유지한다. Protected request middleware는 signed tenant/user identity로 현재 `UserStore`를 읽어 persisted role, `is_active`, credential version을 적용한다. `/events`는 protected route로 두고 browser fetch-stream의 Authorization Bearer token을 같은 resolver에 연결한다. 새 subscription은 연결 전에 검사하고, 열린 subscription은 최대 15초 간격으로 token signature·expiry와 persisted user authority를 다시 확인한다. Invalid access에는 application data가 없는 `auth_revoked` control event를 보낸 뒤 unsubscribe하고, authority read 실패는 `auth_unavailable`을 보낸 뒤 fail closed한다. Browser는 revoked event에서 기존 single-flight refresh를 사용하고 explicit credential rejection만 session cleanup으로 연결한다. Temporary authority failure는 현재 credential과 page-memory evidence를 보존하고 stale stream callback이 replacement stream을 닫지 못하게 한 뒤 reconnect한다.

새 access/refresh pair는 tenant-scoped `AuthSessionStore`가 conditional create한 random lowercase UUID hex session과 결속된다. 신규 record는 nullable `label`을 포함한 strict `auth-session.v2`이고, 기존 `auth-session.v1`은 exact legacy schema로만 읽는다. 첫 label mutation은 v1 record를 v2로 승격한다. Register, login, invite acceptance, LDAP, SAML, GCloud와 password change는 pair 발급 전에 session을 만들고, refresh는 기존 session ID를 유지한다. Protected request, refresh exchange와 `/events`는 token의 tenant·user·credential version뿐 아니라 그 session이 동일 owner/version으로 current인지 확인한다. `POST /auth/logout`은 현재 signed session만 exact owner 조건 아래 CAS로 폐기하며 같은 사용자의 다른 로그인은 바꾸지 않는다.

`GET /auth/sessions`는 tenant session prefix의 direct JSON child를 모두 strict decode한 뒤 현재 user와 credential version의 active record만 newest-first로 반환한다. `PATCH /auth/sessions/label`은 active owner가 입력한 이름을 trim하고 40자로 제한한다. API와 persisted state는 같은 validator를 사용하며 Unicode control, surrogate, line/paragraph separator와 bidirectional·invisible format 문자를 거부한다. 자연어와 emoji 조합에 필요한 ZWNJ와 ZWJ만 format 문자 예외로 허용한다. Foreign, missing, stale-version, revoked와 expired target은 같은 `404`로 숨기고, corrupt persisted label이나 prefix object는 원본을 바꾸지 않은 채 `503`으로 fail closed한다.

`POST /auth/sessions/revoke`는 exact owner의 다른 session만 CAS revoke한다. Current target은 `409`, foreign/missing target은 같은 `404`, already-revoked owner retry는 success로 수렴한다. `POST /auth/sessions/revoke-others`와 `POST /auth/sessions/revoke-all`은 strict `confirm=true`와 whole-prefix validation을 요구한다. Full revoke는 current session을 마지막에 쓴다. Bulk write는 multi-object transaction이 아니므로 중간 failure 뒤 일부 다른 session이 이미 폐기됐을 수 있고, current write 응답 유실과 snapshot 뒤 생성된 session도 원자적으로 조정하지 않는다.

`GET /admin/auth-sessions/retention-preview`는 admin JWT 또는 Ops key로만 접근하는 tenant-scoped read path다. `AuthSessionStore`가 selected local/S3 backend의 session prefix 전체를 strict decode한 뒤 active, retained inactive, retention cutoff 이전 expired/revoked count와 가장 오래된 후보 시각만 계산한다. `auth-session-retention-preview.v1` 응답은 session ID, user ID와 label을 포함하지 않고 `read_only=true`, `deletion_authorized=false`, `Cache-Control: no-store`를 고정한다. 조회와 audit은 aggregate만 사용하며 어떤 object도 쓰거나 삭제하지 않는다. Prefix가 손상됐거나 읽을 수 없으면 원본을 보존하고 `503`으로 닫는다.

`GET /admin/auth-sessions/retention-comparison`은 같은 권한과 strict prefix read를 재사용해 30/90/180/365일 정책을 한 번에 계산한다. `auth-session-retention-comparison.v1`의 네 정책은 같은 검증 record 집합과 `generated_at`을 공유하지만, 여러 backend object를 읽는 과정 자체는 transaction이 아니므로 `snapshot_atomic=false`, `requires_recheck_before_mutation=true`를 명시한다. 응답과 audit은 정책별 aggregate만 포함하고 `read_only=true`, `deletion_authorized=false`, `Cache-Control: no-store`를 유지한다.

`GET /admin/auth-sessions/retention-handoff?retention_days=30|90|180|365`는 comparison 한 번을 tenant-bound `auth-session-retention-review-handoff.v2`로 감싼 JSON attachment다. `tenant_id`, `selected_policy_days`, comparison 전체와 canonical `comparison_sha256`을 포함하며 `review_only=true`, `policy_change_authorized=false`, `deletion_authorized=false`, `scheduler_authorized=false`, `snapshot_atomic=false`, `requires_recheck_before_mutation=true`, `handoff_persisted=false`를 고정한다. v1은 역사적 evidence로만 남고 recheck 입력으로는 사용하지 않는다. 응답 body는 canonical JSON bytes로 만들고 `X-DecisionDoc-Auth-Session-Retention-Handoff-SHA256`, `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`를 함께 반환한다. Handoff artifact는 server-side file/object로 저장하지 않지만 aggregate-only audit history는 기존 경로에 남긴다.

`POST /admin/auth-sessions/retention-handoff/recheck`는 admin JWT 또는 Ops key로만 접근한다. `auth-session-retention-recheck-request.v1`의 exact v2 source handoff와 canonical full-handoff SHA-256을 먼저 tenant·authority·comparison hash까지 검증하고, 같은 selected policy로 한 번 새 strict inspection을 수행한다. `auth-session-retention-recheck-receipt.v1`은 source/current handoff와 SHA-256, source/current aggregate fingerprint SHA-256, `aggregate_status`, `fingerprint_algorithm=sha256`, `volatile_fields_excluded=[comparison.generated_at, comparison.policies[*].eligible_before]`, `aggregate_only=true`, `review_only=true`, false policy/delete/scheduler authority, `snapshot_atomic=false`, `requires_recheck_before_mutation=true`, `recheck_persisted=false`만 포함한다. Exact body SHA-256 header, `no-store`, `nosniff`, safe attachment filename을 반환하며 source/current handoff, receipt, session, policy, scheduler state를 저장하지 않는다. `unchanged`는 aggregate equivalence일 뿐 session set identity나 mutation safety가 아니다.

`POST /admin/auth-sessions/retention-handoff/review-disposition`는 dependency-based admin/Ops authorization 뒤에 exact `auth-session-retention-review-disposition-request.v1`을 받는다. Contract layer는 H117 receipt 전체와 source/current handoff hash, aggregate fingerprint, tenant, selected policy, aggregate status, false authority flags를 재검증한 뒤 `auth-session-retention-review-disposition-receipt.v1`을 canonical attachment로 만든다. Receipt는 `review_disposition`, canonical `disposition_binding_sha256`, `receipt_status=issued`, `review_only=true`, `reviewer_identity_bound=false`, `approval_granted=false`, `execution_authorized=false`, `policy_change_authorized=false`, `deletion_authorized=false`, `scheduler_authorized=false`, `mass_revoke_authorized=false`, `snapshot_atomic=false`, `requires_recheck_before_mutation=true`, `decision_receipt_persisted=false`를 고정한다. Aggregate가 unchanged이면 `acknowledged_unchanged` 또는 `review_deferred`, changed이면 `new_handoff_required` 또는 `review_deferred`만 허용한다. 인증 뒤 disposition handler는 retention 대상 session이나 policy state를 조회·변경하지 않으며 audit에는 policy, aggregate status, disposition, two hashes와 review-only boundary만 남긴다.

Ops dashboard의 retention panel은 30/90/180/365일 selector, icon refresh, 검토용 JSON download, `검토 자료 재확인`과 allowlisted disposition export control을 제공한다. Access token 또는 Ops key가 없으면 요청을 만들지 않고 authorization 안내를 유지한다. Browser는 comparison schema, read/delete boundary, nonnegative integer count와 합계, timestamp를 다시 검증한다. Handoff와 verified recheck receipt는 exact bytes/object/hash를 page memory에만 보관한다. Disposition은 outer body/header hash, H117 source receipt, source/current nested hashes, tenant, policy, aggregate status, disposition matrix와 all-false authority flags를 모두 통과한 경우에만 render/download한다. Selector change, refresh, tenant·auth/Ops-key context change, new handoff/recheck 또는 newer disposition request는 prior source와 late completion을 폐기한다. Ops Key 변경은 H118 전용 generation/source만 무효화하며 진행 중 JWT refresh의 auth revision은 변경하지 않는다. 이 UI는 approval, deletion, scheduler 또는 mass revoke control을 추가하지 않는다.
