...
`GET /admin/auth-sessions/retention-handoff?retention_days=30|90|180|365`는 comparison 한 번을 `auth-session-retention-review-handoff.v1`로 감싼 JSON attachment다. `selected_policy_days`, comparison 전체와 canonical `comparison_sha256`을 포함하며 `review_only=true`, `policy_change_authorized=false`, `deletion_authorized=false`, `scheduler_authorized=false`, `snapshot_atomic=false`, `requires_recheck_before_mutation=true`, `handoff_persisted=false`를 고정한다. 응답 body는 canonical JSON bytes로 만들고 `X-DecisionDoc-Auth-Session-Retention-Handoff-SHA256`, `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`를 함께 반환한다. Handoff artifact는 server-side file/object로 저장하지 않지만 aggregate-only audit history는 기존 경로에 남긴다.

Ops dashboard의 retention panel은 30/90/180/365일 selector, icon refresh와 검토용 JSON download를 제공한다. Access token 또는 Ops key가 없으면 요청을 만들지 않고 authorization 안내를 유지한다. Browser는 comparison schema, read/delete boundary, nonnegative integer count와 합계, timestamp를 다시 검증한다. Handoff는 exact body SHA-256, canonical comparison SHA-256, all-false authority flags를 모두 검증한 뒤에만 같은 response comparison을 렌더링하고 exact bytes를 다운로드한다. Request generation과 tenant가 현재 상태와 일치하는 최신 응답만 화면이나 download를 갱신하므로 tenant 전환 또는 연속 조회의 늦은 completion은 버린다. 이 UI는 삭제 endpoint, scheduler 또는 mutation control을 추가하지 않는다.
