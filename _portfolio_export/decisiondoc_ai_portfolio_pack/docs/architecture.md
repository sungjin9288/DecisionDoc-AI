...
새 access/refresh pair는 tenant-scoped `AuthSessionStore`가 conditional create한 random lowercase UUID hex session과 결속된다. Register, login, invite acceptance, LDAP, SAML, GCloud와 password change는 pair 발급 전에 session을 만들고, refresh는 기존 session ID를 유지한다. Protected request, refresh exchange와 `/events`는 token의 tenant·user·credential version뿐 아니라 그 session이 동일 owner/version으로 current인지 확인한다. `POST /auth/logout`은 현재 signed session만 exact owner 조건 아래 CAS로 `revoked_at`을 기록하며 같은 사용자의 다른 로그인 session은 바꾸지 않는다. Corrupt·unavailable session object는 legacy compatibility로 축소하지 않고 원본을 보존한 채 `503`으로 fail closed하며, duplicate JSON key와 identity drift도 같은 경계에서 거부한다. Browser logout은 access token을 capture한 뒤 server revoke를 시작하고 local credential·current user·draft·pending recovery·SSE를 즉시 정리한다. Revoke endpoint 실패는 local logout을 되돌리지 않고 서버 폐기가 확인되지 않았다는 경고만 남긴다. 같은 session을 복사한 다른 browser/device는 다음 request·refresh 또는 열린 SSE의 최대 15초 recheck에서 차단된다. 이는 exact-session authority와 bounded cross-device invalidation이며 즉시 push, session inventory·admin mass revoke, expired-session GC를 제공하지 않는다. H107 이전 sessionless token은 credential epoch와 만료 규칙으로 계속 승인되지만 exact logout은 `409`로 거부한다.
