...
Ops dashboard의 retention panel은 30/90/180/365일 selector, icon refresh, 검토용 JSON download와 `검토 자료 재확인` control을 제공한다. Access token 또는 Ops key가 없으면 요청을 만들지 않고 authorization 안내를 유지한다. Browser는 comparison schema, read/delete boundary, nonnegative integer count와 합계, timestamp를 다시 검증한다. Handoff는 exact body SHA-256, canonical comparison SHA-256, current tenant와 all-false authority flags를 모두 검증한 뒤 exact bytes/object/hash를 page memory에만 보관한다. Recheck는 source/current exact schema, body hash, aggregate fingerprint hash, tenant, selected policy, authority flags, auth-session revision과 request generation을 모두 통과한 receipt만 렌더링·download한다. Selector change, refresh, tenant·auth-context change와 더 새 handoff/recheck는 이전 completion을 폐기한다. 이 UI는 삭제 endpoint, scheduler 또는 mutation control을 추가하지 않는다.
