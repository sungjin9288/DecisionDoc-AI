...
| DocumentOps captured Agent retry recovery | Trajectory capture를 선택한 Agent run의 optional `operation_id`를 provider 호출 전에 selected backend에서 claim한다. Exact payload replay는 저장된 trajectory-bound 결과를 반환해 provider와 usage 기록을 반복하지 않고, changed/running/failed/corrupt 상태는 자동 재실행 전에 차단한다. Tenant-scoped status는 private receipt를 redacted projection으로만 보여주고 audit에 남긴다. Browser는 schema·operation·state contract를 모두 확인하고 lost response 뒤 terminal success에서만 같은 payload를 replay한다. Mismatched·unavailable·running 상태는 current-page memory에 보존하며 Agent 버튼과 재확인 버튼의 동시 요청도 recovery promise 하나로 수렴한다. Captured POST 전 payload 없는 exact 3-field marker를 same-origin shared storage에 남기고 tenant별 Web Lock claim으로 동시 tab 중 owner 하나만 POST를 시작하게 한다. 다른 tab과 owner tab 종료 뒤 다시 연 화면은 status만 확인하며, backend 실행 비취소 경고를 승인한 explicit release에서만 marker를 제거한다. Shared storage가 막히면 tab fallback을 사용하고 두 storage가 모두 막히면 same-page 실행만 유지한다. Uncaptured run, payload 없는 exact replay, 다른 browser/device, Web Locks 미지원 환경의 완전 동시 claim, process-crash recovery, cross-ID semantic deduplication과 receipt GC는 범위 밖이다 | `app/storage/document_ops_run_operation_store.py`, `app/services/document_ops_service.py`, `app/routers/document_ops_agent.py`, `app/middleware/document_ops_audit.py`, `app/static/index.html`, `tests/storage/test_document_ops_run_operation_store.py`, `tests/test_document_ops_agent_api.py`, `tests/test_audit.py`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4268 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-21 H96 실측) |
