...
| Persisted user authorization authority | Protected request와 `/events` query access token을 tenant `UserStore`의 현재 role·`is_active`에 결속한다. Role 변경은 token 만료 전에도 현재 RBAC로 반영하고 비활성 사용자의 기존 token은 거부하며, browser는 다른 tab의 role mismatch에서 reload해 이전 authorization page memory를 제거한다. Store failure는 fail closed 처리하고 별도 revocation table이나 active SSE termination은 주장하지 않는다 | `app/services/auth_service.py`, `app/middleware/auth.py`, `app/routers/events.py`, `app/static/index.html`, `tests/test_auth.py`, `tests/e2e/test_main_flow.py` |
...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4292 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-22 H105 실측) |
