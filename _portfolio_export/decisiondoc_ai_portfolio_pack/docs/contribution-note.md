...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4233 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-21 H85 실측) |
| GitHub Actions CI | 마지막으로 확인한 main 자동화 증적: commit `8f6cbbb`, CI `29772132367` success (`4232 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 확인한 main 자동화 증적: commit `8f6cbbb`, CD `29772132287` success. image digest는 `sha256:f931396bd22cf1c0fe59e660aa0fa9f1b00d7ec0703aa37529b748e8feb533dc`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
