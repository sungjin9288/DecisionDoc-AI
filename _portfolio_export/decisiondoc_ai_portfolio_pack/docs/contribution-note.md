...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4222 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-21 H79 실측) |
| GitHub Actions CI | 마지막으로 문서화한 main 자동화 증적: commit `2f36438`, CI `29756211158` success (`4222 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 문서화한 main 자동화 증적: commit `2f36438`, CD `29756211147` success. image digest는 `sha256:0706989bb8f7a4113bb77647d2fb626b3b161ee8b390ead08b3c21dd3723b379`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
