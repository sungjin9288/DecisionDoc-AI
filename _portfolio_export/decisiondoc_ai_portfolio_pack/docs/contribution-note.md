...
| Browser auth session failure atomicity | Login·register·refresh·LDAP login의 token과 signed tenant 저장을 공통 helper로 모았다. Claims를 먼저 검증하고 access/refresh token 뒤 tenant를 commit하며, 실패하면 이전 browser credential과 tenant를 복원한다. Current user와 DocumentOps draft/recovery/marker는 성공한 session만 이어받는다 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4271 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-22 H99 실측) |
| GitHub Actions CI | 마지막으로 확인한 main 자동화 증적: commit `78c60a8`, CI `29810817044` success (`4267 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 확인한 main 자동화 증적: commit `78c60a8`, CD `29810817004` success. image digest는 `sha256:82cb4a0d469abad014441e5118d296d313cfece2b39b963a29647d37480a8fb9`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
