...
| Report quality pilot review | 같은 tenant의 ready correction artifact 3~5개를 검증 가능한 ZIP으로 handoff하고, 교정 전후 evidence·validation blocker·required action과 사람의 결정을 잇는 source-bound browser workspace를 준비한다. 내려받은 decision draft는 이동 없이 검증하고 SHA 기반 원본 보관본·draft 반영·application receipt를 한 명령으로 연결하며, ready sync는 현재 manifest와 accepted decision receipt가 모두 유효할 때만 JSONL을 쓴다. 완료된 JSONL은 최종 draft와 검수·source evidence를 포함한 deterministic handoff ZIP으로 다시 묶고, 사람이 읽는 summary와 기계 검증 결과를 같은 archive에서 확인한다. 검증된 summary만 별도 Markdown으로 안전하게 꺼내고 exact SHA-256을 전달 증거로 남기며, write-once publication으로 동시 생성 경쟁에서도 기존 증거를 보존한다. | `app/routers/report_workflows.py`, `app/services/report_quality_pilot_package.py`, `scripts/create_report_quality_pilot_pack.py`, `scripts/create_report_quality_review_sheet.py`, `scripts/create_report_quality_review_workspace.py`, `scripts/apply_report_quality_review_decisions.py`, `scripts/sync_report_quality_pilot_pack.py`, `scripts/manage_report_quality_pilot_handoff.py`, `scripts/report_quality_pilot_handoff_summary.py`, `scripts/local_write_once.py`, `tests/test_manage_report_quality_pilot_handoff.py`, `tests/test_local_write_once.py`, `tests/test_sync_report_quality_pilot_pack.py` |
