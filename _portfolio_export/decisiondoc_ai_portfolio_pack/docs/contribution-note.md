...
| Browser auth session failure atomicity | Login·register·refresh·LDAP login의 token과 signed tenant 저장을 공통 helper로 모았다. Claims를 먼저 검증하고 access/refresh token 뒤 tenant를 commit하며, 실패하면 이전 browser credential과 tenant를 복원한다. 401 recovery는 refreshed·invalid session·temporary unavailable·storage failure를 구분하고 invalid session에서만 credential과 DocumentOps evidence를 정리한다 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` |
...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4273 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-22 H100 실측) |
