...
| Report quality pilot review | 같은 tenant의 ready correction artifact 3~5개를 검증 가능한 ZIP으로 handoff하고, source-bound review pack·worksheet·non-overwriting decision template을 한 번에 준비 | `app/routers/report_workflows.py`, `app/services/report_quality_pilot_package.py`, `scripts/create_report_quality_pilot_pack.py`, `scripts/create_report_quality_review_sheet.py`, `scripts/apply_report_quality_review_decisions.py`, `scripts/report_quality_pilot_pack_provenance.py`, `tests/test_create_report_quality_pilot_pack.py` |
