...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | `pytest tests/ -m "not live" -q` -> `3171 passed, 2 skipped, 4 deselected` (2026-07-16 실측) |
| GitHub Actions CI | 최근 확인한 main 자동화 증적: commit `558b8ed`, CI `29456452543` success (`3146 passed, 5 skipped`) |
| GitHub Actions CD | 최근 확인한 main 자동화 증적: commit `558b8ed`, CD `29456452580` success. image build/push는 통과했고 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
