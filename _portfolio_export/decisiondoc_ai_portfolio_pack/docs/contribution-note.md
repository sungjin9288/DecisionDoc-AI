...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4220 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-20 H77 실측) |
| GitHub Actions CI | 마지막으로 문서화한 main 자동화 증적: commit `7afe707`, CI `29746294342` success (`4217 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 문서화한 main 자동화 증적: commit `7afe707`, CD `29746294177` success. image digest는 `sha256:ee9f3518fe4f29fd7b467b9a835601c408fb7fd8d8ed4973360cb61a51aa5f0f`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
