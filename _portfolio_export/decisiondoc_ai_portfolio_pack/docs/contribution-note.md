...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | `pytest tests/ -m "not live" -q` -> `3120 passed, 2 skipped, 4 deselected` (2026-07-16 실측) |
| GitHub Actions CI | 최근 확인한 main 자동화 증적: commit `73a5d5d`, CI `29449254838` success |
| GitHub Actions CD | 최근 확인한 main 자동화 증적: commit `73a5d5d`, CD `29449254755` success. image build/push는 통과했고 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
