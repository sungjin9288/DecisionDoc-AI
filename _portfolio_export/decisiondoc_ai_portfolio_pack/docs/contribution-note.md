...
| 검증 | 현재 증거 |
|---|---|
| Non-live pytest gate | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4221 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-21 H78 실측) |
| GitHub Actions CI | 마지막으로 문서화한 main 자동화 증적: commit `a09e9b0`, CI `29753102156` success (`4221 passed, 5 skipped`) |
| GitHub Actions CD | 마지막으로 문서화한 main 자동화 증적: commit `a09e9b0`, CD `29753101339` success. image digest는 `sha256:6a20322db548c88816b12256e705635b2eb9a41aa902d4b0706a9e91e73c364e`이며 staging deploy/smoke와 production deploy는 skip되어 M6 proof로 보지 않는다 |
