...
| 축 | 현재 | 완성 기준 |
|----|------|-----------|
| **기능 검증** | non-live test suite 통과 (`pytest tests/ -m "not live" -q` → 4,266 passed, 2 skipped, 4 deselected, 1 warning, 2026-07-21 H95) | 외부 의존 경로(live LLM, G2B 실데이터)도 최소 1회 실증 + 증적 |
...
```bash
# 재현: 테스트 베이스라인
pytest tests/ -m "not live" -q     # 2026-07-21 H95 실측: 4266 passed, 2 skipped, 4 deselected, 1 warning
...
python3 scripts/count_readme_metrics.py --field storage_files     # → 46 (top-level storage modules)
python3 scripts/count_readme_metrics.py --field middleware_files  # → 10 (미들웨어)
python3 scripts/count_readme_metrics.py --field route_decorators  # → 269 (라우트)
```
...
FastAPI (app/main.py — create_app(), 모듈 레벨 side-effect 없음)
  ...
  │
  ├─ Routers (23 top-level files, 라우트 269):
...
30. Captured Agent POST 직전에는 schema version, tenant ID, browser UUID operation ID만 tab-scoped marker로 남기고 payload는 browser storage에 저장하지 않는다. Reload 뒤 marker는 strict `no-store` status 확인과 새 POST 차단에만 사용하며 exact replay authority가 아니다. Operator가 backend 실행 비취소와 evidence 확인 경고를 승인해 상태 추적을 종료해야 marker를 제거한다. Invalid marker와 auth/tenant context 변경은 marker를 폐기하고, storage 접근 실패는 same-page Agent 실행을 막지 않는다. Tab 종료·cross-tab 조정, process-crash recovery와 external provider authority는 이 marker가 제공하지 않는다.
