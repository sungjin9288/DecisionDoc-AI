...
> 기준일: **2026-07-24** (저장소 점검 [docs/inspection-20260630.md](./inspection-20260630.md), M4 CSP nonce 완료, H122 focused local verification과 최근 확인한 CI/CD success 기준)
...
| 축 | 현재 | 완성 기준 |
|----|------|-----------|
| **기능 검증** | non-live test suite 통과 (`pytest tests/ -m "not live" -q` → 4,448 passed, 2 skipped, 4 deselected, 2026-07-24 H122) | 외부 의존 경로(live LLM, G2B 실데이터)도 최소 1회 실증 + 증적 |
...
```bash
# 재현: 테스트 베이스라인
pytest tests/ -m "not live" -q     # 2026-07-24 H122 실측: 4448 passed, 2 skipped, 4 deselected
...
python3 scripts/count_readme_metrics.py --field service_files     # → 45 (서비스)
...
python3 scripts/count_readme_metrics.py --field route_decorators  # → 285 (라우트)
```
...
FastAPI (app/main.py — create_app(), 모듈 레벨 side-effect 없음)
  ...
  │
  ├─ Routers (23 top-level files, 라우트 285):
