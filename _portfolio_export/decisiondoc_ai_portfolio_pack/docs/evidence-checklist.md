...
| Non-live 전체 pytest gate | 완료 | `pytest tests/ -m "not live" -q` -> `3370 passed, 2 skipped, 4 deselected` (2026-07-16 실측) |
| GitHub Actions CI | 완료 | 최근 확인한 main 자동화 증적: commit `97685fb`, CI `29468939994` success (`3331 passed, 5 skipped`) |
| GitHub Actions CD | 완료 | 최근 확인한 main 자동화 증적: commit `97685fb`, CD `29468939963` success. image build/push는 통과했고 staging deploy/smoke와 production deploy는 skip되어 배포 proof에서 제외 |
