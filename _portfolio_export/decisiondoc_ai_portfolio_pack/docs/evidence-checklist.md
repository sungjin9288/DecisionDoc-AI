...
| Non-live 전체 pytest gate | 완료 | 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정한 `pytest tests/ -m "not live" -q` -> `4229 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-21 H83 실측) |
| GitHub Actions CI | 완료 | 마지막으로 확인한 main 자동화 증적: commit `5c7813c`, CI `29766346149` success (`4228 passed, 5 skipped`) |
| GitHub Actions CD | 완료 | 마지막으로 확인한 main 자동화 증적: commit `5c7813c`, CD `29766346150` success. image digest `sha256:5ea5675f00d08e5808710635983b2434db053ff8d0fbdf78bd48f259f2c9095e`; staging deploy/smoke와 production deploy는 skip되어 배포 proof에서 제외 |
