...
| Browser auth session failure atomicity | 검증 완료 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` | Refresh response 뒤 tenant write 실패를 실제 Chromium으로 주입해 기존 access/refresh token, tenant, current user와 DocumentOps draft/recovery/marker가 복원되는지 확인한다. Static contract는 login·register·refresh·LDAP login이 공통 helper만 사용하고 token write 뒤 tenant commit, snapshot-complete rollback 순서를 유지하는지 고정한다 | Browser storage는 signed auth나 backend authority를 대체하지 않는다. 외부 identity provider, provider API와 runtime은 호출하지 않는다 |
...
결과: `4271 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-22 H99 실측, 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정).
