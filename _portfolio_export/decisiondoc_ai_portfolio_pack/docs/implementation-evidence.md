...
| Browser auth session failure atomicity | 검증 완료 | `app/static/index.html`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` | Refresh response 뒤 tenant write 실패를 실제 Chromium으로 주입해 기존 access/refresh token, tenant, current user와 DocumentOps draft/recovery/marker가 복원되는지 확인한다. 동시 401은 refresh 1회에 합류하고, generic API 401은 session 갱신 뒤에도 원 요청을 자동 replay하지 않는다. 같은 origin의 두-page Chromium에서 same-identity replacement session은 늦은 refresh만 `superseded`로 폐기하고 현재 작업을 유지한다. 다른 signed user·tenant를 commit하면 receiving page가 한 번 reload되어 replacement credential을 보존하고 이전 page-memory draft를 제거하는지 검증한다 | Browser storage는 signed auth나 backend authority를 대체하지 않는다. 이 guard는 same-origin page reload와 in-flight refresh 무효화 범위이고 cross-device coordination은 제공하지 않는다. Generic mutating request는 explicit retry가 필요하며 외부 identity provider, provider API와 runtime은 호출하지 않는다 |
...
결과: `4278 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-22 H103 실측, 외부 provider·G2B·Stripe key를 process에서 제거하고 provider capability를 mock으로 고정).
