...
| Persisted exact-session logout authority | 검증 완료 | `app/storage/auth_session_store.py`, `app/services/auth_service.py`, `app/middleware/auth.py`, `app/routers/auth.py`, `app/routers/sso.py`, `app/routers/admin/_invite.py`, `app/static/index.html`, `tests/storage/test_auth_session_store.py`, `tests/test_auth.py`, `tests/test_audit.py`, `tests/test_infrastructure.py`, `tests/e2e/test_main_flow.py` | Token pair의 same-session binding, refresh identity 보존, selected-backend authority, exact current-session CAS revoke, separate login 보존, viewer logout, same-session request·refresh·SSE 차단, corruption/availability fail-closed와 original bytes 보존, audit redaction, browser immediate local cleanup과 revoke-unavailable warning을 확인한다 | Legacy sessionless token은 expiry/credential epoch까지 호환하고 exact logout은 `409`다. Session inventory, mass revoke, expired-session GC, 즉시 push와 외부 identity/runtime은 범위 밖이다 |
...
결과: `4314 passed, 2 skipped, 4 deselected, 1 warning` (2026-07-22 H107 실측, 외부 provider·G2B·Stripe key를 빈 값으로 고정하고 provider capability를 mock으로 강제).
