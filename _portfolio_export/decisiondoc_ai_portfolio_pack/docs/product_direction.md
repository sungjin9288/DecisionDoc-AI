...
- Password rotation is the exception to quiet same-identity token replacement: password hash and persisted credential version advance together, every older access/refresh token is rejected, the initiating browser commits a replacement pair, and another same-origin tab reloads when its version is stale. An open SSE connection rechecks the same token and current user authority at most every 15 seconds. It sends only a fixed revocation or temporary-unavailable control event before closing, and the browser uses the existing single-flight refresh boundary to distinguish invalid credentials from retryable authority failure. This is bounded all-token revocation for a password change, not per-session selection, immediate cross-device push, or a termination guarantee shorter than the recheck interval.
