...
- Password rotation is the exception to quiet same-identity token replacement: password hash and persisted credential version advance together, every older access/refresh token is rejected, the initiating browser commits a replacement pair, and another same-origin tab reloads when its version is stale. New token pairs also share a persisted tenant-scoped `auth-session.v2` identity, while strict v1 reads preserve existing sessions. Exact logout revokes only that session, while separate logins remain active; copied same-session credentials fail on the next protected request, refresh, or open SSE recheck. The profile can list active current-version sessions, assign or clear a user-supplied device name, revoke one owned non-current login, preserve the current browser while revoking the other sessions in a validated snapshot, or revoke that snapshot including current. Labels are bounded to 40 characters and stored with selected-backend CAS. API and persisted-state validation share one Unicode-aware rule: raw display controls, line/paragraph separators, bidi controls, and other invisible format characters are rejected before request trimming, while ZWNJ/ZWJ remain available for natural text and emoji composition. Invalid persisted labels fail closed without rewriting their bytes. Labels remain omitted from audit and do not add User-Agent or IP fields to session state or inventory. Missing, foreign, stale-version, revoked, and expired label targets share one `404`. The all-device path validates the full prefix before mutation and writes current last, so an earlier backend failure preserves the initiating browser when possible. Bulk writes remain sequential rather than transactional: partial other-session progress can survive a failure, a lost current-write response can leave the server revoked before browser cleanup, and a session created after the snapshot remains for the next inspection. An admin JWT or Ops key can request an aggregate-only, tenant-scoped retention preview for old expired/revoked state. The preview is read-only, discloses no user, session, or label identity, and explicitly denies deletion authority. The browser keeps session IDs out of the DOM and applies one single-flight plus stale token/modal/request/mutation generations to label and revoke actions. It clears local credentials and page-memory evidence only after confirmed all-device success. Audit stores aggregate counts without tokens, user IDs, session IDs, or labels. An open SSE connection rechecks the same token and current user/session authority at most every 15 seconds. This is bounded invalidation, not immediate cross-device push or a termination guarantee shorter than the recheck interval. Legacy sessionless tokens cannot use exact logout, inventory, labeling, selected revoke, or either bulk revoke; automatic User-Agent/IP session inventory, administrator mass revoke, and actual expired-session deletion or scheduling are not included.
- The Ops dashboard exposes retention governance as a bounded operational read. One strict inspection compares 30/90/180/365-day policy aggregates, while the browser validates the versioned contract, policy monotonicity, `snapshot_atomic=false`, and the false deletion boundary before rendering the latest current-tenant response. A selected policy can be exported as tenant-bound `auth-session-retention-review-handoff.v2` for review only, then rechecked through `auth-session-retention-recheck-receipt.v1`: the browser verifies source/current exact schema, body SHA-256, comparison and aggregate fingerprint SHA-256, tenant, selected policy, and false policy-change/delete/scheduler authority before it renders or downloads the exact bytes. A verified recheck can produce `auth-session-retention-review-disposition-receipt.v1` with only `acknowledged_unchanged`, `new_handoff_required`, or `review_deferred`; this leaves reviewer identity unbound and approval, execution, policy, deletion, scheduler, and mass-revoke authority false. `unchanged` expresses aggregate equivalence only, not session set identity or mutation safety; `changed` requires a new handoff and creates no approval state. Selector changes, refresh, tenant or auth-context changes, and newer requests discard prior page-memory evidence. Handoff and receipt are not persisted server-side; aggregate-only audit history remains durable. No delete, scheduler, or policy application is available.
...
- Review packet creation, inbox, project history and completed-package download require a current session-bound admin or member. Admin has tenant-wide access; member has only records assigned to the current stable identity. Public summaries expose workflow state rather than receipt, rationale, stable target IDs, attestation or session metadata.
