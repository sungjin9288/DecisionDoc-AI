...
- A receiving tab reconciles same-origin auth changes against the final signed user and tenant. Same-identity token rotation invalidates any older refresh but keeps the current page and unsaved work. A different user, tenant, or logged-out session requests one reload so current user, review drafts, and pending recovery state are rebuilt under the new authentication context. Browser storage remains coordination evidence rather than authorization authority, and this behavior does not coordinate another browser or device.
