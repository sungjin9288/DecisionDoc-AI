...
- DocumentOps browser controls that create governance records or start a provider-backed Agent request are single-flight while pending. Freeze, dry-run approval, execution request, and audit export additionally send a per-action UUID that the backend binds to a private canonical payload hash. Exact replays return the original verified governance artifact and changed-payload reuse returns `409`; provider-backed Agent execution remains outside this backend idempotency contract. Because immutable publication precedes metadata CAS, a concurrent loser may remain as an unreferenced artifact for inventory review, so cross-object atomicity and zero-orphan are not claimed.
