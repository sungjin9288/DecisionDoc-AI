...
- The Ops dashboard exposes retention governance as a bounded operational read. One strict inspection compares 30/90/180/365-day policy aggregates, while the browser validates the versioned contract, policy monotonicity, `snapshot_atomic=false`, and the false deletion boundary before rendering the latest current-tenant response. A selected policy can be exported as `auth-session-retention-review-handoff.v1` for review only: the browser verifies the exact body SHA-256, canonical comparison SHA-256, and false policy-change/delete/scheduler authority before it renders the returned comparison or downloads the exact bytes. Selector changes remain local to a verified comparison, while refresh and handoff download each start one new inspection. The handoff is not persisted server-side; aggregate-only audit history remains durable. No delete, scheduler, or policy application is available.
