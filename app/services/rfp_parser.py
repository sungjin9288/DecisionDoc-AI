...
def build_rfp_context(attachment_text: str, *, normalized_context: str = "") -> str:
    ...
    parts.append(
        "원문 또는 정규화 요약에 없는 날짜, 예산, 기관명, 기술명, 배점, 수치는 임의로 만들지 마세요. "
        "근거가 없으면 '미기재', '확인 필요' 또는 일반 표현으로 유지하세요."
    )
    parts.append(
        "첨부에서 확인된 사실과 사용자가 입력한 제목/목표/추가 context만 근거로 작성하고, "
        "예시 기술 스택이나 마감일을 추정해서 채우지 마세요."
    )
    return "\n".join(parts)
