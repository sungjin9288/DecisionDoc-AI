...
def _quality_guard_proposal_bundle(bundle: dict[str, Any], *, title: str, goal: str) -> None:
    ...
    if isinstance(execution, dict):
        ...
        execution["team_structure"] = _sanitize_rows(
            ...
            [
                "PM·총괄 | 인원·등급 산정 필요 | 일정·범위·대외 협의 총괄 | 착수~종료 전기간",
                "AI 기술 리드 | 인원·등급 산정 필요 | 모델 품질·데이터 설계·성능 검증 책임 | 설계~통합시험",
                "개발 리드 및 구현 인력 | 인원·등급 산정 필요 | 서비스 개발·연계·배포 자동화 수행 | 개발~운영 전환",
                "품질 책임자 | 인원·등급 산정 필요 | 검수 기준 수립·결함 관리·인수시험 총괄 | 시험~종료",
            ],
            min_items=4,
        )
        execution["milestones"] = _sanitize_rows(
            ...
            [
                "착수 및 요구사항 확정 | 착수 구간 | 요구사항 정의서 승인 | 착수보고서, 현행 분석서",
                "아키텍처·상세 설계 완료 | 설계 구간 | 설계 검토 승인 | 아키텍처 설계서, 인터페이스 정의서",
                "개발 및 통합시험 완료 | 개발·검증 구간 | 합의된 핵심 시나리오 통과 | 시험 결과서, 운영자 검수 기록",
                "파일럿 및 최종 이관 완료 | 운영 이관 구간 | 최종 검수 승인 | 완료보고서, 운영 매뉴얼",
            ],
            min_items=4,
        )
