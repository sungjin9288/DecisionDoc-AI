...
def require_session_bound_generated_document_reviewer(request: Request) -> None:
    """Require a current admin or member session for generated review evidence."""
    require_auth(request)
    if getattr(request.state, "user_role", "") not in {"admin", "member"}:
        raise HTTPException(
            status_code=403,
            detail="문서 검토 전달에는 관리자 또는 멤버 권한이 필요합니다.",
        )
    session_id = getattr(request.state, "auth_session_id", None)
    if not isinstance(session_id, str) or not session_id:
        raise HTTPException(
