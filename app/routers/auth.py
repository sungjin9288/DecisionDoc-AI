...
async def revoke_other_auth_sessions(
    request: Request,
    body: RevokeOtherAuthSessionsRequest,
):
    """Revoke the current user's other active sessions from one validated snapshot."""
    from app.storage.auth_session_store import AuthSessionStoreError
    ...
    try:
        ...
        logger.error(
            "[Auth] Other session revocation failed — failing CLOSED.",
            exc_info=exc,
        )
        raise HTTPException(
            status_code=503,
            detail="다른 로그인 세션을 일시적으로 종료할 수 없습니다.",
        ) from exc

    request.state.auth_session_revoked_count = revoked_sessions
    message = (
        f"다른 로그인 세션 {revoked_sessions}개가 종료되었습니다."
        if revoked_sessions
        else "종료할 다른 로그인 세션이 없습니다."
    )
    return JSONResponse(
        content={
            "message": message,
