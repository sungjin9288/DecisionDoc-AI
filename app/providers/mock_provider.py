...
def _proposal_business_understanding(title: str, goal: str, ctx: str) -> dict:
    ...
    return {
        ...
        "project_objectives": [
            f"AI 기술 도입을 통한 {title} 핵심 업무 자동화 및 효율화",
            f"{goal} 달성을 위한 데이터 기반 의사결정 체계 구축",
            "실시간 모니터링 및 이상 징후 조기 감지 시스템 도입",
            "운영 비용 20% 절감 및 업무 처리 속도 2배 향상",
        ],
        "target_users": [
            "사업 담당 공무원 및 운영 관리자 (1차 사용자)",
            "현장 실무자 및 데이터 입력 담당자 (2차 사용자)",
            "경영진 및 의사결정자 — 대시보드 및 보고서 활용",
            "일반 국민 — 서비스 품질 향상 및 처리 속도 개선 간접 수혜",
        ],
        "scope_summary": (
            f"본 사업의 범위는 {title}에 필요한 AI 시스템 설계·개발·운영 전 과정을 포함합니다. "
        ...
        "slide_outline": [
            ...
            _slide(3, "사업 추진 배경",
                   f"정책 환경 변화 + 기술 트렌드(AI·빅데이터 확산) + {title} 필요성 3단 구조",
            ...
            _slide(6, "사업 목표 및 비전",
                   f"비전 선언문 + 3~4개 정량 목표(처리속도 ○%↑, 비용 ○%↓) + {goal} 달성 로드맵",
...
def _proposal_tech_proposal(title: str, goal: str, ctx: str) -> dict:
    return {
        "tech_stack": [
            "Python 3.11 / FastAPI — AI 서비스 백엔드 및 REST API 게이트웨이",
            "PyTorch / Hugging Face Transformers — AI 모델 학습 및 추론",
            "Apache Kafka — 실시간 데이터 스트리밍 및 이벤트 처리",
            "PostgreSQL + Redis — 정형 데이터 저장 및 고속 캐싱",
            "Kubernetes (EKS) — 컨테이너 오케스트레이션 및 무중단 배포",
            "React + TypeScript — 운영자 대시보드 프론트엔드",
        ],
        ...
        "ai_approach": (
            f"{goal}을 달성하기 위해 지도학습(분류·회귀) 및 이상 탐지(비지도 학습) 모델을 병행 적용합니다. "
            "사전 학습된 대형 언어 모델(LLM)을 파인튜닝하여 도메인 특화 문서 분석에 활용하고, "
            "강화학습 기반 최적화 알고리즘으로 운영 효율을 지속적으로 개선합니다."
        ),
        "security_measures": [
            "국가정보원 클라우드 보안인증(CSAP) 기준 준수 및 연 1회 취약점 점검",
            "개인정보보호법·전자정부법 준수, 데이터 암호화(AES-256) 및 접근 권한 최소화 적용",
            "API 게이트웨이 레벨 인증(OAuth 2.0 + JWT) 및 요청 속도 제한(Rate Limiting)",
            "AI 모델 입출력 필터링으로 프롬프트 인젝션 및 데이터 유출 방지",
        ],
        "differentiation": [
            f"기존 솔루션 대비 {title} 도메인 특화 AI 모델로 정확도 15% 이상 향상",
            "실시간 처리(<100ms) + 배치 처리 하이브리드 아키텍처로 상황별 최적 대응",
            "설명 가능한 AI(XAI) 적용으로 AI 판단 근거 투명 제공 — 공공기관 신뢰성 확보",
            "오픈소스 기반 구축으로 벤더 종속 최소화, 장기 유지보수 비용 절감",
        ],
        "total_slides": 15,
        "slide_outline": [
            _slide(1, "기술 제안 개요",
...
def _proposal_execution_plan(title: str, goal: str, ctx: str) -> dict:
    return {
        "team_structure": [
            "PM (1명): 전체 사업 총괄, 고객사 소통, 리스크 관리 및 일정 조율",
            "AI 엔지니어 (3명): 모델 설계·학습·최적화 및 MLOps 파이프라인 구축",
            "백엔드 개발자 (2명): API 서버·데이터 파이프라인·연동 시스템 개발",
            "프론트엔드 개발자 (1명): 운영자 대시보드 및 사용자 인터페이스 개발",
            "데이터 엔지니어 (1명): 데이터 수집·정제·적재 파이프라인 설계 및 운영",
            "품질 관리(QA) (1명): 테스트 계획 수립, 성능·보안·기능 검증",
        ],
        "milestones": [
            "M1 (1개월차): 현황 분석 완료, 요구사항 정의서 및 아키텍처 설계서 승인",
            "M2 (3개월차): 핵심 AI 모델 프로토타입 완성 및 내부 검증 통과",
            "M3 (6개월차): 전체 시스템 통합 개발 완료 및 알파 테스트 시작",
            "M4 (8개월차): 파일럿 운영 개시 — 실사용자 대상 베타 서비스",
            "M5 (10개월차): 성과 지표 검증 완료 및 전면 운영 전환",
            "M6 (12개월차): 최종 산출물 납품 및 유지보수 체계 이관",
        ],
        "methodology": (
        ...
        "risk_management": [
            "리스크 1: 데이터 품질 부족 → 데이터 정제 프로세스 조기 수립 및 합성 데이터 보완",
            "리스크 2: 일정 지연 → 핵심 기능 우선순위 재조정(MoSCoW) 및 버퍼 스프린트 확보",
            "리스크 3: 레거시 시스템 연동 이슈 → API 어댑터 레이어 설계로 인터페이스 표준화",
            "리스크 4: 보안 취약점 → 설계 단계부터 Security by Design 원칙 적용",
        ],
        "deliverables": [
            "요구사항 정의서 및 시스템 아키텍처 설계서",
            "AI 모델 학습 결과 보고서 (성능 지표 포함)",
            "소스코드 및 배포 패키지 (Docker 이미지)",
            "운영자 매뉴얼 및 API 명세서",
            "파일럿 운영 결과 보고서 및 개선 권고안",
        ],
        "total_slides": 12,
        "slide_outline": [
            _slide(1, "수행 계획 개요",
...
def _proposal_expected_impact(title: str, goal: str, ctx: str) -> dict:
    return {
        "quantitative_effects": [
            f"업무 처리 속도 향상: {title} 관련 핵심 업무 처리 시간 60% 단축 (8시간 → 3.2시간)",
            "운영 비용 절감: 수작업 인력 재배치로 연간 운영비 25% 절감 (약 3억 원)",
            "오류율 감소: AI 자동 검증으로 데이터 오류율 현행 12% → 2% 이하 달성",
            "서비스 가용성: 24/7 무중단 운영으로 서비스 가용률 99.9% 보장",
        ],
        "qualitative_effects": [
            "데이터 기반 의사결정 문화 정착으로 조직의 디지털 역량 전반 강화",
            f"{goal} 달성을 통한 국민 신뢰도 향상 및 공공서비스 브랜드 가치 제고",
            "담당자 반복 업무 부담 감소로 창의적·전략적 업무에 집중 가능한 환경 조성",
            "AI 활용 선도 사례 구축으로 타 기관 벤치마킹 및 정책 확산 기여",
        ],
        "social_value": (
            f"{title} 사업을 통해 공공서비스 품질을 획기적으로 개선함으로써 "
        ...
        "monitoring_plan": [
            "월별 KPI 대시보드 운영: 처리 속도·오류율·사용자 만족도 실시간 추적",
            "분기별 성과 보고서 작성 및 발주처 보고 — 목표 대비 달성률 점검",
            "AI 모델 성능 모니터링: 정확도·재현율 주간 자동 측정 및 임계값 이하 시 알림",
        ],
        "total_slides": 10,
        "slide_outline": [
            _slide(1, "기대효과 요약",
                   f"{title} 도입으로 얻는 3대 핵심 효과 한눈에: 속도·비용·품질 개선 수치",
            ...
            _slide(10, "결론 — 왜 지금, 왜 우리인가",
                   f"{title} 도입의 시급성 + 제안사 차별화 + 성공 확신 메시지",
...
def _performance_overview(title: str, goal: str, ctx: str) -> dict:
    ...
    return {
        "project_info": (
            f"{title} 사업 수행계획은 procurement decision state에서 확정된 범위와 리스크를 전제로 구성합니다. {excerpt}"
        ),
        "scope_of_work": [
            "공고 범위, 필수 산출물, 보안·인프라 요구사항을 기준으로 수행 범위를 정리합니다.",
            f"범위 참고 맥락: {excerpt}",
        ],
        "deliverables": [
            "제안 단계에서 확인된 필수 제출물과 계약 후 납품물을 구분해 관리합니다.",
            "Conditional Go 보완 항목은 착수 초기에 닫아야 할 선행 deliverable로 반영합니다.",
        ],
        "team_structure": [
            "BD Lead, Delivery Lead, PM, 전문 컨설턴트 역할을 구분해 투입 구조를 설계합니다.",
            f"인력 적합성 참고: {excerpt}",
        ],
        "wbs_summary": [
            "착수 전 자격·파트너·인력 보완",
            "요구사항 분석 및 발주기관 인터뷰",
            "설계·개발·검수·보고 단계의 월별 WBS 확정",
        ],
    }
...
def _performance_quality_risk(title: str, goal: str, ctx: str) -> dict:
    ...
    return {
        "quality_standards": [
            "필수 자격과 보안 요구사항을 품질 기준의 최상위 제약으로 둡니다.",
            "proposal handoff 시 정의된 평가 포인트와 KPI를 검수 기준에 연결합니다.",
        ],
        "inspection_criteria": [
            "주요 산출물별 검수 항목과 승인 기준을 명확히 정의합니다.",
            f"검수 근거 맥락: {excerpt}",
        ],
        "risk_matrix": [
            "blocking hard filter와 high-severity checklist 항목을 리스크 매트릭스의 상단에 배치합니다.",
            "일정 압박, 인력 가용성, 파트너 의존도, 보안 규제 리스크를 함께 관리합니다.",
        ],
        "change_management": (
            f"Go 또는 Conditional Go 결정 이후 범위·일정·자격 조건 변경을 통제하는 절차를 정의합니다. {excerpt}"
        ),
        "reporting_structure": (
            f"{goal} 달성을 위한 보고 체계는 PM, Delivery Lead, Executive Approver가 procurement decision state를 공유하는 구조로 설계합니다."
        ),
    }
