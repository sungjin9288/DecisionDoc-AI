...
``index.json``은 문서 목록과 최근 mutation receipt를 보존하는 mutable
authority다. Content/style object는 immutable conditional create로 발행하며,
기존 ``{doc_id}.txt``와 ``{doc_id}_style.json``은 read compatibility를
위해서만 유지한다.
