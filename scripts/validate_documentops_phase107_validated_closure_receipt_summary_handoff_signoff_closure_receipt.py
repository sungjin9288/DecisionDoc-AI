#!/usr/bin/env python3
"""Validate the DocumentOps Phase 108 validated closure receipt summary handoff sign-off closure receipt."""
...
from scripts.validate_documentops_phase102_to_phase106_validated_closure_receipt_summary_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_phase102_to_phase106_validated_closure_receipt_summary_handoff_signoff_closure_index,
)
...
DEFAULT_RECEIPT_PATH = (
    ...
    "phase108_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt/"
    "validated_closure_receipt_summary_handoff_signoff_closure_receipt.json"
)
EXPECTED_REPORT_TYPE = (
    "document_ops_phase108_local_feature_completion_validated_closure_receipt_summary_handoff_"
    "signoff_closure_receipt"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase108_local_feature_completion_validated_closure_receipt_summary_handoff_"
    "signoff_closure_receipt_validation"
)
EXPECTED_STATUS = "validated_closure_receipt_summary_handoff_signoff_closure_receipt_recorded_no_aws_no_training_authorization"
REQUIRED_SOURCE_HASH_PATHS = {
    "phase107_closure_index_sha256": (
        "docs/specs/hermes_decisiondoc_agent/"
    ...
    "phase107_closure_validator_sha256": (
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    ...
    "phase107_closure_index_valid",
    "phase102_to_phase106_chain_valid",
...
EXPECTED_SOURCE_COMMAND = (
    "python3 scripts/validate_documentops_phase102_to_phase106_validated_closure_receipt_summary_handoff_"
    "signoff_closure_index.py"
)
...
def validate_documentops_phase107_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 108:
        errors.append("phase must be 108")
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_phase107_validated_closure_receipt_summary_handoff_signoff_closure_receipt: {finding}")

    gate_result: dict[str, Any] = {}
    if not errors:
        gate_result = validate_documentops_phase102_to_phase106_validated_closure_receipt_summary_handoff_signoff_closure_index()
    if not errors and gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 107 closure index validation")
        ...
        for field, expected_value in expected_pairs.items():
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 107 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 108 validated closure receipt summary handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to Phase 108 validated closure receipt summary handoff sign-off closure receipt JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase107_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    elif result["ok"]:
        print("PASS documentops phase108 validated closure receipt summary handoff sign-off closure receipt validated")
        ...
        print("FAIL documentops phase108 validated closure receipt summary handoff sign-off closure receipt validation failed")
