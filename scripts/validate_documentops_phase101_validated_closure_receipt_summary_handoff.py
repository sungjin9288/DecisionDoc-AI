#!/usr/bin/env python3
"""Validate the DocumentOps Phase 100/101 validated closure receipt summary handoff."""
...
EXPECTED_REPORT_TYPE = "document_ops_phase102_local_feature_completion_validated_closure_receipt_summary_handoff"
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase102_local_feature_completion_validated_closure_receipt_summary_handoff_validation"
)
EXPECTED_STATUS = "local_feature_completion_validated_closure_receipt_summary_handoff_ready_no_aws_no_training_authorization"
READY_STATUS = "all_phase99_validated_closure_receipts_confirm_no_cost_freeze"
HANDOFF_VALID_KEY = "phase100_validated_closure_receipt_summary_handoff_valid"
SOURCE_SUMMARY_VALIDATION_CONTRACT_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase101_local_feature_completion_validated_closure_receipt_summary_validation/"
    "validated_closure_receipt_summary_validation_contract.json"
)
SOURCE_SUMMARY_CONTRACT_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase100_local_feature_completion_validated_closure_receipt_summary/"
    "validated_closure_receipt_summary_contract.json"
)
SOURCE_SUMMARY_VALIDATOR_PATH = "scripts/validate_documentops_phase100_validated_closure_receipt_summary.py"
SOURCE_SUMMARY_REPORTER_PATH = "scripts/summarize_documentops_phase99_validated_closure_receipts.py"
REQUIRED_RECIPIENTS = {
    "release_owner",
    "operator",
    "product_pm_reviewer",
...
REQUIRED_ACTION_IDS = {
    "generate_phase100_summary",
    "run_phase101_validator",
    "confirm_validated_local_phase100_summary",
    "confirm_no_cost_boundary",
    "preserve_service_freeze",
    "require_separate_resume_approval",
}
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    "local_phase100_validated_closure_receipt_summary_handoff_recorded",
...
PASS_MESSAGE = "PASS documentops phase102 validated closure receipt summary handoff validated"
FAIL_MESSAGE = "FAIL documentops phase102 validated closure receipt summary handoff validation failed"
...
def validate_documentops_phase101_validated_closure_receipt_summary_handoff(
    ...
    if handoff.get("phase") != 102:
        errors.append("phase must be 102")
    if handoff.get("status") != EXPECTED_STATUS:
        errors.append(f"status must be {EXPECTED_STATUS}")
    if handoff.get("handoff_scope") != "operator_handoff_for_validated_phase100_closure_receipt_summary":
        errors.append("handoff_scope must be operator_handoff_for_validated_phase100_closure_receipt_summary")
    ...
    for finding in _scan_forbidden_true(handoff):
        errors.append(f"documentops_phase101_validated_closure_receipt_summary_handoff: {finding}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 100/101 validated closure receipt summary handoff."
    )
    parser.add_argument(
        "handoff",
        nargs="?",
        ...
        help="Path to Phase 102 validated closure receipt summary handoff JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase101_validated_closure_receipt_summary_handoff(args.handoff)
    if args.json:
