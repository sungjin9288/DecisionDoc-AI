#!/usr/bin/env python3
"""Validate a human review record for a report quality training experiment plan draft."""
...
EXPECTED_SCHEMA = "decisiondoc_report_quality_training_experiment_plan_review.v1"
VALID_DECISIONS = {"pending", "planning_complete", "changes_requested", "rejected"}
COMPLETED_DECISIONS = {"planning_complete", "changes_requested", "rejected"}
VALID_NEXT_STEPS = {"none", "prepare_final_approval_packet", "revise_plan_draft", "archive_no_training"}
REQUIRED_ACKS = (
    "plan_draft_validated",
    "dataset_reference_reviewed",
    "offline_eval_suite_reviewed",
    "training_parameters_left_manual",
    "execution_steps_not_started",
...
def _load_plan_validator():
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"failed to load plan draft validator: {PLAN_VALIDATOR_PATH}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


_PLAN_VALIDATOR = _load_plan_validator()
...
def validate_training_experiment_plan_review(
    ...
    *,
    ...
    if require_complete and not _non_empty_string(payload.get("created_at")):
        errors.append("completed plan review requires created_at")
    ...
    if require_complete and not completed:
        errors.append("plan review decision must be completed when --require-complete is used")
    ...
    if completed:
        expected_next_steps = {
            "planning_complete": "prepare_final_approval_packet",
            "changes_requested": "revise_plan_draft",
    ...
    plan_path_value = payload.get("plan_manifest_path")
    plan_validation: dict[str, Any] = {}
    if _non_empty_string(plan_path_value):
        plan_path = Path(str(plan_path_value)).expanduser().resolve()
        if not plan_path.exists() or not plan_path.is_file():
            errors.append(f"plan_manifest_path does not exist: {plan_path}")
        else:
            expected_hash = payload.get("plan_manifest_sha256")
            if expected_hash and expected_hash != _sha256(plan_path):
                errors.append("plan_manifest_sha256 does not match plan_manifest_path")
            plan_validation = _PLAN_VALIDATOR.validate_training_experiment_plan_draft(plan_path)
            if completed and plan_validation.get("ok") is not True:
                errors.append("completed review requires valid training experiment plan draft")
    elif completed or require_complete:
        errors.append("completed review requires plan_manifest_path")
    ...
    if completed:
        warnings.append("completed plan reviews authorize planning handoff only; they do not authorize training")

    return {
        "report_type": "report_quality_training_experiment_plan_review_validation",
        "ok": not errors,
        ...
        "plan_validation_ok": plan_validation.get("ok") if plan_validation else None,
        "errors": errors,
        "warnings": warnings,
    }
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a report quality training experiment plan review record.")
    parser.add_argument("review_record", type=Path, help="Path to training experiment plan review JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = validate_training_experiment_plan_review(payload, require_complete=bool(args.require_complete))

    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    elif result["ok"]:
        print("PASS report quality training experiment plan review validated")
        ...
        print("FAIL report quality training experiment plan review validation failed")
