#!/usr/bin/env python3
"""Validate the DocumentOps validated closure receipt summary handoff."""
...
from scripts.summarize_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipts import (  # noqa: E402
    DEFAULT_RECEIPT_PATH,
    build_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_summary,
)
from scripts.validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_summary import (  # noqa: E402
    validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_summary,
)
...
EXPECTED_REPORT_TYPE = (
    "document_ops_phase66_local_feature_completion_validated_closure_handoff_signoff_"
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    "local_validated_closure_receipt_summary_handoff_recorded",
...
def _run_generated_summary_validation_probe() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="decisiondoc_phase66_validated_handoff_") as tmp_dir:
        summary = (
            build_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_summary(
                [DEFAULT_RECEIPT_PATH],
        ...
        summary_path = Path(tmp_dir) / "phase66_closure_receipt_summary.json"
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff(
    ...
    if handoff.get("phase") != 66:
        errors.append("phase must be 66")
    if handoff.get("status") != EXPECTED_STATUS:
        errors.append(f"status must be {EXPECTED_STATUS}")
    if handoff.get("handoff_scope") != "operator_handoff_for_validated_phase64_closure_receipt_summary":
        errors.append("handoff_scope must be operator_handoff_for_validated_phase64_closure_receipt_summary")
    ...
    for finding in _scan_forbidden_true(handoff):
        errors.append(f"documentops_validated_closure_receipt_summary_handoff: {finding}")
    ...
    if probe and probe.get("summary_ok") is not True:
        errors.append("generated Phase 64 closure receipt summary must be ok")
    if probe and validation.get("ok") is not True:
        errors.append("generated Phase 64 closure receipt summary must pass Phase 65 validation")
    ...
    if probe and probe.get("receipt_count") != 1:
        errors.append("generated summary receipt_count must be 1 for the default Phase 63 receipt")

    return {
        "report_type": EXPECTED_VALIDATION_REPORT_TYPE,
        "ok": not errors,
        "handoff_path": str(resolved_handoff),
        "validated_closure_receipt_summary_handoff_valid": not errors,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps validated closure receipt summary handoff."
    )
    parser.add_argument(
        "handoff",
        nargs="?",
        ...
        help="Path to phase66 validated closure receipt summary handoff JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            "PASS documentops local feature completion validated closure handoff "
            "signoff closure receipt validated handoff validated"
        )
        print(
            "validated_closure_receipt_summary_handoff_valid="
            f"{str(result['validated_closure_receipt_summary_handoff_valid']).lower()}"
        )
        ...
        print(
            "FAIL documentops local feature completion validated closure handoff "
            "signoff closure receipt validated handoff validation failed"
        )
