#!/usr/bin/env python3
"""Validate a human sign-off record for a no-cost ops lock handoff."""
...
EXPECTED_SCHEMA = "decisiondoc_report_quality_training_no_cost_ops_lock_handoff_signoff.v1"
...
REQUIRED_ACKS = (
    "ops_lock_handoff_reviewed",
    "ops_lock_summary_validated",
    "linked_ops_lock_files_checked",
...
def _load_handoff_validator():
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"failed to load no-cost ops lock handoff validator: {HANDOFF_VALIDATOR_PATH}")
...
def validate_training_no_cost_ops_lock_handoff_signoff(
    ...
    *,
    ...
    if not _non_empty_string(signoff_id):
        ...
        errors.append("signoff_id must match rqp_training_no_cost_ops_lock_handoff_signoff_[A-Za-z0-9_-]{8,96}")
    ...
    if _non_empty_string(handoff_path_value):
        ...
        if not handoff_path.exists():
            ...
            handoff_validation = _HANDOFF_VALIDATOR.validate_training_no_cost_ops_lock_handoff(handoff_path)
            if completed and handoff_validation.get("ok") is not True:
                errors.append("completed signoff requires valid no-cost ops lock handoff manifest")
    ...
    if completed:
        warnings.append(
            "completed no-cost ops lock handoff signoff records evidence review only; "
            "it does not authorize service resume, AWS cost, provider calls, training, or model promotion"
        )

    return {
        "report_type": "report_quality_training_no_cost_ops_lock_handoff_signoff_validation",
        "ok": not errors,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a no-cost ops lock handoff sign-off record.")
    parser.add_argument("signoff", type=Path, help="Path to no-cost ops lock handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = validate_training_no_cost_ops_lock_handoff_signoff(
            payload,
            require_complete=bool(args.require_complete),
        )
    ...
    if args.json:
        ...
        print("PASS report quality training no-cost ops lock handoff signoff validated")
        ...
        print("FAIL report quality training no-cost ops lock handoff signoff validation failed")
