#!/usr/bin/env python3
"""Summarize report quality training no-cost ops lock handoff sign-offs."""
...
EXPECTED_SCHEMA = "decisiondoc_report_quality_training_no_cost_ops_lock_handoff_signoff_summary.v1"
...
def _load_signoff_validator():
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"failed to load no-cost ops lock handoff signoff validator: {SIGNOFF_VALIDATOR_PATH}")
...
def summarize_no_cost_ops_lock_handoff_signoff(
    path: Path,
    payload: dict[str, Any],
    *,
    require_complete: bool = True,
) -> dict[str, Any]:
    validation = _SIGNOFF_VALIDATOR.validate_training_no_cost_ops_lock_handoff_signoff(
        payload,
    ...
    return {
        ...
        "acknowledgements": {
            "ops_lock_handoff_reviewed": acknowledgements.get("ops_lock_handoff_reviewed") is True,
            "ops_lock_summary_validated": acknowledgements.get("ops_lock_summary_validated") is True,
            "linked_ops_lock_files_checked": acknowledgements.get("linked_ops_lock_files_checked") is True,
...
def _confirms_service_lock(signoff: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and acknowledgements.get("ops_lock_handoff_reviewed") is True
        and acknowledgements.get("ops_lock_summary_validated") is True
        and acknowledgements.get("linked_ops_lock_files_checked") is True
...
def build_training_no_cost_ops_lock_handoff_signoff_summary(
    paths: Sequence[Path],
    *,
    ...
    for path in _expand_paths(paths):
        try:
            signoffs.append(
                summarize_no_cost_ops_lock_handoff_signoff(
                    path,
                    _load_json(path),
                    require_complete=require_complete,
                )
    ...
    service_lock_review_count = sum(1 for signoff in signoffs if _confirms_service_lock(signoff))
    invalid_count = len(signoffs) - valid_count
    blocker_reasons: list[str] = []
    if load_errors:
        blocker_reasons.append("ops_lock_handoff_signoff_load_errors")
    if not signoffs:
        blocker_reasons.append("no_ops_lock_handoff_signoffs")
    if invalid_count:
        blocker_reasons.append("invalid_ops_lock_handoff_signoffs")
    if completed_count != len(signoffs):
        blocker_reasons.append("ops_lock_handoff_signoff_not_completed_for_all_records")
    if accepted_count != len(signoffs):
        blocker_reasons.append("ops_lock_handoff_signoff_not_accepted_for_all_records")
    if service_lock_review_count != len(signoffs):
        blocker_reasons.append("ops_lock_handoff_signoff_does_not_confirm_service_lock_for_all_records")
    ok = not blocker_reasons
    return {
        "report_type": "report_quality_training_no_cost_ops_lock_handoff_signoff_summary",
        ...
        "ok": ok,
        "readiness": {
            "status": "all_ops_lock_handoff_signoffs_confirm_service_lock" if ok else "follow_up_required",
        ...
        "counts": {
            ...
            "service_lock_review_count": service_lock_review_count,
            "load_error_count": len(load_errors),
        },
        "signoffs": signoffs,
        "load_errors": load_errors,
        "side_effect_boundary": {
            "reads_local_ops_lock_handoff_signoffs": True,
...
def render_training_no_cost_ops_lock_handoff_signoff_summary_markdown(summary: dict[str, Any]) -> str:
    ...
    rows = "\n".join(
        "| {signoff_id} | {decision} | {completed} | {valid} | {service_lock} | {reviewer} | `{path}` |".format(
            ...
            service_lock=str(_confirms_service_lock(signoff)).lower(),
    ...
    return f"""# Report Quality Training No-Cost Ops Lock Handoff Sign-Off Summary
...
| signoff_id | decision | completed | valid | service_lock | reviewer | path |
| --- | --- | --- | --- | --- | --- | --- |
{rows}
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Summarize report quality training no-cost ops lock handoff sign-offs."
    )
    parser.add_argument(
        "signoffs",
        nargs="+",
        type=Path,
        help="No-cost ops lock handoff sign-off JSON file(s) or directories.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = build_training_no_cost_ops_lock_handoff_signoff_summary(
        args.signoffs,
    ...
    if args.markdown:
        _write_text_atomic(
            ...
            render_training_no_cost_ops_lock_handoff_signoff_summary_markdown(summary),
        )
    ...
    if args.json:
        ...
        print(
            "Report quality training no-cost ops lock handoff signoff summary: "
        ...
        print(f"service_lock_review_count={summary['counts']['service_lock_review_count']}")
