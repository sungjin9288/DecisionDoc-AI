#!/usr/bin/env python3
"""Validate the DocumentOps local feature completion closure index."""
...
from scripts.create_documentops_local_feature_completion_handoff_pending_signoff import (  # noqa: E402
    build_pending_documentops_local_feature_completion_handoff_signoff,
)
from scripts.summarize_documentops_local_feature_completion_handoff_signoffs import (  # noqa: E402
    build_documentops_local_feature_completion_handoff_signoff_summary,
)
...
from scripts.validate_documentops_local_feature_completion_handoff_signoff import (  # noqa: E402
    validate_documentops_local_feature_completion_handoff_signoff,
)
from scripts.validate_documentops_local_feature_completion_handoff_signoff_summary import (  # noqa: E402
    validate_documentops_local_feature_completion_handoff_signoff_summary,
)
...
EXPECTED_REPORT_TYPE = "document_ops_phase53_local_feature_completion_closure_index"
EXPECTED_VALIDATION_REPORT_TYPE = "document_ops_phase53_local_feature_completion_closure_index_validation"
EXPECTED_STATUS = "local_feature_completion_closure_index_ready_no_aws_no_training_authorization"
EXPECTED_ARTIFACTS = {
    "phase47_local_feature_completion": {
        "phase": 47,
        "path": "docs/specs/hermes_decisiondoc_agent/phase47_local_feature_completion/local_feature_completion.json",
        "validator": "scripts/validate_documentops_local_feature_completion.py",
    },
    "phase48_local_feature_completion_handoff": {
        "phase": 48,
        "path": "docs/specs/hermes_decisiondoc_agent/phase48_local_feature_completion_handoff/local_feature_completion_handoff.json",
        "validator": "scripts/validate_documentops_local_feature_completion_handoff.py",
    },
    "phase49_local_feature_completion_handoff_signoff_template": {
        "phase": 49,
        "path": (
            "docs/specs/hermes_decisiondoc_agent/"
    ...
    "phase50_pending_signoff_generation_contract": {
        "phase": 50,
        "path": (
            "docs/specs/hermes_decisiondoc_agent/"
            "phase50_local_feature_completion_pending_signoff/pending_signoff_generation_contract.json"
        ),
        "validator": "scripts/create_documentops_local_feature_completion_handoff_pending_signoff.py",
    },
    "phase51_signoff_summary_contract": {
        "phase": 51,
        "path": "docs/specs/hermes_decisiondoc_agent/phase51_local_feature_completion_signoff_summary/signoff_summary_contract.json",
        "validator": "scripts/summarize_documentops_local_feature_completion_handoff_signoffs.py",
    },
    "phase52_signoff_summary_validation_contract": {
        "phase": 52,
        "path": (
            "docs/specs/hermes_decisiondoc_agent/"
            "phase52_local_feature_completion_signoff_summary_validation/signoff_summary_validation_contract.json"
        ),
        "validator": "scripts/validate_documentops_local_feature_completion_handoff_signoff_summary.py",
    },
}
...
def _run_closure_probe() -> dict[str, Any]:
    phase47_result = validate_documentops_local_feature_completion()
    phase48_result = validate_documentops_local_feature_completion_handoff()
    template_path = _repo_path(EXPECTED_ARTIFACTS["phase49_local_feature_completion_handoff_signoff_template"]["path"])
    phase49_template_result = validate_documentops_local_feature_completion_handoff_signoff(_load_json(template_path))

    with tempfile.TemporaryDirectory(prefix="decisiondoc_phase53_closure_") as tmp_dir:
        tmp_path = Path(tmp_dir)
        pending_signoff = build_pending_documentops_local_feature_completion_handoff_signoff(
            signoff_id="documentops_local_feature_completion_handoff_signoff_phase53probe",
            created_at="2026-05-26T00:00:00+09:00",
        )
        pending_path = tmp_path / "phase53_pending_signoff.json"
        _write_json(pending_path, pending_signoff)
        phase50_pending_result = validate_documentops_local_feature_completion_handoff_signoff(pending_signoff)

        summary = build_documentops_local_feature_completion_handoff_signoff_summary(
            [pending_path],
            generated_at="2026-05-26T00:00:00+09:00",
        )
        summary_path = tmp_path / "phase53_pending_summary.json"
        _write_json(summary_path, summary)
        phase52_summary_result = validate_documentops_local_feature_completion_handoff_signoff_summary(summary_path)

    return {
        "phase47_local_feature_completion": phase47_result,
        "phase48_local_feature_completion_handoff": phase48_result,
        "phase49_signoff_template": phase49_template_result,
        "phase50_temporary_pending_signoff": phase50_pending_result,
        "phase51_temporary_pending_summary": {
        ...
        "phase52_temporary_summary_validation": phase52_summary_result,
    }


def validate_documentops_local_feature_completion_closure_index(
    ...
    if index.get("phase") != 53:
        errors.append("phase must be 53")
    if index.get("status") != EXPECTED_STATUS:
        errors.append(f"status must be {EXPECTED_STATUS}")
    if index.get("closure_scope") != "local_read_only_phase47_to_phase52_closure_validation":
        errors.append("closure_scope must be local_read_only_phase47_to_phase52_closure_validation")
    ...
    if closure_validator.get("path") != "scripts/validate_documentops_local_feature_completion_closure_index.py":
        errors.append("closure_validator.path must reference this validator")
    for field in (
        "checks_source_artifact_hashes",
        "runs_phase47_validator",
        "runs_phase48_validator",
        "validates_phase49_template",
        "generates_temporary_pending_signoff",
        "builds_temporary_phase51_summary",
        "runs_phase52_summary_validator",
    ):
    ...
    for field in (
        ...
        "phase53_is_not_service_resume_approval",
    ...
    for finding in _scan_forbidden_true(index):
        errors.append(f"documentops_closure_index: {finding}")
    ...
    phase51_probe = _as_dict(probe_results.get("phase51_temporary_pending_summary"))
    phase52_probe = _as_dict(probe_results.get("phase52_temporary_summary_validation"))
    if phase51_probe and phase51_probe.get("readiness_status") != "pending_signoff_review_no_training_authorization":
        errors.append("phase51_temporary_pending_summary.readiness_status must be pending non-approval evidence")
    if phase51_probe and phase51_probe.get("completion_ready") is not False:
        errors.append("phase51_temporary_pending_summary.completion_ready must be false")
    if phase52_probe and phase52_probe.get("completion_ready") is not False:
        errors.append("phase52_temporary_summary_validation.completion_ready must be false")
    ...
    return {
        ...
        "temporary_summary_readiness": phase51_probe.get("readiness_status", ""),
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate the DocumentOps local feature completion closure index.")
    parser.add_argument(
        "index",
        nargs="?",
        type=Path,
        default=DEFAULT_INDEX_PATH,
        help="Path to phase53 local_feature_completion_closure_index.json.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_local_feature_completion_closure_index(args.index)
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    elif result["ok"]:
        print("PASS documentops local feature completion closure index validated")
        ...
        print("FAIL documentops local feature completion closure index validation failed")
