#!/usr/bin/env python3
"""Validate a DocumentOps Phase 120 validated closure summary handoff sign-off record."""
...
EXPECTED_SCHEMA = (
    "decisiondoc_documentops_phase121_local_feature_completion_validated_closure_receipt_summary_"
    "handoff_signoff_closure_receipt_summary_handoff_signoff_closure_receipt_summary_handoff_signoff.v1"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase121_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_"
    "closure_receipt_summary_handoff_signoff_closure_receipt_summary_handoff_signoff_validation"
)
PHASE120_HANDOFF_PATH = (
...
SIGNOFF_ID_PREFIX = "documentops_local_feature_completion_phase120_validated_closure_receipt_summary_handoff_signoff_"
...
REQUIRED_ACKS = (
    "phase120_validated_closure_receipt_summary_handoff_reviewed",
    "phase118_summary_chain_reviewed",
    "phase119_summary_validation_reviewed",
...
PASS_MESSAGE = "PASS documentops phase121 validated closure receipt summary handoff sign-off validated"
FAIL_MESSAGE = "FAIL documentops phase121 validated closure receipt summary handoff sign-off validation failed"
...
def validate_documentops_phase120_validated_closure_receipt_summary_handoff_signoff(
    ...
    *,
    ...
    if payload.get("phase") != 121:
        errors.append("phase must be 121")
    expected_scope = "evidence_only_review_of_phase120_validated_closure_receipt_summary_handoff"
    ...
    if source_path_value != PHASE120_HANDOFF_PATH:
        errors.append("source_handoff.path must reference the Phase 120 validated handoff JSON")
        source_path = _repo_path(PHASE120_HANDOFF_PATH)
    else:
    ...
    if not source_path.exists():
        ...
        if source.get("sha256") != _sha256_file(source_path):
            errors.append("source_handoff.sha256 must match the Phase 120 validated handoff JSON")
        source_validation = (
            validate_documentops_phase119_validated_closure_receipt_summary_handoff_signoff_closure_receipt_summary_handoff_signoff_closure_receipt_summary_handoff(
                source_path
            )
        ...
        if source_validation.get("ok") is not True:
            errors.append("source_handoff must pass Phase 120 validation")
            for error in source_validation.get("errors", []):
                errors.append(f"source_handoff: {error}")

    validator_path = _repo_path(PHASE120_VALIDATOR_PATH)
    if source.get("validator") != PHASE120_VALIDATOR_PATH:
        errors.append(f"source_handoff.validator must be {PHASE120_VALIDATOR_PATH}")
    if not validator_path.exists():
        errors.append(f"source_handoff.validator path must exist: {PHASE120_VALIDATOR_PATH}")
    elif source.get("validator_sha256") != _sha256_file(validator_path):
        errors.append(f"source_handoff.validator_sha256 must match {PHASE120_VALIDATOR_PATH}")
    ...
    for finding in _scan_forbidden_true(payload):
        errors.append(f"documentops_phase120_validated_closure_receipt_summary_handoff_signoff: {finding}")

    if completed and not errors:
        warnings.append(
            "completed Phase 120 validated closure receipt summary handoff sign-off records evidence review only; "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 120 validated closure summary handoff sign-off record."
    )
    parser.add_argument("signoff", type=Path, help="Path to Phase 121 handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = validate_documentops_phase120_validated_closure_receipt_summary_handoff_signoff(
            payload,
            require_complete=bool(args.require_complete),
        )
