...
from scripts.post_deploy_reports import (  # noqa: E402
    persist_reports,
...
def _resolve_api_key(env_values: dict[str, str]) -> str:
    ...
    if legacy_key:
        return legacy_key
    raise SystemExit(
        "Missing runtime API key. Set DECISIONDOC_API_KEYS or DECISIONDOC_API_KEY in the env file."
    )
...
def _resolve_base_url(base_url: str, env_values: dict[str, str]) -> str:
    ...
    raise SystemExit(
        "Missing base URL. Pass --base-url or set ALLOWED_ORIGINS in the env file."
    )


def _append_step(
    report: dict[str, Any], *, name: str, status: str, **details: Any
) -> None:
...
def _extract_smoke_failure_details(*, stdout: str, stderr: str) -> dict[str, Any]:
    ...
    failure_line = next(
        (line for line in reversed(lines) if " expected " in line and " got " in line),
        "",
    )
...
def _extract_smoke_result_details(*, stdout: str, stderr: str) -> dict[str, Any]:
    ...
    seen: set[str] = set()
    for raw_line in "\n".join(
        part for part in (stdout, stderr) if str(part or "").strip()
    ).splitlines():
...
def _extract_report_workflow_smoke_result_details(
    *, stdout: str, stderr: str
    ...
    for raw_line in "\n".join(
        ...
        line = str(raw_line).strip()
        if not line.startswith("PASS ") and not line.startswith(
...
def _run_command_with_report(
    ...
    *,
    ...
    parsed_output = (
        output_parser(stdout=stdout, stderr=stderr) if output_parser is not None else {}
    )
    ...
    if completed.returncode != 0:
        ...
        parsed_details = (
            failure_parser(stdout=stdout, stderr=stderr)
        ...
        raise SystemExit(
            f"{label} failed with exit code {completed.returncode}{_build_failure_suffix(parsed_details)}"
        )
    _append_step(
        report,
        name=label,
...
def _fetch_health_json(base_url: str, *, timeout: float = 10.0) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"Health check returned invalid JSON for {health_url}"
        ) from exc
    if payload.get("status") != "ok":
        raise SystemExit(
            f"Health check returned non-ok status for {health_url}: {payload}"
        )
    return payload
...
def _validate_health_provider_routing(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    missing_routes = [
        ...
        if not str(provider_routes.get(key, "")).strip()
    ]
    if missing_routes:
        raise SystemExit(
            f"Health check missing provider_routes keys: {', '.join(missing_routes)}"
        )
    ...
    missing_route_checks = [
        key for key in REQUIRED_PROVIDER_ROUTE_KEYS if key not in provider_route_checks
    ]
    ...
    if invalid_route_checks:
        raise SystemExit(
            f"Health check has invalid provider_route_checks values: {invalid_route_checks}"
        )
    ...
    if missing_checks:
        raise SystemExit(
            f"Health check missing provider check keys: {', '.join(missing_checks)}"
        )
    invalid_checks = {
        key: checks.get(key)
    ...
    if invalid_checks:
        raise SystemExit(
            f"Health check has invalid provider check values: {invalid_checks}"
        )
    ...
    missing_policy_checks = [
        ...
        if key not in provider_policy_checks
    ]
    ...
    if invalid_policy_checks:
        raise SystemExit(
            f"Health check has invalid provider_policy_checks values: {invalid_policy_checks}"
        )
    ...
    missing_policy_issue_keys = [
        key
        for key in REQUIRED_PROVIDER_POLICY_KEYS
        if key not in provider_policy_issues
    ]
    ...
    if invalid_policy_issues:
        raise SystemExit(
            f"Health check has invalid provider_policy_issues values: {invalid_policy_issues}"
        )

    return {
        "provider_routes": {
            key: str(provider_routes[key]) for key in REQUIRED_PROVIDER_ROUTE_KEYS
        },
        "provider_route_checks": {
            key: str(provider_route_checks[key]) for key in REQUIRED_PROVIDER_ROUTE_KEYS
        },
        "checks": {key: str(checks[key]) for key in REQUIRED_PROVIDER_CHECK_KEYS},
        "provider_policy_checks": {
            key: str(provider_policy_checks[key])
            for key in REQUIRED_PROVIDER_POLICY_KEYS
        },
...
def run_post_deploy_check(
    *,
    ...
    resolved_report_file, latest_report_file = resolve_report_targets(
    ...
    try:
        ...
        _run_command_with_report(
            report, [*compose_prefix, "ps"], label="docker compose ps"
        )
