#!/usr/bin/env python3
"""Validate a DocumentOps Phase 217 validated closure receipt summary."""
...
from scripts.validate_documentops_phase215_validated_closure_receipt_summary_handoff_signoff_closure_receipt import (  # noqa: E402
    validate_documentops_phase215_validated_closure_receipt_summary_handoff_signoff_closure_receipt,
)


EXPECTED_REPORT_TYPE = (
    "document_ops_phase217_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_summary"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase218_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_summary_validation"
)
EXPECTED_SCHEMA = "decisiondoc_documentops_phase216_validated_closure_receipt_summary.v1"
READY_STATUS = "all_phase216_validated_closure_receipts_confirm_no_cost_freeze"
READS_LOCAL_RECEIPTS_KEY = "reads_local_phase216_validated_closure_receipts"
SUMMARY_VALID_KEY = "phase217_validated_closure_receipt_summary_valid"
PASS_MESSAGE = "PASS documentops phase217 validated closure receipt summary validated"
FAIL_MESSAGE = "FAIL documentops phase217 validated closure receipt summary validation failed"
FORBIDDEN_TRUE_KEYS = {
...
def _validate_summary_shape(summary: dict[str, Any], errors: list[str]) -> tuple[dict[str, Any], dict[str, Any], int]:
    ...
    if summary.get("phase") != 217:
        errors.append("phase must be 217")
...
def validate_documentops_phase217_validated_closure_receipt_summary(
    summary_path: Path,
) -> dict[str, Any]:
    ...
    for index, receipt in enumerate(receipts, start=1):
        ...
        if receipt.get("sha256") != _sha256_file(receipt_path):
            errors.append(f"{prefix}.sha256 must match linked Phase 216 receipt file")
            continue
        try:
        ...
        if validation.get("ok") is not (current_validation.get("ok") is True):
            errors.append(f"{prefix}.validation.ok must match current Phase 216 validation")
        if validation.get("closure_receipt_valid") is not (current_validation.get("closure_receipt_valid") is True):
            errors.append(f"{prefix}.validation.closure_receipt_valid must match current Phase 216 validation")
        if validation.get("source_closure_gate_valid") is not (
            current_validation.get("source_closure_gate_valid") is True
        ):
            errors.append(f"{prefix}.validation.source_closure_gate_valid must match current Phase 216 validation")
        ...
        expected_pairs = {
            ...
            "phase215_closure_index_sha256": source_hashes.get("phase215_closure_index_sha256", ""),
            "phase215_closure_validator_sha256": source_hashes.get("phase215_closure_validator_sha256", ""),
            "phase216_closure_receipt_validator_sha256": source_hashes.get(
                "phase216_closure_receipt_validator_sha256", ""
            ),
        }
        for field, expected_value in expected_pairs.items():
            if receipt.get(field) != expected_value:
                errors.append(f"{prefix}.{field} must match linked Phase 216 receipt")
    ...
    for finding in _scan_forbidden_true(summary):
        errors.append(f"documentops_phase217_validated_closure_receipt_summary: {finding}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a DocumentOps Phase 217 closure receipt summary.")
    parser.add_argument("summary", type=Path, help="Path to Phase 217 closure receipt summary JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase217_validated_closure_receipt_summary(args.summary)
    if args.json:
