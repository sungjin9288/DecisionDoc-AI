#!/usr/bin/env python3
"""Validate a DocumentOps Phase 93 validated closure summary handoff sign-off record."""
...
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase94_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_validation"
)
PHASE93_HANDOFF_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase93_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_"
    "closure_receipt_validated_handoff/"
    "validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_"
    "signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff.json"
)
PHASE93_VALIDATOR_PATH = "scripts/validate_documentops_phase92_validated_closure_receipt_summary_handoff.py"
SIGNOFF_ID_PREFIX = "documentops_local_feature_completion_phase93_validated_closure_receipt_summary_handoff_signoff_"
...
REQUIRED_ACKS = (
    "phase93_validated_closure_receipt_summary_handoff_reviewed",
    "phase91_summary_chain_reviewed",
    "phase92_summary_validation_reviewed",
...
def validate_documentops_phase93_validated_closure_receipt_summary_handoff_signoff(
    ...
    *,
    ...
    if payload.get("phase") != 94:
        errors.append("phase must be 94")
    if payload.get("signoff_scope") != "evidence_only_review_of_phase93_validated_closure_receipt_summary_handoff":
        errors.append("signoff_scope must be evidence_only_review_of_phase93_validated_closure_receipt_summary_handoff")
    ...
    if source_path_value != PHASE93_HANDOFF_PATH:
        errors.append("source_handoff.path must reference the Phase 93 validated handoff JSON")
        source_path = _repo_path(PHASE93_HANDOFF_PATH)
    else:
    ...
    if not source_path.exists():
        ...
        if source.get("sha256") != _sha256_file(source_path):
            errors.append("source_handoff.sha256 must match the Phase 93 validated handoff JSON")
        source_validation = validate_documentops_phase92_validated_closure_receipt_summary_handoff(source_path)
        if source_validation.get("ok") is not True:
            errors.append("source_handoff must pass Phase 93 validation")
            for error in source_validation.get("errors", []):
                errors.append(f"source_handoff: {error}")

    validator_path = _repo_path(PHASE93_VALIDATOR_PATH)
    if source.get("validator") != PHASE93_VALIDATOR_PATH:
        errors.append(f"source_handoff.validator must be {PHASE93_VALIDATOR_PATH}")
    if not validator_path.exists():
        errors.append(f"source_handoff.validator path must exist: {PHASE93_VALIDATOR_PATH}")
    elif source.get("validator_sha256") != _sha256_file(validator_path):
        errors.append(f"source_handoff.validator_sha256 must match {PHASE93_VALIDATOR_PATH}")
    ...
    for finding in _scan_forbidden_true(payload):
        errors.append(f"documentops_phase93_validated_closure_receipt_summary_handoff_signoff: {finding}")

    if completed and not errors:
        warnings.append(
            "completed Phase 93 validated closure receipt summary handoff sign-off records evidence review only; "
            "they do not authorize service resume, AWS cost, provider calls, training, or model promotion"
        )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 93 validated closure summary handoff sign-off record."
    )
    parser.add_argument("signoff", type=Path, help="Path to Phase 94 handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = validate_documentops_phase93_validated_closure_receipt_summary_handoff_signoff(
            payload,
            require_complete=bool(args.require_complete),
        )
