#!/usr/bin/env python3
"""Create a pending DocumentOps local feature completion handoff sign-off record."""
...
def _safe_signoff_id(value: str | None) -> str:
    signoff_id = (
        ...
        else f"documentops_local_feature_completion_handoff_signoff_{uuid4().hex}"
    )
...
def build_pending_documentops_local_feature_completion_handoff_signoff(
    *,
    ...
    signoff["generation_context"] = {
        "report_type": "document_ops_phase50_local_feature_completion_pending_signoff_generation",
        "generated_from_template": str(resolved_template),
        "generated_from_template_sha256": _sha256_file(resolved_template),
        "source_handoff_validation": validate_documentops_local_feature_completion_handoff_signoff(signoff),
    ...
    signoff["next_step_after_generation"] = (
        ...
        "validate_documentops_local_feature_completion_handoff_signoff.py --require-complete. "
        "This sign-off records local evidence review only and does not resume service operation, "
        "AWS runtime, provider calls, training, or model promotion."
    )

    validation = validate_documentops_local_feature_completion_handoff_signoff(signoff)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a pending DocumentOps local feature completion handoff sign-off JSON record."
    )
    ...
    parser.add_argument(
        "--signoff-id",
        help="Optional deterministic id matching documentops_local_feature_completion_handoff_signoff_[A-Za-z0-9_-]{8,96}.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        signoff = build_pending_documentops_local_feature_completion_handoff_signoff(
