#!/usr/bin/env python3
"""Create a pending DocumentOps Phase 202 validated handoff sign-off record."""
...
DEFAULT_TEMPLATE_PATH = (
    ...
    "phase202_local_feature_completion_validated_closure_receipt_summary_handoff_signoff/"
...
GENERATION_REPORT_TYPE = (
    "document_ops_phase203_local_feature_completion_validated_closure_receipt_summary_"
...
def build_pending_documentops_phase202_validated_closure_receipt_summary_handoff_signoff(
    *,
    ...
    source_template_validation = (
        validate_documentops_phase201_validated_closure_receipt_summary_handoff_signoff(template)
    )
    ...
    source_handoff = _as_dict(signoff.get("source_handoff"))
    source_handoff_path = (REPO_ROOT / PHASE201_HANDOFF_PATH).resolve()
    source_handoff_validator = (REPO_ROOT / PHASE201_VALIDATOR_PATH).resolve()
    ...
    signoff["next_step_after_generation"] = (
        ...
        "and acknowledgements, then runs the Phase 202 validator with --require-complete. "
    ...
    validation = validate_documentops_phase201_validated_closure_receipt_summary_handoff_signoff(signoff)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a pending DocumentOps Phase 202 validated handoff sign-off JSON record."
    )
    ...
    try:
        if not args.json and args.output is not None:
            ...
            if resolved_output.exists() and not args.overwrite:
                raise FileExistsError(f"output already exists: {resolved_output}")
        signoff = build_pending_documentops_phase202_validated_closure_receipt_summary_handoff_signoff(
