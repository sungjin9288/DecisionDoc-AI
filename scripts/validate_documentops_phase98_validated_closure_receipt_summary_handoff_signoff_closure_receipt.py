#!/usr/bin/env python3
"""Validate the DocumentOps Phase 99 validated handoff sign-off closure receipt."""
...
from scripts.validate_documentops_phase93_to_phase97_validated_closure_receipt_summary_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_phase93_to_phase97_validated_closure_receipt_summary_handoff_signoff_closure_index,
)
...
DEFAULT_RECEIPT_PATH = (
    ...
    "phase99_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt/"
    "validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt.json"
)
EXPECTED_REPORT_TYPE = (
    "document_ops_phase99_local_feature_completion_validated_closure_handoff_signoff_"
...
EXPECTED_STATUS = (
    ...
    "validated_handoff_signoff_closure_receipt_recorded_no_aws_no_training_authorization"
)
REQUIRED_SOURCE_HASH_PATHS = {
    "phase98_closure_index_sha256": (
        "docs/specs/hermes_decisiondoc_agent/"
    ...
    "phase98_closure_validator_sha256": (
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    ...
    "phase98_closure_index_valid",
    "phase93_to_phase97_chain_valid",
...
def validate_documentops_phase98_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 99:
        errors.append("phase must be 99")
    ...
    expected_command = (
        "python3 scripts/validate_documentops_phase93_to_phase97_validated_closure_receipt_summary_handoff_"
        "signoff_closure_index.py"
    )
    ...
    if source_gate.get("temporary_summary_readiness") != expected_readiness:
        errors.append(f"source_closure_gate.temporary_summary_readiness must be {expected_readiness}")
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_phase98_validated_closure_receipt_summary_handoff_signoff_closure_receipt: {finding}")

    gate_result: dict[str, Any] = {}
    if not errors:
        gate_result = validate_documentops_phase93_to_phase97_validated_closure_receipt_summary_handoff_signoff_closure_index()
    if not errors and gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 98 closure index validation")
        ...
        expected_pairs = {
            ...
            "probe_count": 5,
            "temporary_summary_readiness": expected_readiness,
        ...
        for field, expected_value in expected_pairs.items():
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 98 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 99 validated handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to Phase 99 validated handoff sign-off closure receipt JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase98_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
        ...
        print(
            ...
            "closure receipt validated handoff signoff closure receipt validated handoff signoff "
            "closure receipt validated handoff signoff closure receipt validated handoff signoff "
            "closure receipt validation failed"
        )
