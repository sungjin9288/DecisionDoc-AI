#!/usr/bin/env python3
"""Validate a DocumentOps validated closure receipt summary handoff sign-off summary."""
...
EXPECTED_REPORT_TYPE = (
    "document_ops_phase69_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_summary"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase70_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_summary_validation"
)
EXPECTED_SCHEMA = (
    "decisiondoc_documentops_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_summary.v1"
)
READY_STATUS = "all_validated_closure_receipt_summary_handoff_signoffs_accepted_no_cost_boundary_preserved"
PENDING_STATUS = "pending_validated_closure_receipt_summary_handoff_signoff_review_no_training_authorization"
FORBIDDEN_TRUE_KEYS = {
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_summary(
    summary_path: Path,
    *,
    ...
    for index, signoff in enumerate(signoffs, start=1):
        ...
        if validation.get("ok") is not (current_validation.get("ok") is True):
            errors.append(f"{prefix}.validation.ok must match current Phase 67 validation")
        if validation.get("completed") is not (current_validation.get("completed") is True):
            errors.append(f"{prefix}.validation.completed must match current Phase 67 validation")
        if validation.get("source_handoff_validation_ok") != current_validation.get("source_handoff_validation_ok"):
            errors.append(f"{prefix}.validation.source_handoff_validation_ok must match current Phase 67 validation")
    ...
    for finding in _scan_forbidden_true(summary):
        errors.append(f"documentops_validated_closure_receipt_summary_handoff_signoff_summary: {finding}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps validated closure receipt summary handoff sign-off summary."
    )
    parser.add_argument("summary", type=Path, help="Path to Phase 69 sign-off summary JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_summary(
        args.summary,
    ...
    if args.json:
        ...
        print(
            ...
            "closure receipt validated handoff signoff summary validated"
        )
        ...
        print(
            ...
            "closure receipt validated handoff signoff summary validation failed"
        )
