#!/usr/bin/env python3
"""Validate a DocumentOps local feature completion validated closure handoff sign-off record."""
...
EXPECTED_SCHEMA = "decisiondoc_documentops_local_feature_completion_validated_closure_handoff_signoff.v1"
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase58_local_feature_completion_validated_closure_handoff_signoff_validation"
)
PHASE57_HANDOFF_PATH = (
...
PHASE57_VALIDATOR_PATH = "scripts/validate_documentops_local_feature_completion_validated_closure_handoff.py"
SIGNOFF_ID_PATTERN = re.compile(
    r"documentops_local_feature_completion_validated_closure_handoff_signoff_[A-Za-z0-9_-]{8,96}"
)
VALID_DECISIONS = {"pending", "accepted", "changes_requested", "rejected"}
COMPLETED_DECISIONS = {"accepted", "changes_requested", "rejected"}
REQUIRED_ACKS = (
    "phase57_validated_closure_handoff_reviewed",
    "phase55_summary_chain_reviewed",
    "phase56_summary_validation_reviewed",
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff(
    ...
    *,
    ...
    if payload.get("phase") != 58:
        errors.append("phase must be 58")
    if payload.get("signoff_scope") != "evidence_only_review_of_phase57_validated_closure_handoff":
        errors.append("signoff_scope must be evidence_only_review_of_phase57_validated_closure_handoff")
    ...
    if not _non_empty_string(signoff_id):
        ...
        errors.append(
            ...
            "documentops_local_feature_completion_validated_closure_handoff_signoff_[A-Za-z0-9_-]{8,96}"
        )
    ...
    if source_path_value != PHASE57_HANDOFF_PATH:
        errors.append("source_handoff.path must reference the Phase 57 validated closure handoff JSON")
        source_path = _repo_path(PHASE57_HANDOFF_PATH)
    else:
    ...
    if not source_path.exists():
        ...
        if source.get("sha256") != _sha256_file(source_path):
            errors.append("source_handoff.sha256 must match the Phase 57 validated closure handoff JSON")
        source_validation = validate_documentops_local_feature_completion_validated_closure_handoff(source_path)
        if source_validation.get("ok") is not True:
            errors.append("source_handoff must pass Phase 57 validation")
            for error in source_validation.get("errors", []):
                errors.append(f"source_handoff: {error}")

    validator_path = _repo_path(PHASE57_VALIDATOR_PATH)
    if source.get("validator") != PHASE57_VALIDATOR_PATH:
        errors.append(f"source_handoff.validator must be {PHASE57_VALIDATOR_PATH}")
    if not validator_path.exists():
        errors.append(f"source_handoff.validator path must exist: {PHASE57_VALIDATOR_PATH}")
    elif source.get("validator_sha256") != _sha256_file(validator_path):
        errors.append(f"source_handoff.validator_sha256 must match {PHASE57_VALIDATOR_PATH}")
    ...
    for finding in _scan_forbidden_true(payload):
        errors.append(f"documentops_validated_closure_handoff_signoff: {finding}")

    if completed and not errors:
        warnings.append(
            "completed validated closure handoff sign-off records evidence review only; "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps local feature completion validated closure handoff sign-off record."
    )
    parser.add_argument("signoff", type=Path, help="Path to Phase 58 handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = validate_documentops_local_feature_completion_validated_closure_handoff_signoff(
            payload,
            require_complete=bool(args.require_complete),
        )
    ...
    if args.json:
        ...
        print("PASS documentops local feature completion validated closure handoff sign-off validated")
        ...
        print("FAIL documentops local feature completion validated closure handoff sign-off validation failed")
