#!/usr/bin/env python3
"""Summarize DocumentOps Phase 171 validated closure receipt records."""
...
def build_documentops_phase171_validated_closure_receipt_summary(
    paths: Sequence[Path],
    *,
    ...
    if load_errors:
        blocker_reasons.append("phase171_validated_closure_receipt_load_errors")
    if not receipts:
        blocker_reasons.append("no_phase171_validated_closure_receipts")
    if invalid_count:
        blocker_reasons.append("invalid_phase171_validated_closure_receipts")
    if boundary_break_count:
        blocker_reasons.append("phase171_validated_closure_receipt_boundary_breaks")

    status = (
        "all_phase171_validated_closure_receipts_confirm_no_cost_freeze"
    ...
    return {
        ...
        "side_effect_boundary": {
            "reads_local_phase171_validated_closure_receipts": True,
...
def _render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# DocumentOps Phase 171 Validated Closure Receipt Summary",
        "",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Summarize DocumentOps Phase 171 validated closure receipt JSON records."
    )
    parser.add_argument(
        "receipts",
        nargs="*",
        ...
        help="Closure receipt JSON file(s) or directories. Defaults to the Phase 171 receipt.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = build_documentops_phase171_validated_closure_receipt_summary(
        args.receipts,
