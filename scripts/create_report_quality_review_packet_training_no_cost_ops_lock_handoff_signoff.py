#!/usr/bin/env python3
"""Create a pending human sign-off record for a no-cost ops lock handoff."""
...
DEFAULT_TEMPLATE_PATH = REPO_ROOT / "docs/specs/report_quality_learning/training_no_cost_ops_lock_handoff_signoff_template.json"
...
SIGNOFF_ID_PATTERN = re.compile(r"rqp_training_no_cost_ops_lock_handoff_signoff_[A-Za-z0-9_-]{8,96}")
BOUNDARY_FALSE_KEYS = (
...
def _safe_signoff_id(value: str | None) -> str:
    signoff_id = (
        ...
        else f"rqp_training_no_cost_ops_lock_handoff_signoff_{uuid4().hex}"
    )
    if not SIGNOFF_ID_PATTERN.fullmatch(signoff_id):
        raise ValueError("signoff id must match rqp_training_no_cost_ops_lock_handoff_signoff_[A-Za-z0-9_-]{8,96}")
    return signoff_id


def _default_output_path(handoff_manifest_path: Path) -> Path:
    name = handoff_manifest_path.name
    if name.endswith("-training-no-cost-ops-lock-handoff-manifest.json"):
        base = name.removesuffix("-training-no-cost-ops-lock-handoff-manifest.json")
    else:
        base = handoff_manifest_path.stem
    return handoff_manifest_path.with_name(f"{base}-training-no-cost-ops-lock-handoff-signoff.json")
...
def _evidence_to_review(handoff_manifest_path: Path, handoff_manifest: dict[str, Any]) -> list[str]:
    ...
    _append_unique_path(paths, handoff_manifest.get("ops_lock_summary_path"))
...
def build_pending_training_no_cost_ops_lock_handoff_signoff(
    *,
    ...
    handoff_validation = _HANDOFF_VALIDATOR.validate_training_no_cost_ops_lock_handoff(
        resolved_handoff_manifest,
        require_ready=require_ready,
    )
    if require_ready and handoff_validation.get("ok") is not True:
        errors = "; ".join(handoff_validation.get("errors") or ["no-cost ops lock handoff validation failed"])
        raise ValueError(f"no-cost ops lock handoff is not ready for sign-off generation: {errors}")
    ...
    signoff.update(
        {
            ...
            "generation_context": {
                "report_type": "report_quality_training_no_cost_ops_lock_handoff_pending_signoff_generation",
            ...
            "next_step_after_generation": (
                ...
                "validate_report_quality_review_packet_training_no_cost_ops_lock_handoff_signoff.py "
                "--require-complete. This sign-off records ops lock handoff review only and does not "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create a pending no-cost ops lock handoff sign-off JSON record.")
    parser.add_argument(
        "handoff_manifest",
        type=Path,
        help="Path to *-training-no-cost-ops-lock-handoff-manifest.json.",
    )
    parser.add_argument("--template", type=Path, default=DEFAULT_TEMPLATE_PATH)
    parser.add_argument("--output", type=Path, default=None)
    parser.add_argument(
        "--signoff-id",
        help="Optional deterministic id matching rqp_training_no_cost_ops_lock_handoff_signoff_[A-Za-z0-9_-]{8,96}.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        signoff = build_pending_training_no_cost_ops_lock_handoff_signoff(
        ...
        if args.json:
            ...
            print("FAIL report quality training no-cost ops lock handoff pending signoff generation failed")
            print(f"ERROR {exc}")
        return 1
    ...
    if args.json:
        ...
        print("Report quality training no-cost ops lock handoff pending signoff: PASS")
