...
def _describe_same_sha_evidence(runs: list[dict[str, Any]], *, head_sha: str) -> str:
    same_sha_dev = _same_sha_run_url(runs, stage="dev", head_sha=head_sha)
    if same_sha_dev:
        run_id = same_sha_dev.rstrip("/").split("/")[-1]
        return f"ready — deploy-smoke [dev] run {run_id} succeeded on {head_sha}"
    return f"missing — run deploy-smoke [dev] on {head_sha} first"


def _derive_defaults(args: argparse.Namespace) -> dict[str, str]:
    ...
    return {
        ...
        "openai_fallback": _placeholder(args.openai_fallback, "yes / no"),
...
def _render_plan(
    ...
    *,
    ...
    defaults = _derive_defaults(args)
    owner = _placeholder(args.owner, "<OWNER_NAME>")
    same_sha_evidence = _describe_same_sha_evidence(runs, head_sha=head_sha)
...
| Same-SHA `deploy-smoke` evidence for current `main` | `{same_sha_evidence}` |
