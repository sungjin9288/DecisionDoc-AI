#!/usr/bin/env python3
"""Validate a DocumentOps Phase 75 validated handoff sign-off record."""
...
EXPECTED_SCHEMA = (
    "decisiondoc_documentops_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff.v1"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase76_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_validation"
)
PHASE75_HANDOFF_PATH = (
...
SIGNOFF_ID_PATTERN = re.compile(
    r"documentops_local_feature_completion_validated_closure_receipt_validated_handoff_summary_handoff_signoff_[A-Za-z0-9_-]{8,96}"
)
VALID_DECISIONS = {"pending", "accepted", "changes_requested", "rejected"}
COMPLETED_DECISIONS = {"accepted", "changes_requested", "rejected"}
REQUIRED_ACKS = (
    "phase75_validated_closure_receipt_validated_handoff_summary_handoff_reviewed",
    "phase73_summary_chain_reviewed",
    "phase74_summary_validation_reviewed",
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff(
    ...
    *,
    ...
    if payload.get("phase") != 76:
        errors.append("phase must be 76")
    if payload.get("signoff_scope") != (
        "evidence_only_review_of_phase75_validated_closure_receipt_validated_handoff_summary_handoff"
    ):
        errors.append(
            "signoff_scope must be "
            "evidence_only_review_of_phase75_validated_closure_receipt_validated_handoff_summary_handoff"
        )
    ...
    if not _non_empty_string(signoff_id):
        ...
        errors.append(
            ...
            "documentops_local_feature_completion_validated_closure_receipt_validated_handoff_summary_handoff_signoff_"
            "[A-Za-z0-9_-]{8,96}"
        )
    ...
    if source_path_value != PHASE75_HANDOFF_PATH:
        errors.append("source_handoff.path must reference the Phase 75 validated handoff JSON")
        source_path = _repo_path(PHASE75_HANDOFF_PATH)
    else:
    ...
    if not source_path.exists():
        ...
        if source.get("sha256") != _sha256_file(source_path):
            errors.append("source_handoff.sha256 must match the Phase 75 validated handoff JSON")
        source_validation = (
            validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff(
                source_path
            )
        ...
        if source_validation.get("ok") is not True:
            errors.append("source_handoff must pass Phase 75 validation")
            for error in source_validation.get("errors", []):
                errors.append(f"source_handoff: {error}")

    validator_path = _repo_path(PHASE75_VALIDATOR_PATH)
    if source.get("validator") != PHASE75_VALIDATOR_PATH:
        errors.append(f"source_handoff.validator must be {PHASE75_VALIDATOR_PATH}")
    if not validator_path.exists():
        errors.append(f"source_handoff.validator path must exist: {PHASE75_VALIDATOR_PATH}")
    elif source.get("validator_sha256") != _sha256_file(validator_path):
        errors.append(f"source_handoff.validator_sha256 must match {PHASE75_VALIDATOR_PATH}")
    ...
    for finding in _scan_forbidden_true(payload):
        errors.append(f"documentops_validated_closure_receipt_validated_handoff_summary_handoff_signoff: {finding}")

    if completed and not errors:
        warnings.append(
            "completed validated closure receipt validated handoff summary handoff sign-off records evidence "
            "review only; they do not authorize service resume, AWS cost, provider calls, training, or model "
            "promotion"
        )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 75 validated handoff sign-off record."
    )
    parser.add_argument("signoff", type=Path, help="Path to Phase 76 handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            ...
            "closure receipt validated handoff signoff closure receipt validated handoff sign-off validated"
        )
        ...
        print(
            ...
            "closure receipt validated handoff signoff closure receipt validated handoff sign-off validation failed"
        )
