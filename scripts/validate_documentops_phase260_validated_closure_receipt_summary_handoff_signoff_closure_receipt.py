#!/usr/bin/env python3
"""Validate the DocumentOps Phase 261 validated handoff sign-off closure receipt."""
...
from scripts.validate_documentops_phase255_to_phase259_validated_closure_receipt_summary_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_phase255_to_phase259_validated_closure_receipt_summary_handoff_signoff_closure_index,
)
...
DEFAULT_RECEIPT_PATH = (
    ...
    "phase261_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt/"
    "validated_closure_receipt_summary_handoff_signoff_closure_receipt.json"
)
EXPECTED_REPORT_TYPE = (
    "document_ops_phase261_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase261_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_validation"
)
EXPECTED_STATUS = (
...
REQUIRED_SOURCE_HASH_PATHS = {
    ...
    "phase260_closure_validator_sha256": PHASE260_VALIDATOR,
    "phase261_closure_receipt_validator_sha256": (
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    ...
    "phase260_closure_index_valid",
    "phase255_to_phase259_chain_valid",
...
EXPECTED_SOURCE_COMMAND = f"python3 {PHASE260_VALIDATOR}"
...
PASS_MESSAGE = (
    "PASS documentops phase261 validated closure receipt summary handoff sign-off closure receipt validated"
)
FAIL_MESSAGE = (
    "FAIL documentops phase261 validated closure receipt summary handoff sign-off closure receipt validation failed"
)
...
def validate_documentops_phase260_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 261:
        errors.append("phase must be 261")
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_phase261_validated_closure_receipt: {finding}")
    ...
    if not errors and gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 260 closure index validation")
        ...
        for field, expected_value in expected_pairs.items():
            ...
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 260 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 261 validated handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to Phase 261 validated handoff sign-off closure receipt JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase260_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
