#!/usr/bin/env python3
"""Validate the DocumentOps Phase 144 validated handoff sign-off closure receipt."""
...
from scripts.validate_documentops_phase138_to_phase142_validated_closure_receipt_summary_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_phase138_to_phase142_validated_closure_receipt_summary_handoff_signoff_closure_index,
)
...
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase144_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_validation"
)
EXPECTED_STATUS = (
...
REQUIRED_SOURCE_HASH_PATHS = {
    ...
    "phase143_closure_validator_sha256": (
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    ...
    "phase143_closure_index_valid",
    "phase138_to_phase142_chain_valid",
...
PASS_MESSAGE = "PASS documentops phase144 validated closure receipt summary handoff sign-off closure receipt validated"
FAIL_MESSAGE = (
    "FAIL documentops phase144 validated closure receipt summary handoff sign-off closure receipt validation failed"
)
...
def validate_documentops_phase143_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 144:
        errors.append("phase must be 144")
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_phase144_validated_closure_receipt: {finding}")

    gate_result: dict[str, Any] = {}
    if not errors:
        gate_result = validate_documentops_phase138_to_phase142_validated_closure_receipt_summary_handoff_signoff_closure_index()
    if not errors and gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 143 closure index validation")
        ...
        for field, expected_value in expected_pairs.items():
            ...
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 143 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 144 validated handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to Phase 144 validated handoff sign-off closure receipt JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase143_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
