#!/usr/bin/env python3
"""Validate a DocumentOps Phase 231 validated handoff sign-off summary."""
...
EXPECTED_REPORT_TYPE = (
    "document_ops_phase231_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_summary"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase232_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_summary_validation"
)
EXPECTED_SCHEMA = "decisiondoc_documentops_phase229_validated_closure_receipt_summary_handoff_signoff_summary.v1"
READY_STATUS = "all_phase228_validated_closure_receipt_summary_handoff_signoffs_accepted_no_cost_boundary_preserved"
PENDING_STATUS = "pending_phase228_validated_closure_receipt_summary_handoff_signoff_review_no_training_authorization"
FORBIDDEN_TRUE_KEYS = {
...
PASS_MESSAGE = "PASS documentops phase232 validated closure receipt summary handoff sign-off summary validated"
FAIL_MESSAGE = "FAIL documentops phase232 validated closure receipt summary handoff sign-off summary validation failed"
...
def validate_documentops_phase231_validated_closure_receipt_summary_handoff_signoff_summary(
    summary_path: Path,
    *,
    ...
    for index, signoff in enumerate(signoffs, start=1):
        ...
        if signoff.get("sha256") != _sha256_file(signoff_path):
            errors.append(f"{prefix}.sha256 must match linked Phase 229 sign-off file")
        try:
            signoff_payload = _load_json(signoff_path)
        except (OSError, ValueError, json.JSONDecodeError) as exc:
            errors.append(f"{prefix}.path could not be loaded: {exc}")
            continue
        current_validation = validate_documentops_phase228_validated_closure_receipt_summary_handoff_signoff(
            signoff_payload,
            require_complete=effective_require_complete,
        )
        if validation.get("ok") is not (current_validation.get("ok") is True):
            errors.append(f"{prefix}.validation.ok must match current Phase 229 validation")
        if validation.get("completed") is not (current_validation.get("completed") is True):
            errors.append(f"{prefix}.validation.completed must match current Phase 229 validation")
        if validation.get("source_handoff_validation_ok") != current_validation.get("source_handoff_validation_ok"):
            errors.append(f"{prefix}.validation.source_handoff_validation_ok must match current Phase 229 validation")
    ...
    for finding in _scan_forbidden_true(summary):
        errors.append(f"documentops_phase231_validated_closure_receipt_summary_handoff_signoff_summary: {finding}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 231 validated handoff sign-off summary."
    )
    parser.add_argument("summary", type=Path, help="Path to Phase 231 sign-off summary JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase231_validated_closure_receipt_summary_handoff_signoff_summary(
        args.summary,
