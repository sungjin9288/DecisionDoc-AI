#!/usr/bin/env python3
"""Validate the DocumentOps Phase 396 validated handoff sign-off closure receipt."""
...
from scripts.validate_documentops_phase390_to_phase394_validated_closure_receipt_summary_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_phase390_to_phase394_validated_closure_receipt_summary_handoff_signoff_closure_index,
)
...
DEFAULT_RECEIPT_PATH = (
    ...
    "phase396_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt/"
    "validated_closure_receipt_summary_handoff_signoff_closure_receipt.json"
)
EXPECTED_REPORT_TYPE = (
    "document_ops_phase396_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase396_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_validation"
)
EXPECTED_STATUS = (
...
REQUIRED_SOURCE_HASH_PATHS = {
    ...
    "phase395_closure_validator_sha256": PHASE395_VALIDATOR,
    "phase396_closure_receipt_validator_sha256": (
        "scripts/validate_documentops_phase395_validated_closure_receipt_summary_handoff_signoff_closure_receipt.py"
    ),
}
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    "local_receipt_recorded",
    "phase395_closure_index_valid",
    "phase390_to_phase394_chain_valid",
...
EXPECTED_SOURCE_COMMAND = f"python3 {PHASE395_VALIDATOR}"
...
PASS_MESSAGE = (
    "PASS documentops phase396 validated closure receipt summary handoff sign-off closure receipt validated"
)
FAIL_MESSAGE = (
    "FAIL documentops phase396 validated closure receipt summary handoff sign-off closure receipt validation failed"
)
...
def validate_documentops_phase395_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 396:
        errors.append("phase must be 396")
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_phase396_validated_closure_receipt: {finding}")
    ...
    if not errors and gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 395 closure index validation")
        ...
        for field, expected_value in expected_pairs.items():
            ...
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 395 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 396 validated handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to Phase 396 validated handoff sign-off closure receipt JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase395_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
