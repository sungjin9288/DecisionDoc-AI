#!/usr/bin/env python3
"""Create a pending DocumentOps Phase 76 validated handoff sign-off record."""
...
def _safe_signoff_id(value: str | None) -> str:
    signoff_id = (
        ...
        else f"documentops_local_feature_completion_validated_closure_receipt_validated_handoff_summary_handoff_signoff_{uuid4().hex}"
    )
    if not SIGNOFF_ID_PATTERN.fullmatch(signoff_id):
        raise ValueError(
            "signoff id must match "
...
def build_pending_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff(
    *,
    ...
    signoff["signoff_id"] = _safe_signoff_id(signoff_id)
    signoff["created_at"] = created_at or _now_iso()
    signoff["generation_context"] = {
        "report_type": (
            "document_ops_phase77_local_feature_completion_validated_closure_handoff_signoff_"
            "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_"
            "pending_signoff_generation"
        ),
    ...
    signoff["next_step_after_generation"] = (
        ...
        "--require-complete. This sign-off records local evidence review only and does not "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a pending DocumentOps Phase 76 validated handoff sign-off JSON record."
    )
    ...
    parser.add_argument(
        "--signoff-id",
        help=(
            "Optional deterministic id matching "
            "documentops_local_feature_completion_validated_closure_receipt_validated_handoff_summary_handoff_signoff_"
            "[A-Za-z0-9_-]{8,96}."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        signoff = build_pending_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff(
