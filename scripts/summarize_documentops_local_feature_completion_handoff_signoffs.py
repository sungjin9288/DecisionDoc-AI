#!/usr/bin/env python3
"""Summarize DocumentOps local feature completion handoff sign-off records."""
...
def _forbidden_true_findings(*boundaries: dict[str, Any]) -> list[str]:
    findings: list[str] = []
...
def summarize_signoff(path: Path, payload: dict[str, Any], *, require_complete: bool) -> dict[str, Any]:
    validation = validate_documentops_local_feature_completion_handoff_signoff(
        payload,
...
def build_documentops_local_feature_completion_handoff_signoff_summary(
    paths: Sequence[Path],
    *,
    ...
    if blocker_reasons:
        ...
        status = "all_signoffs_accepted_no_cost_boundary_preserved"
    else:
        status = "pending_signoff_review_no_training_authorization"

    return {
        "report_type": "document_ops_phase51_local_feature_completion_handoff_signoff_summary",
...
def _render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# DocumentOps Local Feature Completion Handoff Sign-Off Summary",
        "",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Summarize DocumentOps local feature completion handoff sign-off JSON records."
    )
