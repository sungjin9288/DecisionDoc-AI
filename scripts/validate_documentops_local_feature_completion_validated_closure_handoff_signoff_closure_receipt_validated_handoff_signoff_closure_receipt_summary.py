#!/usr/bin/env python3
"""Validate a DocumentOps Phase 73 validated closure receipt summary."""
...
from scripts.validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt import (  # noqa: E402
...
EXPECTED_REPORT_TYPE = (
    "document_ops_phase73_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_summary"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase74_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_summary_validation"
)
EXPECTED_SCHEMA = (
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_summary(
    summary_path: Path,
) -> dict[str, Any]:
    ...
    if summary.get("phase") != 73:
        errors.append("phase must be 73")
    ...
    for index, receipt in enumerate(receipts, start=1):
        ...
        if receipt.get("sha256") != _sha256_file(receipt_path):
            errors.append(f"{prefix}.sha256 must match linked Phase 72 receipt file")
        try:
        ...
        if validation.get("ok") is not (current_validation.get("ok") is True):
            errors.append(f"{prefix}.validation.ok must match current Phase 72 validation")
        if validation.get("closure_receipt_valid") is not (
            current_validation.get("closure_receipt_valid") is True
        ):
            errors.append(f"{prefix}.validation.closure_receipt_valid must match current Phase 72 validation")
        if validation.get("source_closure_gate_valid") is not (
            current_validation.get("source_closure_gate_valid") is True
        ):
            errors.append(f"{prefix}.validation.source_closure_gate_valid must match current Phase 72 validation")
        ...
        expected_pairs = {
            ...
            "phase71_closure_index_sha256": source_hashes.get("phase71_closure_index_sha256", ""),
            "phase71_closure_validator_sha256": source_hashes.get("phase71_closure_validator_sha256", ""),
        }
        for field, expected_value in expected_pairs.items():
            if receipt.get(field) != expected_value:
                errors.append(f"{prefix}.{field} must match linked Phase 72 receipt")
    ...
    if (
        ...
        errors.append(
            "side_effect_boundary.reads_local_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipts must be true"
        )
    ...
    for finding in _scan_forbidden_true(summary):
        errors.append(
            ...
            f"signoff_closure_receipt_summary: {finding}"
        )
    ...
    return {
        ...
        "ok": not errors,
        "summary_path": str(resolved_summary),
        "closure_receipt_validated_handoff_signoff_closure_receipt_summary_valid": not errors,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 73 validated closure receipt summary."
    )
    parser.add_argument("summary", type=Path, help="Path to Phase 73 closure receipt summary JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            ...
            "closure receipt validated handoff signoff closure receipt summary validated"
        )
        print(
            "closure_receipt_validated_handoff_signoff_closure_receipt_summary_valid="
            f"{str(result['closure_receipt_validated_handoff_signoff_closure_receipt_summary_valid']).lower()}"
        )
        ...
        print(
            ...
            "closure receipt validated handoff signoff closure receipt summary validation failed"
        )
