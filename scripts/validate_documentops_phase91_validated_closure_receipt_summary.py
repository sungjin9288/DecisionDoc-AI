#!/usr/bin/env python3
"""Validate a DocumentOps Phase 91 validated closure receipt summary."""
...
EXPECTED_REPORT_TYPE = f"document_ops_phase91_local_feature_completion_{SUMMARY_NAME}_summary"
EXPECTED_VALIDATION_REPORT_TYPE = (
    f"document_ops_phase92_local_feature_completion_{SUMMARY_NAME}_summary_validation"
)
EXPECTED_SCHEMA = f"decisiondoc_documentops_local_feature_completion_{SUMMARY_NAME}_summary.v1"
READY_STATUS = f"all_{SUMMARY_NAME}s_confirm_no_cost_freeze"
READS_LOCAL_RECEIPTS_KEY = f"reads_local_{SUMMARY_NAME}s"
SUMMARY_VALID_KEY = (
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_"
    "closure_receipt_summary_valid"
)
PASS_MESSAGE = (
...
def _validate_summary_shape(summary: dict[str, Any], errors: list[str]) -> tuple[dict[str, Any], dict[str, Any], int]:
    ...
    if summary.get("phase") != 91:
        errors.append("phase must be 91")
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_summary(
    summary_path: Path,
) -> dict[str, Any]:
    ...
    should_run_current_receipt_validation = not errors
    for index, receipt in enumerate(receipts, start=1):
        ...
        if receipt.get("sha256") != _sha256_file(receipt_path):
            errors.append(f"{prefix}.sha256 must match linked Phase 90 receipt file")
            continue
        try:
        ...
        if should_run_current_receipt_validation and not errors:
            current_validation = (
                validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt(
                    receipt_path
                )
            ...
            if validation.get("ok") is not (current_validation.get("ok") is True):
                errors.append(f"{prefix}.validation.ok must match current Phase 90 validation")
            if validation.get("closure_receipt_valid") is not (
                current_validation.get("closure_receipt_valid") is True
            ):
                errors.append(f"{prefix}.validation.closure_receipt_valid must match current Phase 90 validation")
            if validation.get("source_closure_gate_valid") is not (
                current_validation.get("source_closure_gate_valid") is True
            ):
                errors.append(f"{prefix}.validation.source_closure_gate_valid must match current Phase 90 validation")
        ...
        for field, expected_value in expected_pairs.items():
            if receipt.get(field) != expected_value:
                errors.append(f"{prefix}.{field} must match linked Phase 90 receipt")
    ...
    for finding in _scan_forbidden_true(summary):
        errors.append(f"documentops_{SUMMARY_NAME}_summary: {finding}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 91 validated closure receipt summary."
    )
    parser.add_argument("summary", type=Path, help="Path to Phase 91 closure receipt summary JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = (
        validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_summary(
            args.summary
        )
    )
    if args.json:
