#!/usr/bin/env python3
"""Validate a DocumentOps Phase 183 validated closure summary handoff sign-off record."""
...
EXPECTED_SCHEMA = "decisiondoc_documentops_phase184_validated_closure_receipt_summary_handoff_signoff.v1"
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase184_local_feature_completion_validated_closure_receipt_summary_handoff_"
    "signoff_validation"
)
PHASE183_HANDOFF_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase183_local_feature_completion_validated_closure_receipt_summary_handoff/"
...
SIGNOFF_ID_PREFIX = "documentops_local_feature_completion_phase183_validated_closure_receipt_summary_handoff_signoff_"
...
REQUIRED_ACKS = (
    "phase183_validated_closure_receipt_summary_handoff_reviewed",
    "phase181_summary_chain_reviewed",
    "phase182_summary_validation_reviewed",
...
PASS_MESSAGE = "PASS documentops phase184 validated closure receipt summary handoff sign-off validated"
FAIL_MESSAGE = "FAIL documentops phase184 validated closure receipt summary handoff sign-off validation failed"
...
def _load_phase183_validator() -> Callable[[Path], dict[str, Any]]:
    ...
    validator = getattr(module, PHASE183_VALIDATOR_FUNCTION)
    return validator
...
def validate_documentops_phase183_validated_closure_receipt_summary_handoff_signoff(
    ...
    *,
    ...
    if payload.get("phase") != 184:
        errors.append("phase must be 184")
    expected_scope = "evidence_only_review_of_phase183_validated_closure_receipt_summary_handoff"
    ...
    if source_path_value != PHASE183_HANDOFF_PATH:
        errors.append("source_handoff.path must reference the Phase 183 validated handoff JSON")
        source_path = _repo_path(PHASE183_HANDOFF_PATH)
    else:
    ...
    if not source_path.exists():
        ...
        if source.get("sha256") != _sha256_file(source_path):
            errors.append("source_handoff.sha256 must match the Phase 183 validated handoff JSON")
        try:
            phase183_validator = _load_phase183_validator()
            source_validation = phase183_validator(source_path)
        except (AttributeError, OSError, RuntimeError) as exc:
            errors.append(f"source_handoff validation could not load Phase 183 validator: {exc}")
        if source_validation and source_validation.get("ok") is not True:
            errors.append("source_handoff must pass Phase 183 validation")
            for error in source_validation.get("errors", []):
                errors.append(f"source_handoff: {error}")

    validator_path = _repo_path(PHASE183_VALIDATOR_PATH)
    if source.get("validator") != PHASE183_VALIDATOR_PATH:
        errors.append(f"source_handoff.validator must be {PHASE183_VALIDATOR_PATH}")
    if not validator_path.exists():
        errors.append(f"source_handoff.validator path must exist: {PHASE183_VALIDATOR_PATH}")
    elif source.get("validator_sha256") != _sha256_file(validator_path):
        errors.append(f"source_handoff.validator_sha256 must match {PHASE183_VALIDATOR_PATH}")
    ...
    for finding in _scan_forbidden_true(payload):
        errors.append(f"documentops_phase183_validated_closure_receipt_summary_handoff_signoff: {finding}")

    if completed and not errors:
        warnings.append(
            "completed Phase 183 validated closure receipt summary handoff sign-off records evidence review only; "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 183 validated closure summary handoff sign-off record."
    )
    parser.add_argument("signoff", type=Path, help="Path to Phase 184 handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = validate_documentops_phase183_validated_closure_receipt_summary_handoff_signoff(
            payload,
            require_complete=bool(args.require_complete),
        )
