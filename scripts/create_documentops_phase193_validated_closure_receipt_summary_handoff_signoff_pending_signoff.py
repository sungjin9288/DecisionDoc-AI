#!/usr/bin/env python3
"""Create a pending DocumentOps Phase 193 validated handoff sign-off record."""
...
DEFAULT_TEMPLATE_PATH = (
    ...
    "phase193_local_feature_completion_validated_closure_receipt_summary_handoff_signoff/"
...
GENERATION_REPORT_TYPE = (
    "document_ops_phase194_local_feature_completion_validated_closure_receipt_summary_"
...
def _reset_pending_fields(signoff: dict[str, Any]) -> None:
    ...
    for key in REQUIRED_FALSE_BOUNDARY_FIELDS:
        boundary[key] = False
...
def _append_unique_path(paths: list[str], value: Any) -> None:
    ...
    if resolved_value not in paths:
        paths.append(resolved_value)
...
def _evidence_to_review(signoff: dict[str, Any], template_path: Path) -> list[str]:
    ...
    _append_unique_path(paths, "scripts/validate_documentops_phase192_validated_closure_receipt_summary_handoff_signoff.py")
    return paths


def build_pending_documentops_phase193_validated_closure_receipt_summary_handoff_signoff(
    *,
    ...
    source_template_validation = (
        validate_documentops_phase192_validated_closure_receipt_summary_handoff_signoff(template)
    )
    ...
    source_handoff = _as_dict(signoff.get("source_handoff"))
    source_handoff_path = (REPO_ROOT / PHASE192_HANDOFF_PATH).resolve()
    source_handoff_validator = (REPO_ROOT / PHASE192_VALIDATOR_PATH).resolve()

    signoff["signoff_id"] = safe_signoff_id
    signoff["created_at"] = created_at or _now_iso()
    signoff["generation_context"] = {
        "report_type": GENERATION_REPORT_TYPE,
        "phase": 194,
        "source_template_path": str(resolved_template),
        ...
        "source_handoff_sha256": source_handoff.get("sha256"),
        "source_handoff_validator": str(source_handoff_validator),
        "source_handoff_validator_sha256": source_handoff.get("validator_sha256"),
    ...
    signoff["next_step_after_generation"] = (
        ...
        "and acknowledgements, then runs the Phase 193 validator with --require-complete. "
    ...
    validation = validate_documentops_phase192_validated_closure_receipt_summary_handoff_signoff(signoff)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a pending DocumentOps Phase 193 validated handoff sign-off JSON record."
    )
    ...
    try:
        if not args.json and args.output is not None:
            ...
            if resolved_output.exists() and not args.overwrite:
                raise FileExistsError(f"output already exists: {resolved_output}")
        signoff = build_pending_documentops_phase193_validated_closure_receipt_summary_handoff_signoff(
        ...
        if args.json:
            ...
            return 0
        output_path = args.output or _default_output_path(signoff, args.output_dir)
        written_path = write_pending_signoff(signoff, output_path=output_path, overwrite=bool(args.overwrite))
        result = {
            "ok": True,
            ...
            "generated_record_validation": signoff["generation_context"]["generated_record_validation"],
            "generation_boundary": signoff["generation_boundary"],
        }
