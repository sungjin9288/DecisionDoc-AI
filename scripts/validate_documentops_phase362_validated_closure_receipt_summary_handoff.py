#!/usr/bin/env python3
"""Validate the DocumentOps Phase 361/362 validated closure receipt summary handoff."""
...
EXPECTED_REPORT_TYPE = (
    "document_ops_phase363_local_feature_completion_validated_closure_receipt_summary_handoff"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase363_local_feature_completion_validated_closure_receipt_summary_handoff_validation"
)
EXPECTED_STATUS = (
...
HANDOFF_VALID_KEY = "phase361_validated_closure_receipt_summary_handoff_valid"
...
SOURCE_SUMMARY_VALIDATOR_PATH = "scripts/validate_documentops_phase361_validated_closure_receipt_summary.py"
SOURCE_SUMMARY_REPORTER_PATH = "scripts/summarize_documentops_phase360_validated_closure_receipts.py"
REQUIRED_RECIPIENTS = {
    "release_owner",
    "operator",
    "product_pm_reviewer",
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    "local_phase361_validated_closure_receipt_summary_handoff_recorded",
...
PASS_MESSAGE = "PASS documentops phase363 validated closure receipt summary handoff validated"
FAIL_MESSAGE = "FAIL documentops phase363 validated closure receipt summary handoff validation failed"
...
def validate_documentops_phase362_validated_closure_receipt_summary_handoff(
    ...
    if handoff.get("phase") != 363:
        errors.append("phase must be 363")
    if handoff.get("status") != EXPECTED_STATUS:
        errors.append(f"status must be {EXPECTED_STATUS}")
    expected_scope = "operator_handoff_for_validated_phase361_closure_receipt_summary"
    ...
    for finding in _scan_forbidden_true(handoff):
        errors.append(f"documentops_phase363_validated_closure_receipt_summary_handoff: {finding}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 361/362 validated closure receipt summary handoff."
    )
    parser.add_argument(
        "handoff",
        nargs="?",
        ...
        help="Path to Phase 363 validated closure receipt summary handoff JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase362_validated_closure_receipt_summary_handoff(args.handoff)
    if args.json:
