#!/usr/bin/env python3
"""Summarize DocumentOps local feature completion closure receipts."""
...
def summarize_receipt(path: Path, payload: dict[str, Any]) -> dict[str, Any]:
    validation = validate_documentops_local_feature_completion_closure_receipt(path)
    ...
    return {
        ...
        "phase53_closure_index_sha256": source_hashes.get("phase53_closure_index_sha256", ""),
        "phase53_closure_validator_sha256": source_hashes.get("phase53_closure_validator_sha256", ""),
...
def build_documentops_local_feature_completion_closure_receipt_summary(
    paths: Sequence[Path],
    *,
    ...
    if load_errors:
        blocker_reasons.append("closure_receipt_load_errors")
    if not receipts:
        blocker_reasons.append("no_closure_receipts")
    if invalid_count:
        blocker_reasons.append("invalid_closure_receipts")
    if boundary_break_count:
        blocker_reasons.append("closure_receipt_boundary_breaks")

    status = "all_closure_receipts_confirm_no_cost_freeze" if not blocker_reasons else "follow_up_required"

    return {
        "report_type": "document_ops_phase55_local_feature_completion_closure_receipt_summary",
        ...
        "side_effect_boundary": {
            "reads_local_closure_receipts": True,
...
def _render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# DocumentOps Local Feature Completion Closure Receipt Summary",
        "",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Summarize DocumentOps local feature completion closure receipt JSON records."
    )
    parser.add_argument(
        "receipts",
        nargs="*",
        ...
        help="Closure receipt JSON file(s) or directories. Defaults to the Phase 54 receipt.",
    )
