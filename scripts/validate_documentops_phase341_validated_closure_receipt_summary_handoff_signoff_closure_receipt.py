#!/usr/bin/env python3
"""Validate the DocumentOps Phase 342 validated handoff sign-off closure receipt."""
...
from scripts.validate_documentops_phase336_to_phase340_validated_closure_receipt_summary_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_phase336_to_phase340_validated_closure_receipt_summary_handoff_signoff_closure_index,
)
...
DEFAULT_RECEIPT_PATH = (
    ...
    "phase342_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt/"
    "validated_closure_receipt_summary_handoff_signoff_closure_receipt.json"
)
EXPECTED_REPORT_TYPE = (
    "document_ops_phase342_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase342_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_validation"
)
EXPECTED_STATUS = (
...
REQUIRED_SOURCE_HASH_PATHS = {
    ...
    "phase341_closure_validator_sha256": PHASE341_VALIDATOR,
    "phase342_closure_receipt_validator_sha256": (
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    ...
    "phase341_closure_index_valid",
    "phase336_to_phase340_chain_valid",
...
EXPECTED_SOURCE_COMMAND = f"python3 {PHASE341_VALIDATOR}"
...
PASS_MESSAGE = (
    "PASS documentops phase342 validated closure receipt summary handoff sign-off closure receipt validated"
)
FAIL_MESSAGE = (
    "FAIL documentops phase342 validated closure receipt summary handoff sign-off closure receipt validation failed"
)
...
def validate_documentops_phase341_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 342:
        errors.append("phase must be 342")
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_phase342_validated_closure_receipt: {finding}")
    ...
    if not errors and gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 341 closure index validation")
        ...
        for field, expected_value in expected_pairs.items():
            ...
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 341 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 342 validated handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to Phase 342 validated handoff sign-off closure receipt JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase341_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
