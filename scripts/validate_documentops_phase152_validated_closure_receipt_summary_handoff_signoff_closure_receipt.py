#!/usr/bin/env python3
"""Validate the DocumentOps Phase 153 validated handoff sign-off closure receipt."""
...
from scripts.validate_documentops_phase147_to_phase151_validated_closure_receipt_summary_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_phase147_to_phase151_validated_closure_receipt_summary_handoff_signoff_closure_index,
)
...
DEFAULT_RECEIPT_PATH = (
    ...
    "phase153_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt/"
    "validated_closure_receipt_summary_handoff_signoff_closure_receipt.json"
)
EXPECTED_REPORT_TYPE = (
    "document_ops_phase153_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase153_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_validation"
)
EXPECTED_STATUS = (
...
REQUIRED_SOURCE_HASH_PATHS = {
    ...
    "phase152_closure_validator_sha256": (
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    ...
    "phase152_closure_index_valid",
    "phase147_to_phase151_chain_valid",
...
EXPECTED_SOURCE_COMMAND = (
    "python3 scripts/validate_documentops_phase147_to_phase151_validated_closure_receipt_summary_handoff_"
    "signoff_closure_index.py"
)
EXPECTED_TEMPORARY_READINESS = (
    "pending_phase147_validated_closure_receipt_summary_handoff_signoff_review_no_training_authorization"
)
PASS_MESSAGE = "PASS documentops phase153 validated closure receipt summary handoff sign-off closure receipt validated"
FAIL_MESSAGE = (
    "FAIL documentops phase153 validated closure receipt summary handoff sign-off closure receipt validation failed"
)
...
def validate_documentops_phase152_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 153:
        errors.append("phase must be 153")
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_phase153_validated_closure_receipt: {finding}")

    gate_result: dict[str, Any] = {}
    if not errors:
        gate_result = validate_documentops_phase147_to_phase151_validated_closure_receipt_summary_handoff_signoff_closure_index()
    if not errors and gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 152 closure index validation")
        ...
        for field, expected_value in expected_pairs.items():
            ...
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 152 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 153 validated handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to Phase 153 validated handoff sign-off closure receipt JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase152_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
