#!/usr/bin/env python3
"""Validate a DocumentOps Phase 87 validated handoff sign-off summary."""
...
EXPECTED_REPORT_TYPE = (
    "document_ops_phase87_local_feature_completion_validated_closure_handoff_signoff_"
...
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase88_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_"
...
READY_STATUS = (
    "all_validated_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_"
    "summary_handoff_signoffs_"
    "accepted_no_cost_boundary_preserved"
)
PENDING_STATUS = (
    "pending_validated_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_"
    "summary_handoff_signoff_"
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_summary(
    summary_path: Path,
    *,
    ...
    for index, signoff in enumerate(signoffs, start=1):
        ...
        if validation.get("ok") is not (current_validation.get("ok") is True):
            errors.append(f"{prefix}.validation.ok must match current Phase 85 validation")
        if validation.get("completed") is not (current_validation.get("completed") is True):
            errors.append(f"{prefix}.validation.completed must match current Phase 85 validation")
        if validation.get("source_handoff_validation_ok") != current_validation.get("source_handoff_validation_ok"):
            errors.append(f"{prefix}.validation.source_handoff_validation_ok must match current Phase 85 validation")
    ...
    for finding in _scan_forbidden_true(summary):
        errors.append(
            ...
            f"validated_handoff_summary_handoff_signoff_summary: {finding}"
        )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 87 validated handoff sign-off summary."
    )
    parser.add_argument("summary", type=Path, help="Path to Phase 87 sign-off summary JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_summary(
        args.summary,
