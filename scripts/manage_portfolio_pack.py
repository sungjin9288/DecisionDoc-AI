...
def _assert_pack_directory_is_safe(pack_dir: Path) -> None:
    if pack_dir.name != "decisiondoc_ai_portfolio_pack":
        raise ValueError("portfolio pack directory must be named decisiondoc_ai_portfolio_pack")
...
def build_manifest(root: Path, source_paths: Sequence[str]) -> dict[str, object]:
    ...
    for relative_path in source_paths:
        ...
        if source.is_symlink():
            raise ValueError(f"portfolio source file must not be a symlink: {safe_path}")
...
def check_pack(
    *,
    ...
    if actual_files != expected_files:
        ...
        raise ValueError(f"portfolio pack membership drifted: missing={missing}, unexpected={unexpected}")
    ...
    try:
        actual_manifest = json.loads((pack_dir / MANIFEST_NAME).read_text(encoding="utf-8"))
...
def package_zip(
    *,
    ...
    try:
        ...
        raise ValueError("portfolio ZIP must be written outside the portfolio pack directory")
    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as archive:
            ...
            for relative_path in sorted(expected_files):
                if archive.read(relative_path) != (pack_dir / relative_path).read_bytes():
                    raise ValueError(f"portfolio ZIP file drifted: {relative_path}")
    except (FileNotFoundError, zipfile.BadZipFile) as exc:
        raise ValueError(f"portfolio ZIP is unavailable or invalid: {zip_path}") from exc
    return {
        "ok": True,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sync and verify the DecisionDoc portfolio pack.")
    ...
    parser.add_argument("--prune", action="store_true", help="Remove files not present in the tracked source allowlist.")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "sync":
            ...
            result = check_pack(root=REPO_ROOT, pack_dir=pack_dir, source_paths=source_paths)
