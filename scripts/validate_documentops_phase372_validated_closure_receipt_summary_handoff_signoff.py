#!/usr/bin/env python3
"""Validate a DocumentOps Phase 373 validated closure summary handoff sign-off record."""
...
EXPECTED_SCHEMA = "decisiondoc_documentops_phase373_validated_closure_receipt_summary_handoff_signoff.v1"
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase373_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_validation"
)
SOURCE_HANDOFF_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase372_local_feature_completion_validated_closure_receipt_summary_handoff/"
    "validated_closure_receipt_summary_handoff.json"
)
SOURCE_HANDOFF_VALIDATOR_PATH = (
    "scripts/validate_documentops_phase371_validated_closure_receipt_summary_handoff.py"
)
SIGNOFF_ID_PREFIX = "documentops_local_feature_completion_phase372_validated_closure_receipt_summary_handoff_signoff_"
...
REQUIRED_ACKS = (
    "phase372_validated_closure_receipt_summary_handoff_reviewed",
    "phase370_summary_chain_reviewed",
    "phase371_summary_validation_reviewed",
...
PASS_MESSAGE = "PASS documentops phase373 validated closure receipt summary handoff sign-off validated"
FAIL_MESSAGE = "FAIL documentops phase373 validated closure receipt summary handoff sign-off validation failed"
...
def validate_documentops_phase372_validated_closure_receipt_summary_handoff_signoff(
    ...
    *,
    ...
    if payload.get("phase") != 373:
        errors.append("phase must be 373")
    expected_scope = "evidence_only_review_of_phase372_validated_closure_receipt_summary_handoff"
    ...
    if source_path_value != SOURCE_HANDOFF_PATH:
        errors.append("source_handoff.path must reference the Phase 372 validated handoff JSON")
    ...
    if not source_path.exists():
        ...
        if source.get("sha256") != _sha256_file(source_path):
            errors.append("source_handoff.sha256 must match the Phase 372 validated handoff JSON")
        source_validation = validate_documentops_phase371_validated_closure_receipt_summary_handoff(
            source_path
        )
        if source_validation.get("ok") is not True:
            errors.append("source_handoff must pass Phase 372 validation")
    ...
    for finding in _scan_forbidden_true(payload):
        errors.append(f"documentops_phase372_validated_closure_receipt_summary_handoff_signoff: {finding}")

    if completed and not errors:
        warnings.append(
            "completed Phase 372 validated closure receipt summary handoff sign-off records evidence review only; "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 373 validated closure summary handoff sign-off record."
    )
    parser.add_argument("signoff", type=Path, help="Path to Phase 373 handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = validate_documentops_phase372_validated_closure_receipt_summary_handoff_signoff(
            payload,
            require_complete=bool(args.require_complete),
        )
