#!/usr/bin/env python3
"""Validate a DocumentOps validated closure handoff sign-off closure receipt summary."""
...
from scripts.validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt import (  # noqa: E402
    validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt,
)


EXPECTED_REPORT_TYPE = (
    "document_ops_phase64_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_summary"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase65_local_feature_completion_validated_closure_handoff_signoff_"
...
READY_STATUS = "all_validated_closure_handoff_signoff_closure_receipts_confirm_no_cost_freeze"
FORBIDDEN_TRUE_KEYS = {
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_summary(
    summary_path: Path,
) -> dict[str, Any]:
    ...
    if summary.get("phase") != 64:
        errors.append("phase must be 64")
    ...
    for index, receipt in enumerate(receipts, start=1):
        ...
        if receipt.get("sha256") != _sha256_file(receipt_path):
            errors.append(f"{prefix}.sha256 must match linked Phase 63 receipt file")
        try:
        ...
        if validation.get("ok") is not (current_validation.get("ok") is True):
            errors.append(f"{prefix}.validation.ok must match current Phase 63 validation")
        if validation.get("closure_receipt_valid") is not (
            current_validation.get("closure_receipt_valid") is True
        ):
            errors.append(f"{prefix}.validation.closure_receipt_valid must match current Phase 63 validation")
        if validation.get("source_closure_gate_valid") is not (
            current_validation.get("source_closure_gate_valid") is True
        ):
            errors.append(f"{prefix}.validation.source_closure_gate_valid must match current Phase 63 validation")
        ...
        expected_pairs = {
            ...
            "phase62_closure_index_sha256": source_hashes.get("phase62_closure_index_sha256", ""),
            "phase62_closure_validator_sha256": source_hashes.get("phase62_closure_validator_sha256", ""),
        }
        for field, expected_value in expected_pairs.items():
            if receipt.get(field) != expected_value:
                errors.append(f"{prefix}.{field} must match linked Phase 63 receipt")
    ...
    if boundary.get("reads_local_validated_closure_handoff_signoff_closure_receipts") is not True:
        errors.append(
            "side_effect_boundary.reads_local_validated_closure_handoff_signoff_closure_receipts must be true"
        )
    ...
    for finding in _scan_forbidden_true(summary):
        errors.append(f"documentops_validated_closure_handoff_signoff_closure_receipt_summary: {finding}")

    return {
        "report_type": EXPECTED_VALIDATION_REPORT_TYPE,
        "ok": not errors,
        "summary_path": str(resolved_summary),
        "closure_receipt_summary_valid": not errors,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("summary", type=Path, help="Path to Phase 64 closure receipt summary JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_summary(
        args.summary
    )
    if args.json:
        ...
        print(
            ...
            "signoff closure receipt summary validated"
        )
        print(f"closure_receipt_summary_valid={str(result['closure_receipt_summary_valid']).lower()}")
        ...
        print(
            ...
            "signoff closure receipt summary validation failed"
        )
