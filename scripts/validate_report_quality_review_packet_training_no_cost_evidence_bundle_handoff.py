#!/usr/bin/env python3
"""Validate a local handoff for report quality training no-cost evidence bundles."""
...
def _load_bundle_validator():
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"failed to load no-cost evidence bundle validator: {BUNDLE_VALIDATOR_PATH}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


_BUNDLE_VALIDATOR = _load_bundle_validator()
...
def validate_training_no_cost_evidence_bundle_handoff(
    ...
    *,
    ...
    try:
        ...
        return {
            "report_type": "report_quality_training_no_cost_evidence_bundle_handoff_validation",
            "ok": False,
    ...
    if handoff.get("report_type") != "report_quality_training_no_cost_evidence_bundle_handoff":
        errors.append("report_type must be report_quality_training_no_cost_evidence_bundle_handoff")
    ...
    if markdown_path is not None:
        ...
        if "Report Quality Training No-Cost Evidence Bundle Handoff" not in markdown:
    ...
    bundle_path = _resolve_path(handoff.get("evidence_bundle_path"), field="evidence_bundle_path", errors=errors)
    _validate_hash(
        path=bundle_path,
        expected_hash=handoff.get("evidence_bundle_sha256"),
        field="evidence_bundle_sha256",
        errors=errors,
    )
    bundle_validation: dict[str, Any] = {}
    if bundle_path is not None:
        bundle_validation = _BUNDLE_VALIDATOR.validate_training_no_cost_evidence_bundle(bundle_path)
        if require_ready and bundle_validation.get("ok") is not True:
            errors.append("no-cost evidence bundle validation must pass")

    embedded_bundle_validation = _as_dict(handoff.get("evidence_bundle_validation"))
    if require_ready and embedded_bundle_validation.get("ok") is not True:
        errors.append("embedded evidence_bundle_validation.ok must be true")
    ...
    if readiness.get("status") != "no_cost_evidence_bundle_handoff_ready":
        errors.append("readiness.status must be no_cost_evidence_bundle_handoff_ready")
    ...
    if bundle_path is not None:
        ...
        bundle_counts = _as_dict(bundle.get("counts"))
        for field in (
            "archive_closure_count",
            "valid_archive_closure_count",
            "archived_no_cost_hold_count",
        ):
            if counts.get(field) != bundle_counts.get(field):
                errors.append(f"counts.{field} must match evidence bundle counts")
    ...
    for finding in _scan_forbidden_true(handoff):
        errors.append(f"training_no_cost_evidence_bundle_handoff: {finding}")
    ...
    return {
        ...
        "evidence_bundle_validation_ok": bundle_validation.get("ok") if bundle_validation else None,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a local no-cost evidence bundle handoff manifest.")
    parser.add_argument("handoff_manifest", type=Path, help="Path to *-training-no-cost-evidence-bundle-handoff-manifest.json.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_training_no_cost_evidence_bundle_handoff(
    ...
    if args.json:
        ...
        print("PASS report quality training no-cost evidence bundle handoff validated")
        ...
        print("FAIL report quality training no-cost evidence bundle handoff validation failed")
