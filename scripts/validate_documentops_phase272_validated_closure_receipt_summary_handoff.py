#!/usr/bin/env python3
"""Validate the DocumentOps Phase 271/272 validated closure receipt summary handoff."""
...
EXPECTED_REPORT_TYPE = (
    "document_ops_phase273_local_feature_completion_validated_closure_receipt_summary_handoff"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase273_local_feature_completion_validated_closure_receipt_summary_handoff_validation"
)
EXPECTED_STATUS = (
...
HANDOFF_VALID_KEY = "phase271_validated_closure_receipt_summary_handoff_valid"
SOURCE_SUMMARY_VALIDATION_CONTRACT_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase272_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_summary_validation/"
...
SOURCE_SUMMARY_CONTRACT_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase271_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_summary/"
    "validated_closure_receipt_summary_handoff_signoff_closure_receipt_summary_contract.json"
)
SOURCE_SUMMARY_VALIDATOR_PATH = "scripts/validate_documentops_phase271_validated_closure_receipt_summary.py"
SOURCE_SUMMARY_REPORTER_PATH = "scripts/summarize_documentops_phase270_validated_closure_receipts.py"
REQUIRED_RECIPIENTS = {
    "release_owner",
    "operator",
    "product_pm_reviewer",
...
REQUIRED_ACTION_IDS = {
    "generate_phase271_summary",
    "run_phase272_validator",
    "confirm_validated_local_phase271_summary",
    "confirm_no_cost_boundary",
    "preserve_service_freeze",
    "require_separate_resume_approval",
}
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    "local_phase271_validated_closure_receipt_summary_handoff_recorded",
...
PASS_MESSAGE = "PASS documentops phase273 validated closure receipt summary handoff validated"
FAIL_MESSAGE = "FAIL documentops phase273 validated closure receipt summary handoff validation failed"
...
def validate_documentops_phase272_validated_closure_receipt_summary_handoff(
    ...
    if handoff.get("phase") != 273:
        errors.append("phase must be 273")
    if handoff.get("status") != EXPECTED_STATUS:
        errors.append(f"status must be {EXPECTED_STATUS}")
    expected_scope = "operator_handoff_for_validated_phase271_closure_receipt_summary"
    ...
    for finding in _scan_forbidden_true(handoff):
        errors.append(f"documentops_phase273_validated_closure_receipt_summary_handoff: {finding}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 271/272 validated closure receipt summary handoff."
    )
    parser.add_argument(
        "handoff",
        nargs="?",
        ...
        help="Path to Phase 273 validated closure receipt summary handoff JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase272_validated_closure_receipt_summary_handoff(args.handoff)
    if args.json:
