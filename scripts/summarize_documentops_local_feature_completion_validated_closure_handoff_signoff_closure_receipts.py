#!/usr/bin/env python3
"""Summarize DocumentOps validated closure handoff sign-off closure receipts."""
...
def summarize_receipt(path: Path, payload: dict[str, Any]) -> dict[str, Any]:
    validation = validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt(path)
...
def build_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_summary(
    paths: Sequence[Path],
    *,
    ...
    if load_errors:
        blocker_reasons.append("validated_closure_handoff_signoff_closure_receipt_load_errors")
    if not receipts:
        blocker_reasons.append("no_validated_closure_handoff_signoff_closure_receipts")
    if invalid_count:
        blocker_reasons.append("invalid_validated_closure_handoff_signoff_closure_receipts")
    if boundary_break_count:
        blocker_reasons.append("validated_closure_handoff_signoff_closure_receipt_boundary_breaks")
    ...
    return {
        ...
        "side_effect_boundary": {
            "reads_local_validated_closure_handoff_signoff_closure_receipts": True,
...
def _render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# DocumentOps Validated Closure Handoff Sign-Off Closure Receipt Summary",
        "",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Summarize DocumentOps validated closure handoff sign-off closure receipt JSON records."
        )
    )
    parser.add_argument(
        "receipts",
        ...
        help="Closure receipt JSON file(s) or directories. Defaults to the Phase 63 receipt.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = build_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_summary(
        args.receipts,
