#!/usr/bin/env python3
"""Create a pending DocumentOps Phase 355 validated handoff sign-off record."""
...
DEFAULT_TEMPLATE_PATH = (
    ...
    "phase355_local_feature_completion_validated_closure_receipt_summary_handoff_signoff/"
...
GENERATION_REPORT_TYPE = (
    "document_ops_phase356_local_feature_completion_validated_closure_receipt_summary_"
...
def build_pending_documentops_phase355_validated_closure_receipt_summary_handoff_signoff(
    *,
    ...
    source_template_validation = (
        validate_documentops_phase354_validated_closure_receipt_summary_handoff_signoff(template)
    )
    ...
    signoff["next_step_after_generation"] = (
        ...
        "and acknowledgements, then runs the Phase 355 validator with --require-complete. "
    ...
    validation = validate_documentops_phase354_validated_closure_receipt_summary_handoff_signoff(signoff)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create a pending DocumentOps Phase 355 validated handoff sign-off JSON record."
    )
    ...
    try:
        if not args.json and args.output is not None:
            ...
            if resolved_output.exists() and not args.overwrite:
                raise FileExistsError(f"output already exists: {resolved_output}")
        signoff = build_pending_documentops_phase355_validated_closure_receipt_summary_handoff_signoff(
