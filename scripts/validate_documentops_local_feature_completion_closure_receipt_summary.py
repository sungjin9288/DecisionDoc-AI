#!/usr/bin/env python3
"""Validate a DocumentOps local feature completion closure receipt summary."""
...
from scripts.validate_documentops_local_feature_completion_closure_receipt import (  # noqa: E402
    validate_documentops_local_feature_completion_closure_receipt,
)


EXPECTED_REPORT_TYPE = "document_ops_phase55_local_feature_completion_closure_receipt_summary"
EXPECTED_VALIDATION_REPORT_TYPE = "document_ops_phase56_local_feature_completion_closure_receipt_summary_validation"
EXPECTED_SCHEMA = "decisiondoc_documentops_local_feature_completion_closure_receipt_summary.v1"
READY_STATUS = "all_closure_receipts_confirm_no_cost_freeze"
FORBIDDEN_TRUE_KEYS = {
...
def validate_documentops_local_feature_completion_closure_receipt_summary(
    summary_path: Path,
) -> dict[str, Any]:
    ...
    for index, receipt in enumerate(receipts, start=1):
        ...
        if receipt.get("sha256") != _sha256_file(receipt_path):
            errors.append(f"{prefix}.sha256 must match linked Phase 54 receipt file")
        try:
        ...
        current_validation = validate_documentops_local_feature_completion_closure_receipt(receipt_path)
        if validation.get("ok") is not (current_validation.get("ok") is True):
            errors.append(f"{prefix}.validation.ok must match current Phase 54 validation")
        if validation.get("closure_receipt_valid") is not (current_validation.get("closure_receipt_valid") is True):
            errors.append(f"{prefix}.validation.closure_receipt_valid must match current Phase 54 validation")
        if validation.get("source_closure_gate_valid") is not (
            current_validation.get("source_closure_gate_valid") is True
        ):
            errors.append(f"{prefix}.validation.source_closure_gate_valid must match current Phase 54 validation")
        ...
        for field, expected_value in expected_pairs.items():
            if receipt.get(field) != expected_value:
                errors.append(f"{prefix}.{field} must match linked Phase 54 receipt")
    ...
    if boundary.get("reads_local_closure_receipts") is not True:
        errors.append("side_effect_boundary.reads_local_closure_receipts must be true")
    ...
    for finding in _scan_forbidden_true(summary):
        errors.append(f"documentops_closure_receipt_summary: {finding}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps local feature completion closure receipt summary."
    )
    parser.add_argument("summary", type=Path, help="Path to Phase 55 closure receipt summary JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_local_feature_completion_closure_receipt_summary(args.summary)
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    elif result["ok"]:
        print("PASS documentops local feature completion closure receipt summary validated")
        ...
        print("FAIL documentops local feature completion closure receipt summary validation failed")
