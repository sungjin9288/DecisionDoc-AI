#!/usr/bin/env python3
"""Validate the DocumentOps Phase 189 validated handoff sign-off closure receipt."""
...
from scripts.validate_documentops_phase183_to_phase187_validated_closure_receipt_summary_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_phase183_to_phase187_validated_closure_receipt_summary_handoff_signoff_closure_index,
)
...
DEFAULT_RECEIPT_PATH = (
    ...
    "phase189_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt/"
    "validated_closure_receipt_summary_handoff_signoff_closure_receipt.json"
)
EXPECTED_REPORT_TYPE = (
    "document_ops_phase189_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase189_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_validation"
)
EXPECTED_STATUS = (
...
REQUIRED_SOURCE_HASH_PATHS = {
    ...
    "phase188_closure_validator_sha256": PHASE188_VALIDATOR,
    "phase189_closure_receipt_validator_sha256": (
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    ...
    "phase188_closure_index_valid",
    "phase183_to_phase187_chain_valid",
...
EXPECTED_SOURCE_COMMAND = f"python3 {PHASE188_VALIDATOR}"
...
PASS_MESSAGE = (
    "PASS documentops phase189 validated closure receipt summary handoff sign-off closure receipt validated"
)
FAIL_MESSAGE = (
    "FAIL documentops phase189 validated closure receipt summary handoff sign-off closure receipt validation failed"
)
...
def validate_documentops_phase188_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 189:
        errors.append("phase must be 189")
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_phase189_validated_closure_receipt: {finding}")

    gate_result: dict[str, Any] = {}
    if not errors:
        gate_result = validate_documentops_phase183_to_phase187_validated_closure_receipt_summary_handoff_signoff_closure_index()
    if not errors and gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 188 closure index validation")
        ...
        for field, expected_value in expected_pairs.items():
            ...
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 188 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps Phase 189 validated handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to Phase 189 validated handoff sign-off closure receipt JSON.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_phase188_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
