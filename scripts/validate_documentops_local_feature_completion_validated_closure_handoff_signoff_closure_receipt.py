#!/usr/bin/env python3
"""Validate the DocumentOps validated closure handoff sign-off closure receipt."""
...
from scripts.validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_index import (  # noqa: E402
    validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_index,
)
...
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase63_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validation"
)
EXPECTED_STATUS = (
...
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    ...
    "phase62_closure_index_valid",
    "phase57_to_phase61_chain_valid",
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt(
    ...
    if receipt.get("phase") != 63:
        errors.append("phase must be 63")
    ...
    expected_command = (
        "python3 scripts/"
        "validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_index.py"
    )
    ...
    for finding in _scan_forbidden_true(receipt):
        errors.append(f"documentops_validated_closure_handoff_signoff_closure_receipt: {finding}")

    gate_result = validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_index()
    if gate_result.get("ok") is not True:
        errors.append("source_closure_gate must still pass Phase 62 closure index validation")
        ...
        for field, expected_value in expected_pairs.items():
            if source_gate.get(field) != expected_value or gate_result.get(field) != expected_value:
                errors.append(f"source_closure_gate.{field} must match current Phase 62 validation")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps validated closure handoff sign-off closure receipt."
    )
    parser.add_argument(
        "receipt",
        nargs="?",
        ...
        help="Path to phase63 validated_closure_handoff_signoff_closure_receipt.json.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt(
        args.receipt
    )
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    elif result["ok"]:
        print("PASS documentops local feature completion validated closure handoff signoff closure receipt validated")
        ...
        print("FAIL documentops local feature completion validated closure handoff signoff closure receipt validation failed")
