#!/usr/bin/env python3
"""Validate the DocumentOps local feature completion validated closure handoff."""
...
from scripts.summarize_documentops_local_feature_completion_closure_receipts import (  # noqa: E402
    DEFAULT_RECEIPT_PATH,
    build_documentops_local_feature_completion_closure_receipt_summary,
)
from scripts.validate_documentops_local_feature_completion_closure_receipt_summary import (  # noqa: E402
    validate_documentops_local_feature_completion_closure_receipt_summary,
)
...
EXPECTED_REPORT_TYPE = "document_ops_phase57_local_feature_completion_validated_closure_handoff"
EXPECTED_VALIDATION_REPORT_TYPE = "document_ops_phase57_local_feature_completion_validated_closure_handoff_validation"
EXPECTED_STATUS = "local_feature_completion_validated_closure_handoff_ready_no_aws_no_training_authorization"
SOURCE_SUMMARY_VALIDATION_CONTRACT_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase56_local_feature_completion_closure_receipt_summary_validation/closure_receipt_summary_validation_contract.json"
)
SOURCE_SUMMARY_CONTRACT_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase55_local_feature_completion_closure_receipt_summary/closure_receipt_summary_contract.json"
)
SOURCE_SUMMARY_VALIDATOR_PATH = "scripts/validate_documentops_local_feature_completion_closure_receipt_summary.py"
SOURCE_SUMMARY_REPORTER_PATH = "scripts/summarize_documentops_local_feature_completion_closure_receipts.py"
REQUIRED_RECIPIENTS = {
    "release_owner",
    "operator",
    "product_pm_reviewer",
...
REQUIRED_ACTION_IDS = {
    "generate_phase55_summary",
    "run_phase56_validator",
    "confirm_validated_local_closure",
    "confirm_no_cost_boundary",
    "preserve_service_freeze",
    "require_separate_resume_approval",
}
REQUIRED_TRUE_BOUNDARY_FIELDS = (
    "local_validated_closure_handoff_recorded",
...
def _run_generated_summary_validation_probe() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="decisiondoc_phase57_validated_handoff_") as tmp_dir:
        summary = build_documentops_local_feature_completion_closure_receipt_summary(
            [DEFAULT_RECEIPT_PATH],
            generated_at="2026-05-27T00:00:00+09:00",
        )
        summary_path = Path(tmp_dir) / "phase57_closure_receipt_summary.json"
        _write_json(summary_path, summary)
        validation = validate_documentops_local_feature_completion_closure_receipt_summary(summary_path)
    return {
...
def validate_documentops_local_feature_completion_validated_closure_handoff(
    ...
    if handoff.get("phase") != 57:
        errors.append("phase must be 57")
    if handoff.get("status") != EXPECTED_STATUS:
        errors.append(f"status must be {EXPECTED_STATUS}")
    if handoff.get("handoff_scope") != "operator_handoff_for_validated_phase55_closure_summary":
        errors.append("handoff_scope must be operator_handoff_for_validated_phase55_closure_summary")
    ...
    for finding in _scan_forbidden_true(handoff):
        errors.append(f"documentops_validated_closure_handoff: {finding}")
    ...
    if probe and probe.get("summary_ok") is not True:
        errors.append("generated Phase 55 closure receipt summary must be ok")
    if probe and validation.get("ok") is not True:
        errors.append("generated Phase 55 closure receipt summary must pass Phase 56 validation")
        for error in validation.get("errors", []):
            errors.append(f"generated_summary_validation: {error}")
    if probe and probe.get("summary_readiness_status") != "all_closure_receipts_confirm_no_cost_freeze":
        errors.append("generated summary readiness must confirm no-cost freeze")
    if probe and probe.get("receipt_count") != 1:
        errors.append("generated summary receipt_count must be 1 for the default Phase 54 receipt")

    return {
        "report_type": EXPECTED_VALIDATION_REPORT_TYPE,
        "ok": not errors,
        "handoff_path": str(resolved_handoff),
        "validated_closure_handoff_valid": not errors,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the DocumentOps local feature completion validated closure handoff."
    )
    parser.add_argument(
        "handoff",
        nargs="?",
        ...
        help="Path to phase57 validated_closure_handoff.json.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_documentops_local_feature_completion_validated_closure_handoff(args.handoff)
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    elif result["ok"]:
        print("PASS documentops local feature completion validated closure handoff validated")
        print(f"validated_closure_handoff_valid={str(result['validated_closure_handoff_valid']).lower()}")
        ...
        print("FAIL documentops local feature completion validated closure handoff validation failed")
