#!/usr/bin/env python3
"""Summarize DocumentOps Phase 324 validated closure receipt records."""
...
def summarize_receipt(path: Path, payload: dict[str, Any]) -> dict[str, Any]:
    validation = validate_documentops_phase323_validated_closure_receipt_summary_handoff_signoff_closure_receipt(
        path
    )
...
def build_documentops_phase324_validated_closure_receipt_summary(
    paths: Sequence[Path],
    *,
    ...
    if load_errors:
        blocker_reasons.append("phase324_validated_closure_receipt_load_errors")
    if not receipts:
        blocker_reasons.append("no_phase324_validated_closure_receipts")
    if invalid_count:
        blocker_reasons.append("invalid_phase324_validated_closure_receipts")
    if boundary_break_count:
        blocker_reasons.append("phase324_validated_closure_receipt_boundary_breaks")

    status = (
        "all_phase324_validated_closure_receipts_confirm_no_cost_freeze"
        if not blocker_reasons
        else "follow_up_required"
    )

    return {
        "report_type": "document_ops_phase325_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_summary",
        "phase": 325,
        ...
        "side_effect_boundary": {
            "reads_local_phase324_validated_closure_receipts": True,
...
def _render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        "# DocumentOps Phase 324 Validated Closure Receipt Summary",
        "",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Summarize DocumentOps Phase 324 validated closure receipt JSON records."
    )
    parser.add_argument(
        "receipts",
        nargs="*",
        ...
        help="Closure receipt JSON file(s) or directories. Defaults to the Phase 324 receipt.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = build_documentops_phase324_validated_closure_receipt_summary(
        args.receipts,
