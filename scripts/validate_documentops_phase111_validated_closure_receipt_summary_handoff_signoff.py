#!/usr/bin/env python3
"""Validate a DocumentOps Phase 111 validated closure summary handoff sign-off record."""
...
EXPECTED_SCHEMA = (
    "decisiondoc_documentops_phase112_local_feature_completion_validated_closure_receipt_summary_handoff_signoff.v1"
)
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase112_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_validation"
)
PHASE111_HANDOFF_PATH = (
    "docs/specs/hermes_decisiondoc_agent/"
    "phase111_local_feature_completion_validated_closure_receipt_summary_handoff_signoff_closure_receipt_"
    "summary_handoff/"
    "validated_closure_receipt_summary_handoff_signoff_closure_receipt_summary_handoff.json"
)
PHASE111_VALIDATOR_PATH = "scripts/validate_documentops_phase110_validated_closure_receipt_summary_handoff.py"
SIGNOFF_ID_PREFIX = "documentops_local_feature_completion_phase111_validated_closure_receipt_summary_handoff_signoff_"
...
REQUIRED_ACKS = (
    "phase111_validated_closure_receipt_summary_handoff_reviewed",
    "phase109_summary_chain_reviewed",
    "phase110_summary_validation_reviewed",
...
PASS_MESSAGE = "PASS documentops phase112 validated closure receipt summary handoff sign-off validated"
FAIL_MESSAGE = "FAIL documentops phase112 validated closure receipt summary handoff sign-off validation failed"
...
def validate_documentops_phase111_validated_closure_receipt_summary_handoff_signoff(
    ...
    *,
    ...
    if payload.get("phase") != 112:
        errors.append("phase must be 112")
    if payload.get("signoff_scope") != "evidence_only_review_of_phase111_validated_closure_receipt_summary_handoff":
        errors.append("signoff_scope must be evidence_only_review_of_phase111_validated_closure_receipt_summary_handoff")
    ...
    if source_path_value != PHASE111_HANDOFF_PATH:
        errors.append("source_handoff.path must reference the Phase 111 validated handoff JSON")
        source_path = _repo_path(PHASE111_HANDOFF_PATH)
    else:
    ...
    if not source_path.exists():
        ...
        if source.get("sha256") != _sha256_file(source_path):
            errors.append("source_handoff.sha256 must match the Phase 111 validated handoff JSON")
        source_validation = validate_documentops_phase110_validated_closure_receipt_summary_handoff(source_path)
        if source_validation.get("ok") is not True:
            errors.append("source_handoff must pass Phase 111 validation")
            for error in source_validation.get("errors", []):
                errors.append(f"source_handoff: {error}")

    validator_path = _repo_path(PHASE111_VALIDATOR_PATH)
    if source.get("validator") != PHASE111_VALIDATOR_PATH:
        errors.append(f"source_handoff.validator must be {PHASE111_VALIDATOR_PATH}")
    if not validator_path.exists():
        errors.append(f"source_handoff.validator path must exist: {PHASE111_VALIDATOR_PATH}")
    elif source.get("validator_sha256") != _sha256_file(validator_path):
        errors.append(f"source_handoff.validator_sha256 must match {PHASE111_VALIDATOR_PATH}")
    ...
    for finding in _scan_forbidden_true(payload):
        errors.append(f"documentops_phase111_validated_closure_receipt_summary_handoff_signoff: {finding}")

    if completed and not errors:
        warnings.append(
            "completed Phase 111 validated closure receipt summary handoff sign-off records evidence review only; "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 111 validated closure summary handoff sign-off record."
    )
    parser.add_argument("signoff", type=Path, help="Path to Phase 112 handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = validate_documentops_phase111_validated_closure_receipt_summary_handoff_signoff(
            payload,
            require_complete=bool(args.require_complete),
        )
