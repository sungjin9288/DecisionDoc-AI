#!/usr/bin/env python3
"""Create a pending human sign-off record for a no-cost operator handoff."""
...
DEFAULT_TEMPLATE_PATH = REPO_ROOT / "docs/specs/report_quality_learning/training_no_cost_operator_handoff_signoff_template.json"
HANDOFF_VALIDATOR_PATH = REPO_ROOT / "scripts/validate_report_quality_review_packet_training_no_cost_operator_handoff.py"
...
SIGNOFF_ID_PATTERN = re.compile(r"rqp_training_no_cost_operator_handoff_signoff_[A-Za-z0-9_-]{8,96}")
BOUNDARY_FALSE_KEYS = (
...
def _safe_signoff_id(value: str | None) -> str:
    signoff_id = (
        ...
        else f"rqp_training_no_cost_operator_handoff_signoff_{uuid4().hex}"
    )
    if not SIGNOFF_ID_PATTERN.fullmatch(signoff_id):
        raise ValueError("signoff id must match rqp_training_no_cost_operator_handoff_signoff_[A-Za-z0-9_-]{8,96}")
    return signoff_id


def _default_output_path(operator_handoff_manifest_path: Path) -> Path:
    name = operator_handoff_manifest_path.name
    if name.endswith("-training-no-cost-operator-handoff-manifest.json"):
        base = name.removesuffix("-training-no-cost-operator-handoff-manifest.json")
    else:
        base = operator_handoff_manifest_path.stem
    return operator_handoff_manifest_path.with_name(f"{base}-training-no-cost-operator-handoff-signoff.json")
...
def _evidence_to_review(operator_handoff_manifest_path: Path, operator_handoff_manifest: dict[str, Any]) -> list[str]:
    paths: list[str] = []
    _append_unique_path(paths, str(operator_handoff_manifest_path))
    _append_unique_path(paths, operator_handoff_manifest.get("operator_handoff_markdown_path"))
    _append_unique_path(paths, operator_handoff_manifest.get("service_lock_report_summary_path"))
    return paths


def build_pending_training_no_cost_operator_handoff_signoff(
    *,
    ...
    resolved_handoff_manifest = operator_handoff_manifest_path.expanduser().resolve()
    resolved_template = template_path.expanduser().resolve()
    handoff_validation = _HANDOFF_VALIDATOR.validate_training_no_cost_operator_handoff(
        resolved_handoff_manifest,
        require_ready=require_ready,
    )
    if require_ready and handoff_validation.get("ok") is not True:
        errors = "; ".join(handoff_validation.get("errors") or ["no-cost operator handoff validation failed"])
        raise ValueError(f"no-cost operator handoff is not ready for sign-off generation: {errors}")
    ...
    signoff.update(
        {
            ...
            "generation_context": {
                "report_type": "report_quality_training_no_cost_operator_handoff_pending_signoff_generation",
                "generated_from_template": str(resolved_template),
                "handoff_validation": handoff_validation,
                "handoff_state": _as_dict(handoff_manifest.get("handoff_state")),
            ...
            "next_step_after_generation": (
                ...
                "validate_report_quality_review_packet_training_no_cost_operator_handoff_signoff.py "
                "--require-complete. This sign-off records operator handoff review only and does not "
                "resume service operation, AWS runtime, provider calls, training, or model promotion."
            ),
        }
    )
    return signoff
...
def write_pending_signoff(signoff: dict[str, Any], *, output_path: Path, overwrite: bool = False) -> Path:
    ...
    with tmp.open("w", encoding="utf-8") as f:
        f.write(json.dumps(signoff, ensure_ascii=False, indent=2, sort_keys=True) + "\n")
        f.flush()
        os.fsync(f.fileno())
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create a pending no-cost operator handoff sign-off JSON record.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--signoff-id",
        help="Optional deterministic id matching rqp_training_no_cost_operator_handoff_signoff_[A-Za-z0-9_-]{8,96}.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        resolved_handoff = args.operator_handoff_manifest.expanduser().resolve()
        signoff = build_pending_training_no_cost_operator_handoff_signoff(
            operator_handoff_manifest_path=resolved_handoff,
        ...
        if args.json:
            ...
            print("FAIL report quality training no-cost operator handoff pending signoff generation failed")
            print(f"ERROR {exc}")
        return 1
    ...
    if args.json:
        ...
        print("Report quality training no-cost operator handoff pending signoff: PASS")
