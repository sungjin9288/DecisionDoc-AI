#!/usr/bin/env python3
"""Validate a final no-cost hold manifest for report quality training work."""
...
SIGNOFF_SUMMARY_SCHEMA = "decisiondoc_report_quality_training_no_cost_ops_lock_handoff_signoff_summary.v1"
FORBIDDEN_TRUE_KEYS = {
...
def validate_ops_lock_handoff_signoff_summary_ready(summary: dict[str, Any]) -> dict[str, Any]:
    errors: list[str] = []
    if summary.get("schema_version") != SIGNOFF_SUMMARY_SCHEMA:
        errors.append(f"summary.schema_version must be {SIGNOFF_SUMMARY_SCHEMA!r}")
    if summary.get("report_type") != "report_quality_training_no_cost_ops_lock_handoff_signoff_summary":
        errors.append("summary.report_type must be report_quality_training_no_cost_ops_lock_handoff_signoff_summary")
    ...
    if readiness.get("status") != "all_ops_lock_handoff_signoffs_confirm_service_lock":
        errors.append("summary.readiness.status must confirm service lock sign-offs")
...
def validate_training_no_cost_final_hold(
    final_hold_manifest_path: Path,
    *,
    require_active: bool = True,
) -> dict[str, Any]:
    resolved_manifest = final_hold_manifest_path.expanduser().resolve()
    errors: list[str] = []
    warnings: list[str] = []
    try:
        final_hold = _load_json(resolved_manifest)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        return {
            "report_type": "report_quality_training_no_cost_final_hold_validation",
            "ok": False,
            "require_active": require_active,
            "final_hold_manifest_path": str(resolved_manifest),
            "errors": [str(exc)],
            "warnings": [],
        }

    if final_hold.get("schema_version") != EXPECTED_SCHEMA:
        errors.append(f"schema_version must be {EXPECTED_SCHEMA!r}")
    if final_hold.get("report_type") != "report_quality_training_no_cost_final_hold":
        errors.append("report_type must be report_quality_training_no_cost_final_hold")

    recorded_manifest = _resolve_path(
        final_hold.get("final_hold_manifest_path"),
        field="final_hold_manifest_path",
        errors=errors,
    )
    if recorded_manifest is not None and recorded_manifest != resolved_manifest:
        warnings.append("final_hold_manifest_path points to a different path than the validated manifest")

    markdown_path = _resolve_path(
        final_hold.get("final_hold_markdown_path"),
        field="final_hold_markdown_path",
        errors=errors,
    )
    if markdown_path is not None:
        markdown = markdown_path.read_text(encoding="utf-8")
        if "Report Quality Training No-Cost Final Hold" not in markdown:
            errors.append("final hold markdown is missing title")
        if "final_hold_active: `true`" not in markdown:
            errors.append("final hold markdown must show final_hold_active=true")
        if "service_operation_locked: `true`" not in markdown:
            errors.append("final hold markdown must show service_operation_locked=true")
        if "resume_blocked: `true`" not in markdown:
            errors.append("final hold markdown must show resume_blocked=true")
        if "operation_resume_approved: `false`" not in markdown:
            errors.append("final hold markdown must show operation_resume_approved=false")
        if "aws_cost_increase_allowed: `false`" not in markdown:
            errors.append("final hold markdown must show aws_cost_increase_allowed=false")
        if "training_execution_started: `false`" not in markdown:
            errors.append("final hold markdown must show training_execution_started=false")

    summary_path = _resolve_path(
        final_hold.get("ops_lock_handoff_signoff_summary_path"),
        field="ops_lock_handoff_signoff_summary_path",
        errors=errors,
    )
    _validate_hash(
        path=summary_path,
        expected_hash=final_hold.get("ops_lock_handoff_signoff_summary_sha256"),
        field="ops_lock_handoff_signoff_summary_sha256",
        errors=errors,
    )
    ...
    if summary_path is not None:
        ...
        summary_validation = validate_ops_lock_handoff_signoff_summary_ready(summary)
        if require_active and summary_validation.get("ok") is not True:
            errors.append("no-cost ops lock handoff sign-off summary validation must pass")

    embedded_summary_validation = _as_dict(final_hold.get("summary_validation"))
    if require_active and embedded_summary_validation.get("ok") is not True:
        errors.append("embedded summary_validation.ok must be true")

    state = _as_dict(final_hold.get("final_hold_state"))
    if require_active and state.get("active") is not True:
        errors.append("final_hold_state.active must be true")
    if state.get("status") != "no_cost_final_hold_active":
        errors.append("final_hold_state.status must be no_cost_final_hold_active")
    if state.get("final_hold_only") is not True:
        errors.append("final_hold_state.final_hold_only must be true")
    if state.get("service_operation_locked") is not True:
        errors.append("final_hold_state.service_operation_locked must be true")
    if state.get("resume_blocked") is not True:
        errors.append("final_hold_state.resume_blocked must be true")
    ...
    operator_actions = final_hold.get("operator_actions")
    ...
    for finding in _scan_forbidden_true(final_hold):
        errors.append(f"training_no_cost_final_hold: {finding}")
    ...
    return {
        ...
        "require_active": require_active,
        "final_hold_manifest_path": str(resolved_manifest),
        "schema_version": final_hold.get("schema_version"),
        "final_hold_active": state.get("active") is True,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a final no-cost hold manifest.")
    parser.add_argument("final_hold_manifest", type=Path, help="Path to *-training-no-cost-final-hold-manifest.json.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_training_no_cost_final_hold(
        args.final_hold_manifest,
        require_active=not args.allow_not_active,
    )
    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    elif result["ok"]:
        print("PASS report quality training no-cost final hold validated")
        print(f"final_hold_active={str(result['final_hold_active']).lower()}")
        ...
        print("FAIL report quality training no-cost final hold validation failed")
