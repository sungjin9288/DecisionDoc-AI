#!/usr/bin/env python3
"""Create a pending human sign-off record for a no-cost evidence bundle handoff."""
...
DEFAULT_TEMPLATE_PATH = (
    REPO_ROOT / "docs/specs/report_quality_learning/training_no_cost_evidence_bundle_handoff_signoff_template.json"
)
...
def _safe_signoff_id(value: str | None) -> str:
    signoff_id = (
        ...
        else f"rqp_training_no_cost_evidence_bundle_handoff_signoff_{uuid4().hex}"
    )
    if not SIGNOFF_ID_PATTERN.fullmatch(signoff_id):
        raise ValueError(
            "signoff id must match "
            "rqp_training_no_cost_evidence_bundle_handoff_signoff_[A-Za-z0-9_-]{8,96}"
        )
    return signoff_id
...
def _default_output_path(handoff_manifest_path: Path) -> Path:
    ...
    if name.endswith("-training-no-cost-evidence-bundle-handoff-manifest.json"):
        base = name.removesuffix("-training-no-cost-evidence-bundle-handoff-manifest.json")
    else:
        base = handoff_manifest_path.stem
    return handoff_manifest_path.with_name(f"{base}-training-no-cost-evidence-bundle-handoff-signoff.json")
...
def _evidence_to_review(handoff_manifest_path: Path, handoff_manifest: dict[str, Any]) -> list[str]:
    ...
    _append_unique_path(paths, handoff_manifest.get("evidence_bundle_path"))
...
def build_pending_training_no_cost_evidence_bundle_handoff_signoff(
    *,
    ...
    handoff_validation = _HANDOFF_VALIDATOR.validate_training_no_cost_evidence_bundle_handoff(
        resolved_handoff_manifest,
        require_ready=require_ready,
    )
    if require_ready and handoff_validation.get("ok") is not True:
        errors = "; ".join(handoff_validation.get("errors") or ["no-cost evidence bundle handoff validation failed"])
        raise ValueError(f"no-cost evidence bundle handoff is not ready for sign-off generation: {errors}")
    ...
    signoff.update(
        {
            ...
            "generation_context": {
                "report_type": "report_quality_training_no_cost_evidence_bundle_handoff_pending_signoff_generation",
            ...
            "next_step_after_generation": (
                ...
                "validate_report_quality_review_packet_training_no_cost_evidence_bundle_handoff_signoff.py "
                "--require-complete. This sign-off records final archive handoff review only and does not "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create a pending no-cost evidence bundle handoff sign-off JSON record.")
    parser.add_argument(
        "handoff_manifest",
        type=Path,
        help="Path to *-training-no-cost-evidence-bundle-handoff-manifest.json.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        signoff = build_pending_training_no_cost_evidence_bundle_handoff_signoff(
        ...
        if args.json:
            ...
            print("FAIL report quality training no-cost evidence bundle handoff pending signoff generation failed")
            print(f"ERROR {exc}")
        return 1
    ...
    if args.json:
        ...
        print("Report quality training no-cost evidence bundle handoff pending signoff: PASS")
