#!/usr/bin/env python3
"""Validate a DocumentOps Phase 84 validated handoff sign-off record."""
...
EXPECTED_VALIDATION_REPORT_TYPE = (
    "document_ops_phase85_local_feature_completion_validated_closure_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_"
    "closure_receipt_validated_handoff_signoff_validation"
)
PHASE84_HANDOFF_PATH = (
...
SIGNOFF_ID_PREFIX = (
    ...
    "signoff_closure_receipt_validated_handoff_summary_handoff_signoff_"
)
...
REQUIRED_ACKS = (
    "phase84_validated_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_summary_handoff_reviewed",
    "phase82_summary_chain_reviewed",
    "phase83_summary_validation_reviewed",
...
def validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff(
    ...
    *,
    ...
    if payload.get("phase") != 85:
        errors.append("phase must be 85")
    if payload.get("signoff_scope") != (
        "evidence_only_review_of_phase84_validated_closure_receipt_validated_handoff_"
        "signoff_closure_receipt_validated_handoff_summary_handoff"
    ):
        errors.append(
            "signoff_scope must be evidence_only_review_of_phase84_validated_closure_receipt_"
            "validated_handoff_signoff_closure_receipt_validated_handoff_summary_handoff"
        )
    ...
    if source_path_value != PHASE84_HANDOFF_PATH:
        errors.append("source_handoff.path must reference the Phase 84 validated handoff JSON")
        source_path = _repo_path(PHASE84_HANDOFF_PATH)
    else:
    ...
    if not source_path.exists():
        ...
        if source.get("sha256") != _sha256_file(source_path):
            errors.append("source_handoff.sha256 must match the Phase 84 validated handoff JSON")
        source_validation = (
            validate_documentops_local_feature_completion_validated_closure_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff_signoff_closure_receipt_validated_handoff(
                source_path
            )
        ...
        if source_validation.get("ok") is not True:
            errors.append("source_handoff must pass Phase 84 validation")
            for error in source_validation.get("errors", []):
                errors.append(f"source_handoff: {error}")

    validator_path = _repo_path(PHASE84_VALIDATOR_PATH)
    if source.get("validator") != PHASE84_VALIDATOR_PATH:
        errors.append(f"source_handoff.validator must be {PHASE84_VALIDATOR_PATH}")
    if not validator_path.exists():
        errors.append(f"source_handoff.validator path must exist: {PHASE84_VALIDATOR_PATH}")
    elif source.get("validator_sha256") != _sha256_file(validator_path):
        errors.append(f"source_handoff.validator_sha256 must match {PHASE84_VALIDATOR_PATH}")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate a DocumentOps Phase 84 validated handoff sign-off record."
    )
    parser.add_argument("signoff", type=Path, help="Path to Phase 85 handoff sign-off JSON.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            ...
            "closure receipt validated handoff sign-off validated"
        )
        ...
        print(
            ...
            "closure receipt validated handoff signoff closure receipt validated handoff signoff "
            "closure receipt validated handoff sign-off validation failed"
        )
