#!/usr/bin/env python3
"""Validate a local handoff for report quality training no-cost freezes."""
...
def _load_summary_script():
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"failed to load no-cost freeze summary script: {SUMMARY_SCRIPT_PATH}")
...
def validate_training_no_cost_freeze_handoff(
    ...
    *,
    ...
    try:
        ...
        return {
            "report_type": "report_quality_training_no_cost_freeze_handoff_validation",
            "ok": False,
    ...
    if handoff.get("report_type") != "report_quality_training_no_cost_freeze_handoff":
        errors.append("report_type must be report_quality_training_no_cost_freeze_handoff")
    ...
    if markdown_path is not None:
        ...
        if "Report Quality Training No-Cost Freeze Handoff" not in markdown:
    ...
    summary_path = _resolve_path(handoff.get("freeze_summary_path"), field="freeze_summary_path", errors=errors)
    _validate_hash(
        path=summary_path,
        expected_hash=handoff.get("freeze_summary_sha256"),
        field="freeze_summary_sha256",
        errors=errors,
    )
    rebuilt_summary: dict[str, Any] = {}
    if summary_path is not None:
        summary = _load_json(summary_path)
        rebuilt_summary = _SUMMARY_SCRIPT.build_training_no_cost_freeze_summary(_summary_freeze_paths(summary))
        if require_ready and rebuilt_summary.get("ok") is not True:
            errors.append("rebuilt no-cost freeze summary validation must pass")
        if summary.get("ok") is not True:
            errors.append("freeze summary ok must be true")
        readiness = _as_dict(summary.get("readiness"))
        if readiness.get("status") != "all_freezes_confirm_no_cost_hold":
            errors.append("freeze summary status must be all_freezes_confirm_no_cost_hold")

    embedded_summary_validation = _as_dict(handoff.get("freeze_summary_validation"))
    if require_ready and embedded_summary_validation.get("ok") is not True:
        errors.append("embedded freeze_summary_validation.ok must be true")
    ...
    if readiness.get("status") != "no_cost_freeze_handoff_ready":
        errors.append("readiness.status must be no_cost_freeze_handoff_ready")
    if readiness.get("freeze_only") is not True:
        errors.append("readiness.freeze_only must be true")
    ...
    if rebuilt_counts:
        for field in ("freeze_count", "valid_freeze_count", "no_cost_hold_count"):
            if counts.get(field) != rebuilt_counts.get(field):
                errors.append(f"counts.{field} must match rebuilt freeze summary")
    ...
    for finding in _scan_forbidden_true(handoff):
        errors.append(f"training_no_cost_freeze_handoff: {finding}")
    ...
    return {
        ...
        "freeze_summary_validation_ok": rebuilt_summary.get("ok") if rebuilt_summary else None,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a local no-cost freeze handoff manifest.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = validate_training_no_cost_freeze_handoff(
    ...
    if args.json:
        ...
        print("PASS report quality training no-cost freeze handoff validated")
        ...
        print("FAIL report quality training no-cost freeze handoff validation failed")
