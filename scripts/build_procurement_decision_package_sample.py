from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from app.services.procurement_decision_package_service import (
    LOCAL_DEMO_SAMPLE_DIR,
    LOCAL_DEMO_SAMPLE_INPUT_PATH,
    build_and_write,
    build_sample_builder_failure_result,
)


SAMPLE_DIR = ROOT / LOCAL_DEMO_SAMPLE_DIR
DEFAULT_SAMPLE_INPUT_PATH = ROOT / LOCAL_DEMO_SAMPLE_INPUT_PATH
DEFAULT_OUTPUT_DIR = Path("/tmp/decisiondoc-procurement-decision-package-local-demo")


def _emit_result(result: dict[str, object], *, exit_code: int) -> int:
    print(json.dumps(result, indent=2))
    return exit_code


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build the local procurement decision package demo artifacts."
    )
    parser.add_argument("--sample-input", type=Path, default=DEFAULT_SAMPLE_INPUT_PATH)
    parser.add_argument("--out-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    return parser.parse_args()


def main() -> int:
    args = _parse_args()
    try:
        result = build_and_write(sample_input_path=args.sample_input, output_dir=args.out_dir)
    except Exception as exc:
        result = build_sample_builder_failure_result(
            sample_input_path=args.sample_input,
            output_dir=args.out_dir,
            exc=exc,
        )
        return _emit_result(result, exit_code=1)

    return _emit_result(result, exit_code=0)


if __name__ == "__main__":
    raise SystemExit(main())
