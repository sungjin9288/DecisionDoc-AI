...
from scripts.check_pilot_delivery_ready import check_pilot_delivery_ready
...
def test_check_pilot_delivery_ready_publishes_and_asserts(tmp_path):
    ...
    assert Path(str(result["snapshot_file"])).exists()
    assert Path(str(result["latest_file"])).exists()
