...
from scripts.check_pilot_delivery_latest_summary_ready import (
    check_pilot_delivery_latest_summary_ready,
    main,
)
...
def _seed_closeout(tmp_path: Path) -> tuple[Path, Path]:
...
def test_check_pilot_delivery_latest_summary_ready_refreshes_and_reports(tmp_path):
    closeout_file, pilot_dir = _seed_closeout(tmp_path)
    ...
    assert result["latest_overview_markdown"].endswith("latest-pilot-delivery-overview.md")


def test_check_pilot_delivery_latest_summary_ready_json_output(tmp_path, capsys):
    closeout_file, pilot_dir = _seed_closeout(tmp_path)
    ...
    assert payload["latest_overview_markdown"].endswith("latest-pilot-delivery-overview.md")
