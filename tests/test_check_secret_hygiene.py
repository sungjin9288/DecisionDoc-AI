...
def test_secret_hygiene_flags_github_actions_hardcoded_credentials(tmp_path: Path) -> None:
    _init_git_repo(tmp_path)
    _track_file(
        tmp_path,
        ".github/workflows/example.yml",
    ...
    assert (
        f".github/workflows/example.yml:6: possible AWS access key id {ACCESS_KEY_ID}" in completed.stderr
    )
    assert ".github/workflows/example.yml:6: credential assignment pattern detected" in completed.stderr
...
def test_secret_hygiene_allows_kubernetes_secret_key_ref_patterns(tmp_path: Path) -> None:
    _init_git_repo(tmp_path)
    _track_file(
        tmp_path,
        "deployment.yaml",
...
def test_secret_hygiene_allows_kubernetes_secret_key_ref_optional_patterns(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _track_file(
...
def test_secret_hygiene_allows_kubernetes_config_map_key_ref_patterns(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _track_file(
...
def test_secret_hygiene_allows_kubernetes_env_from_config_map_ref_with_prefix_patterns(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _track_file(
...
def test_secret_hygiene_flags_powershell_env_assignment_patterns(tmp_path: Path) -> None:
    _init_git_repo(tmp_path)
    _track_file(
        tmp_path,
        "scripts/env.ps1",
    ...
    assert completed.returncode == 1
    assert f"scripts/env.ps1:1: possible AWS access key id {ACCESS_KEY_ID}" in completed.stderr
    assert "scripts/env.ps1:1: credential assignment pattern detected" in completed.stderr
