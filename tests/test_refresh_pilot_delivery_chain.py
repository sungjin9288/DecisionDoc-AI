...
def test_refresh_pilot_delivery_chain_rebuilds_artifacts(tmp_path):
    ...
    assert result["pilot_status"] == "PILOT_COMPLETE"
    assert result["verification_status"] == "PASS"
    assert result["audit_status"] == "PASS"
    assert result["receipt_matches"] is True
    assert Path(str(result["bundle_file"])).exists()
    assert Path(str(result["manifest_file"])).exists()
    assert Path(str(result["receipt_file"])).exists()
    assert Path(str(result["audit_file"])).exists()
