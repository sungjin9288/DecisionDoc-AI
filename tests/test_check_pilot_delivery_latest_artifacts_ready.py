...
from scripts.check_pilot_delivery_latest_artifacts_ready import (
    check_pilot_delivery_latest_artifacts_ready,
)
...
def test_check_pilot_delivery_latest_artifacts_ready_syncs_and_asserts(tmp_path):
    ...
    assert Path(str(result["latest_status_file"])).exists()
    assert Path(str(result["latest_audit_file"])).exists()
