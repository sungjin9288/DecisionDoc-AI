...
def _write_complete_handoff_docs(repo: Path) -> None:
    _write_fixture_doc(
        ...
        "admin_v1_1_58_acceptance_20260430.md\n"
        "Sales Pack 인덱스",
    )
    _write_fixture_doc(
        repo,
...
def test_company_handoff_ready_fails_when_latest_acceptance_is_stale(tmp_path: Path) -> None:
    ...
    _write_complete_handoff_docs(tmp_path)
    _write_complete_pdfs(tmp_path)
    acceptance = tmp_path / "docs" / "deployment" / "admin_v1_1_58_acceptance_20260430.md"
    acceptance.write_text("# Fixture\n\nv1.1.57\n\n" + ("내용\n" * 80), encoding="utf-8")

    result = checker.check_company_handoff_ready(repo_root=tmp_path)

    assert result["ok"] is False
    assert "required text not found in docs/deployment/admin_v1_1_58_acceptance_20260430.md: v1.1.58" in result["errors"]
