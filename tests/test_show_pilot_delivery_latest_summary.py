...
from scripts.publish_pilot_delivery_latest_overview import publish_pilot_delivery_latest_overview
from scripts.refresh_pilot_delivery_chain import refresh_pilot_delivery_chain
from scripts.show_pilot_delivery_latest_summary import (
    main,
    show_pilot_delivery_latest_summary,
)
...
def _seed_latest_artifacts(tmp_path: Path) -> Path:
    ...
    publish_pilot_delivery_latest_overview(closeout_file=closeout_file, output_dir=pilot_dir)
    return pilot_dir


def test_show_pilot_delivery_latest_summary_reports_current_latest_files(tmp_path):
    pilot_dir = _seed_latest_artifacts(tmp_path)

    result = show_pilot_delivery_latest_summary(output_dir=pilot_dir)

    assert result["ok"] is True
    assert result["status"] == "PASS"
    assert result["stale"] is False
    assert any(item["name"] == "overview_markdown" and item["status"] == "ok" for item in result["checks"])


def test_show_pilot_delivery_latest_summary_json_output(tmp_path, capsys):
    pilot_dir = _seed_latest_artifacts(tmp_path)

    exit_code = main(["--output-dir", str(pilot_dir), "--json"])
