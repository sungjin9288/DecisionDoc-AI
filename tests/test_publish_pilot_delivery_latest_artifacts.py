...
from scripts.publish_pilot_delivery_latest_artifacts import publish_pilot_delivery_latest_artifacts
...
def test_publish_pilot_delivery_latest_artifacts_syncs_status_and_audit(tmp_path):
    ...
    latest_status = json.loads(Path(result["latest_status_file"]).read_text(encoding="utf-8"))
    latest_audit = Path(result["latest_audit_file"]).read_text(encoding="utf-8")
    assert latest_status["status"] == "PASS"
    assert latest_status["stale"] is False
    assert "audit_status: **PASS**" in latest_audit
