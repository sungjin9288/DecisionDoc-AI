...
from scripts.publish_pilot_delivery_latest_audit import publish_pilot_delivery_latest_audit
...
def test_publish_pilot_delivery_latest_audit_writes_stable_markdown(tmp_path):
    ...
    assert "audit_status: **PASS**" in latest
    assert "receipt_matches_current_verification: `true`" in latest
