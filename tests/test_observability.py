...
def test_meeting_recording_logs_capture_missing_project_state_errors(tmp_path, monkeypatch, caplog, capsys):
    caplog.set_level(logging.INFO)
    monkeypatch.setenv("OPENAI_API_KEY", "test-openai-key")
    client = _create_client(tmp_path, monkeypatch)
    ...
    transcribe_events = [
        event for event in events
        if event.get("event") == "request.completed"
        and event.get("path") == "/projects/missing-project/recordings/missing-recording/transcribe"
        and event.get("meeting_recording_action") == "transcribe"
    ]
    assert transcribe_events
    assert transcribe_events[-1]["status_code"] == 404
    assert transcribe_events[-1]["error_code"] == "project_not_found"
    assert transcribe_events[-1]["meeting_recording_project_id"] == "missing-project"
    assert transcribe_events[-1]["meeting_recording_recording_id"] == "missing-recording"

    approve_events = [
        event for event in events
        if event.get("event") == "request.completed"
        and event.get("path") == "/projects/missing-project/recordings/missing-recording/approve"
        and event.get("meeting_recording_action") == "approve"
    ]
    assert approve_events
    assert approve_events[-1]["status_code"] == 404
    assert approve_events[-1]["error_code"] == "project_not_found"
    assert approve_events[-1]["meeting_recording_project_id"] == "missing-project"
    assert approve_events[-1]["meeting_recording_recording_id"] == "missing-recording"

    generate_events = [
        event for event in events
        if event.get("event") == "request.completed"
        and event.get("path") == "/projects/missing-project/recordings/missing-recording/generate-documents"
        and event.get("meeting_recording_action") == "generate_documents"
    ]
    assert generate_events
    assert generate_events[-1]["status_code"] == 404
    assert generate_events[-1]["error_code"] == "project_not_found"
    assert generate_events[-1]["meeting_recording_project_id"] == "missing-project"
    assert generate_events[-1]["meeting_recording_recording_id"] == "missing-recording"
