...
| DocumentOps 검토 작업대 | tenant-scoped trajectory를 검색·필터·정렬하고 summary 목록에서 선택한 기록만 상세 조회한다. 사람 검토는 메모와 품질 점수를 export/freeze/governance 증적으로 연결하며, 상세 열람과 review 결정은 본문·메모를 제외한 append-only audit으로 추적한다. |
