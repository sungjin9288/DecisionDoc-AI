...
테스트 함수는 **3,494개**, **255개 파일**입니다 (AST source definition 기준 카운트). 자동생성 phase 영수증 검증 테스트(제품 기능과 무관)는 2026-07-02 정리에서 제거해 수치에서 제외했습니다.

```bash
python3 scripts/count_readme_metrics.py --field test_functions  # → 3494
...
현재 non-live test suite는 통과했습니다 (`pytest tests/ -m "not live" -q` → 4,250 passed, 2 skipped, 4 deselected, 1 warning, 2026-07-21 H91 실측). "완성"을 막는 갭과 마일스톤은 [docs/development-plan.md](./docs/development-plan.md)에 정의돼 있습니다.
...
- Freeze·dry-run approval·execution request·audit export는 caller가 보낸 optional `operation_id`와 canonical request payload SHA-256을 private trajectory metadata에 함께 결속합니다. 같은 tenant·collection에서 동일 operation과 payload를 재생하면 원래 artifact의 size·SHA-256·tenant·identity를 다시 검증해 반환하고, payload가 다르면 `409 Conflict`로 중단합니다. Operation receipt의 부분 필드·잘못된 hash·중복 identity는 원본을 덮어쓰지 않고 fail closed 처리하며 public 목록에는 receipt를 노출하지 않습니다. 다만 artifact를 먼저 immutable publish한 뒤 metadata CAS를 확정하므로 같은 operation이 여러 worker에서 동시에 시작되면 CAS loser의 비권위 orphan이 남을 수 있습니다. 이 보장은 authoritative metadata record 중복 방지 범위이며 distributed transaction, zero-orphan, provider-backed Agent 실행 idempotency를 주장하지 않습니다.
...
<sub>이 README의 모든 정량 수치(라우트 268 · 테스트 3,494 · env 키 94 등)는 소스 코드에서 직접 카운트했으며, 재현 커맨드를 함께 표기했습니다. 측정 근거가 없는 비용 절감률·자동화율·정확도 수치는 사용하지 않습니다.</sub>
